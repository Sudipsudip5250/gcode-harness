...
struct SearchInput {
    ...
    #[serde(default)]
    canary: Option<bool>,
    /// Restrict source: gcode, claude, codex, pi, opencode, or all.
    #[serde(default)]
...
impl Tool for SessionSearchTool {
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "source": {
                    "type": "string",
                    "enum": ["all", "gcode", "claude", "codex", "pi", "opencode"],
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        let sessions_dir = storage::gcode_dir()?.join("sessions");
...
fn normalize_source_filter(raw: Option<&str>) -> std::result::Result<Option<String>, String> {
    ...
    match normalized.as_str() {
        "all" => Ok(None),
        "gcode" | "claude" | "claude-code" | "codex" | "pi" | "opencode" => {
            Ok(Some(normalized.replace("claude-code", "claude")))
        }
        _ => Err(format!(
            "source must be one of all, gcode, claude, codex, pi, or opencode; received {source}."
        )),
    }
}
...
fn search_sessions_blocking(
    ...
    if source_matches_filter("gcode", options) {
        ...
        if !files.is_empty() {
            ...
            report.scanned_gcode_sessions = files.len();
            ...
            if !files.is_empty() {
                ...
                report.candidate_gcode_sessions = candidates.len();
...
fn append_session_results(
    ...
    if !gcode_session_matches_filters(session, options) {
        return;
    }
    ...
    for (message_index, msg) in session.messages.iter().enumerate() {
        ...
        results.push(SearchResult {
            ...
            matched_terms: match_score.matched_terms,
            exact_match: match_score.exact_match,
            context: build_gcode_context(&session.messages, message_index, options),
        });
    }
}
...
fn gcode_session_matches_filters(session: &Session, options: &SearchOptions) -> bool {
    if !source_matches_filter("gcode", options) {
        return false;
    }
    if !provider_matches(session.provider_key.as_deref(), "gcode", options) {
        return false;
    }
