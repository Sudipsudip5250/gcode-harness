...
impl McpConfig {
    ...
    fn import_from_external() {
        ...
        if !imported.servers.is_empty() {
            if let Err(e) = imported.save_to_file(&gcode_mcp) {
    ...
    /// Load from default locations (merges gcode global + local, local overrides)
    #[expect(
        clippy::collapsible_if,
    ...
    pub fn load() -> Self {
        ...
        // Load gcode's own global config (~/.gcode/mcp.json)
        if let Ok(gcode_dir) = crate::storage::gcode_dir() {
            let gcode_mcp = gcode_dir.join("mcp.json");
            if gcode_mcp.exists() {
                if let Ok(config) = Self::load_from_file(&gcode_mcp) {
                    merged.servers.extend(config.servers);
                }
            }
        }

        // Load project-local gcode config (.gcode/mcp.json)
        let local_gcode = std::path::Path::new(".gcode/mcp.json");
        if local_gcode.exists() {
            if let Ok(config) = Self::load_from_file(local_gcode) {
                merged.servers.extend(config.servers);
            }
        }
