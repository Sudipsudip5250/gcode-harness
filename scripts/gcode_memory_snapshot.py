...
DEFAULT_SOCKET = f"/run/user/{os.getuid()}/gcode.sock"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Summarize gcode server/client process memory using smaps_rollup")
    p.add_argument("--json", action="store_true", help="Print JSON instead of a human summary")
    p.add_argument(
        "--include-aux",
        action="store_true",
        help="Include non-default-socket helper/test gcode processes in the output",
    )
    p.add_argument(
        "--socket",
        default=DEFAULT_SOCKET,
        help=f"Main gcode socket to treat as the primary instance (default: {DEFAULT_SOCKET})",
    )
    return p.parse_args()
...
def classify_process(argv: list[str], cmd: str, main_socket: str) -> tuple[str | None, bool]:
    ...
    if not argv0.startswith("gcode"):
        return None, False

    if "gcode serve" in cmd:
    ...
    if "cargo build" in cmd or "rustc --crate-name gcode" in cmd or "handle_resume_session" in cmd:
        return None, False
...
def iter_gcode_processes(main_socket: str, include_aux: bool) -> Iterable[ProcMem]:
...
def main() -> int:
    args = parse_args()
    procs = list(iter_gcode_processes(args.socket, args.include_aux))
