...
Targets:
  ...
  release-gcode    Run scripts/dev_cargo.sh build --release -p gcode --bin gcode --quiet
  selfdev-gcode    Run scripts/dev_cargo.sh build --profile selfdev -p gcode --bin gcode --quiet
...
Examples:
  ...
  scripts/bench_compile.sh release-gcode --json
  scripts/bench_compile.sh selfdev-gcode --json
USAGE
}
...
case "$target" in
  ...
  build)
    ...
    ;;
  release-gcode)
    cmd=(scripts/dev_cargo.sh build --release -p gcode --bin gcode --quiet)
    ;;
  selfdev-gcode)
    cmd=(scripts/dev_cargo.sh build --profile selfdev -p gcode --bin gcode --quiet)
    ;;
  *)
