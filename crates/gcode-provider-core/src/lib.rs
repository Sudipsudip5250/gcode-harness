...
pub trait Provider: Send + Sync {
    ...
    /// Returns true if gcode should use its own compaction for this provider.
    fn supports_compaction(&self) -> bool {
        false
    }

    /// Returns true if gcode should proactively run its own summary-based compaction.
...
/// Native tool result to send back to provider bridges that delegate tool execution to gcode.
    ...
    CLIENT
        .get_or_init(|| {
            reqwest::Client::builder()
                ...
                .build()
                .unwrap_or_else(|err| {
                    eprintln!("gcode: failed to build shared provider HTTP client: {err}");
                    reqwest::Client::new()
                })
        })
        .clone()
