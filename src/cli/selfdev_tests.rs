...
fn set_socket_test_env(socket_path: &Path, runtime_dir: &Path) -> EnvVarGuard {
    let guard = EnvVarGuard::capture(&["GCODE_SOCKET", "GCODE_RUNTIME_DIR"]);
    crate::server::set_socket_path(socket_path.to_str().expect("utf8 socket path"));
    crate::env::set_var("GCODE_RUNTIME_DIR", runtime_dir);
    guard
}
...
impl TestEnvGuard {
    fn new() -> anyhow::Result<Self> {
        ...
        let temp_home = tempfile::Builder::new()
            .prefix("gcode-selfdev-test-home-")
            .tempdir()?;
        let env = EnvVarGuard::capture(&["GCODE_HOME", "GCODE_TEST_SESSION"]);

        crate::env::set_var("GCODE_HOME", temp_home.path());
        crate::env::set_var("GCODE_TEST_SESSION", "1");
...
async fn test_wait_for_reloading_server_returns_false_when_reload_failed() {
    ...
    let socket_path = temp.path().join("gcode.sock");
...
fn isolated_launcher_env() -> (
    ...
    let lock = lock_env();
    let temp = tempfile::tempdir().expect("tempdir");
    let env = EnvVarGuard::capture(&["GCODE_INSTALL_DIR", "GCODE_HOME", "HOME", "USERPROFILE"]);
    crate::env::set_var("HOME", temp.path());
    crate::env::set_var("USERPROFILE", temp.path());
    crate::env::remove_var("GCODE_INSTALL_DIR");
...
#[test]
fn test_launcher_dir_uses_trimmed_install_dir_before_gcode_home() {
    let (_lock, _env, temp) = isolated_launcher_env();
    let install_dir = temp.path().join("install bin");
    let gcode_home = temp.path().join("gcode-home");
    set_var(
        "GCODE_INSTALL_DIR",
        format!("  {}  ", install_dir.display()),
    );
    set_var("GCODE_HOME", &gcode_home);
...
fn default_launcher_dir(home: &Path) -> PathBuf {
    if cfg!(windows) {
        home.join("AppData").join("Local").join("gcode").join("bin")
    } else {
...
fn test_selfdev_build_command_prefers_repo_wrapper_when_present() {
    ...
    assert!(command.contains("dev_cargo.sh' build --profile selfdev -p gcode --bin gcode"));
    assert!(!command.contains("gcode-desktop"));
    assert!(build.display.contains("-p gcode --bin gcode"));
    assert!(!build.display.contains("gcode-desktop"));
}
...
fn test_selfdev_build_command_falls_back_to_cargo_when_wrapper_missing() {
    ...
    assert_eq!(build.args.first().map(String::as_str), Some("-lc"));
    let command = build.args.get(1).expect("shell command");
    assert!(command.contains("cargo build --profile selfdev -p gcode --bin gcode"));
    assert!(!command.contains("gcode-desktop"));
}
...
fn test_selfdev_build_command_can_target_all() {
    ...
    assert!(
        build
            .display
            .contains("-p gcode-desktop --bin gcode-desktop")
    );
}
