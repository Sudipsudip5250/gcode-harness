...
fn test_provider_choice_aliases_parse() {
    let args = Args::try_parse_from(["gcode", "--provider", "z.ai", "run", "smoke"]).unwrap();
    assert_eq!(args.provider, ProviderChoice::Zai);

    let args =
        Args::try_parse_from(["gcode", "--provider", "kimi-for-coding", "run", "smoke"]).unwrap();
    assert_eq!(args.provider, ProviderChoice::Kimi);

    let args =
        Args::try_parse_from(["gcode", "--provider", "cerebrascode", "run", "smoke"]).unwrap();
    assert_eq!(args.provider, ProviderChoice::Cerebras);

    let args = Args::try_parse_from(["gcode", "--provider", "compat", "run", "smoke"]).unwrap();
    assert_eq!(args.provider, ProviderChoice::OpenaiCompatible);

    let args = Args::try_parse_from(["gcode", "--provider", "bailian", "run", "smoke"]).unwrap();
    assert_eq!(args.provider, ProviderChoice::AlibabaCodingPlan);

    let args = Args::try_parse_from(["gcode", "--provider", "together", "run", "smoke"]).unwrap();
    assert_eq!(args.provider, ProviderChoice::TogetherAi);

    let args = Args::try_parse_from(["gcode", "--provider", "grok", "run", "smoke"]).unwrap();
    assert_eq!(args.provider, ProviderChoice::Xai);

    let args = Args::try_parse_from(["gcode", "--provider", "cgc", "run", "smoke"]).unwrap();
    assert_eq!(args.provider, ProviderChoice::Comtegra);
}

#[test]
fn model_list_subcommand_parses() {
    let args = Args::try_parse_from(["gcode", "model", "list", "--json", "--verbose"]).unwrap();
    match args.command {
...
fn session_rename_subcommand_parses() {
    ...
    let args = Args::try_parse_from(["gcode", "session", "rename", "fox", "--clear"]).unwrap();
    match args.command {
        Some(Command::Session(SessionCommand::Rename {
            session,
            name,
...
fn login_no_browser_flag_parses() {
    let args = Args::try_parse_from(["gcode", "login", "--no-browser"]).unwrap();
    match args.command {
        Some(Command::Login {
            account,
            no_browser,
    ...
    let args = Args::try_parse_from(["gcode", "login", "--headless"]).unwrap();
    match args.command {
...
fn login_scriptable_flags_parse() {
    let args = Args::try_parse_from(["gcode", "login", "--print-auth-url", "--json"]).unwrap();
    match args.command {
        Some(Command::Login {
            print_auth_url,
            json,
    ...
    let args = Args::try_parse_from(["gcode", "login", "--auth-code", "abc123"]).unwrap();
    match args.command {
...
fn quiet_global_flag_parses() {
    let args = Args::try_parse_from(["gcode", "--quiet", "model", "list"]).unwrap();
    assert!(args.quiet);
}

#[test]
fn run_json_subcommand_parses() {
    let args = Args::try_parse_from(["gcode", "run", "--json", "hello"]).unwrap();
    match args.command {
        Some(Command::Run {
            json,
            ndjson,
...
fn run_ndjson_subcommand_parses() {
    let args = Args::try_parse_from(["gcode", "run", "--ndjson", "hello"]).unwrap();
    match args.command {
        Some(Command::Run {
            json,
            ndjson,
...
fn version_subcommand_parses() {
    let args = Args::try_parse_from(["gcode", "version", "--json"]).unwrap();
    match args.command {
...
fn usage_subcommand_parses() {
    let args = Args::try_parse_from(["gcode", "usage", "--json"]).unwrap();
    match args.command {
...
fn auth_status_subcommand_parses() {
    let args = Args::try_parse_from(["gcode", "auth", "status", "--json"]).unwrap();
    match args.command {
...
fn auth_doctor_subcommand_parses() {
    let args = Args::try_parse_from(["gcode", "auth", "doctor", "openai", "--validate", "--json"])
        .unwrap();
    match args.command {
...
fn provider_list_subcommand_parses() {
    let args = Args::try_parse_from(["gcode", "provider", "list", "--json"]).unwrap();
    match args.command {
...
fn provider_current_subcommand_parses() {
    let args = Args::try_parse_from(["gcode", "provider", "current", "--json"]).unwrap();
    match args.command {
...
fn restart_save_subcommand_parses() {
    let args = Args::try_parse_from(["gcode", "restart", "save"]).unwrap();
    match args.command {
...
fn restart_save_auto_restore_flag_parses() {
    let args = Args::try_parse_from(["gcode", "restart", "save", "--auto-restore"]).unwrap();
    match args.command {
        Some(Command::Restart {
