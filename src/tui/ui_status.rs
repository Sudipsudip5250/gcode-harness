...
pub(super) fn semver() -> &'static str {
    ...
    SEMVER.get_or_init(|| format!("v{}", env!("GCODE_SEMVER")))
}

/// True when this process is running from the stable release binary path.
/// Only matches the explicit ~/.gcode/builds/stable/gcode path, NOT
/// ~/.local/bin/gcode launcher path (which now points to current).
...
pub(super) fn binary_age() -> Option<String> {
    let git_date = env!("GCODE_GIT_DATE");
