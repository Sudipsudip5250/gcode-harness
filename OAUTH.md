...
For auth files managed by other tools/CLIs, gcode asks before reading them. If you
approve a source, gcode remembers that approval for that external auth file path
...
- J-Code Claude OAuth (if logged in via `gcode login --provider claude`): `~/.gcode/auth.json`
...
- J-Code OpenAI/Codex OAuth: `~/.gcode/openai-auth.json`
- Codex CLI auth source (read in place only after confirmation): `~/.codex/auth.json`
- Gemini native OAuth: `~/.gcode/gemini_oauth.json`
...
- OpenAI-compatible provider metadata/login descriptors: `crates/gcode-provider-metadata/src/lib.rs`

## Claude (Claude Max)

### Login steps
1. Run `gcode login --provider claude` (recommended), or `gcode login` and choose Claude.
   - For headless / SSH use: `gcode login --provider claude --no-browser`
   - For scriptable remote flows: `gcode login --provider claude --print-auth-url`, then later complete with `--callback-url` or `--auth-code`
2. Alternative: run `claude` (or `claude setup-token`). gcode can detect `~/.claude/.credentials.json`, ask before reading it, and remember that approval for future sessions.
3. Verify with `gcode --provider claude run "Say hello from gcode"`.
...
gcode owns the full runtime path itself: auth, refresh, request shaping, tool
...
if the request matches the Claude Code "OAuth contract". gcode applies this
...
Claude OAuth requests reject certain tool names. gcode remaps tool names on the
...
- `GCODE_CLAUDE_CLI_PATH` (default: `claude`)
- `GCODE_CLAUDE_CLI_MODEL` (default: `claude-opus-4-5-20251101`)
- `GCODE_CLAUDE_CLI_PERMISSION_MODE` (default: `bypassPermissions`)
- `GCODE_CLAUDE_CLI_PARTIAL` (set to `0` to disable partial streaming)

## OpenAI / Codex OAuth

### Login steps
1. Run `gcode login --provider openai`.
   - For headless / SSH use: `gcode login --provider openai --no-browser`
   - For scriptable remote flows: `gcode login --provider openai --print-auth-url`, then later complete with `--callback-url`
2. Your browser opens to the OpenAI OAuth page unless you use `--no-browser`. The local callback listens on
   ...
   If port `1455` is unavailable, gcode falls back to a manual paste flow where
   you can paste the full callback URL or query string.
3. After login, tokens are saved to `~/.gcode/openai-auth.json`.

Credential discovery order is:
1. `~/.gcode/openai-auth.json`
2. `~/.codex/auth.json`
3. trusted OpenCode/pi OAuth in `~/.local/share/opencode/auth.json` / `~/.pi/agent/auth.json`
4. `OPENAI_API_KEY`

If gcode finds existing credentials in `~/.codex/auth.json`, it asks before
reading them. When approved, it remembers that trust decision for future gcode
...
### Troubleshooting
- Claude 401/auth errors: run `gcode login --provider claude`.
- 401/403: re-run `gcode login --provider openai`.
...
### Login/setup steps
1. Run `gcode login --provider azure`.
...
4. Choose one auth mode:
   ...
   - **API key**
5. gcode saves settings to `~/.config/gcode/azure-openai.env`.
...
### Runtime behavior
- gcode normalizes the endpoint to the newer Azure OpenAI `/openai/v1` base.
- In **Entra ID** mode, gcode obtains bearer tokens using `azure_identity::DefaultAzureCredential` with scope:
  - `https://cognitiveservices.azure.com/.default`
- In **API key** mode, gcode sends the credential in the Azure-style `api-key` header.
...
- If you prefer static credentials, re-run `gcode login --provider azure` and choose API key mode.

## Gemini OAuth

### Login steps
1. Run `gcode login --provider gemini` or `/login gemini` inside the TUI.
   - For headless / SSH use: `gcode login --provider gemini --no-browser`
   - For scriptable remote flows: `gcode login --provider gemini --print-auth-url`, then later complete with `--auth-code`
2. gcode opens a browser to the Google OAuth flow used for Gemini Code Assist unless you use `--no-browser`.
3. If local callback binding is unavailable, gcode falls back to a manual paste flow using `https://codeassist.google.com/authcode`.
4. Tokens are saved to `~/.gcode/gemini_oauth.json`.

### Credential discovery order
1. Native gcode Gemini tokens: `~/.gcode/gemini_oauth.json`
2. Gemini CLI OAuth source (read only after approval): `~/.gemini/oauth_creds.json`
3. trusted OpenCode/pi OAuth in `~/.local/share/opencode/auth.json` / `~/.pi/agent/auth.json`

### Runtime notes
- gcode uses native Google OAuth and talks to the Google Code Assist backend directly.
...
- If login succeeds but requests fail later, re-run `gcode login --provider gemini` to refresh the stored session.
...
These providers use the same built-in login flow pattern: `gcode login --provider <name>`.
...
```bash
printf '%s' "$MY_API_KEY" | gcode provider add my-api \
...
gcode --provider-profile my-api auth-test --no-tool-smoke
```

This writes `[providers.my-api]` in `~/.gcode/config.toml` and stores the key in gcode's private app config dir, for example `~/.config/gcode/provider-my-api.env`. For localhost servers, use `--no-api-key`.

Two notable presets are:

### Fireworks
- Login: `gcode login --provider fireworks`
- Stored env file: `~/.config/gcode/fireworks.env`
...
### MiniMax
- Login: `gcode login --provider minimax`
- Stored env file: `~/.config/gcode/minimax.env`
- API key env var: `OPENAI_API_KEY`
- Base URL: `https://api.minimaxi.com/v1`
- Default model hint: `MiniMax-M2.7`
- Docs: <https://platform.minimax.io/docs/api-reference/text-openai-api>

These are first-class gcode provider presets, not just manual custom endpoint examples.
You can still use `openai-compatible` for arbitrary custom providers when there is not a built-in preset.

If gcode finds matching API keys in trusted OpenCode/pi auth files, it can reuse them for the corresponding provider preset without asking you to paste the key again.
...
Cursor uses gcode's native HTTPS transport. Copilot uses GitHub device-flow auth. Antigravity login/auth storage is handled natively by gcode.

### Cursor
- Login: `gcode login --provider cursor`
  - saves `CURSOR_API_KEY` to `~/.config/gcode/cursor.env`
- Runtime:
  - gcode uses native HTTPS requests
  - if a Cursor API key is configured, gcode exchanges/uses it directly
- Env vars:
  - `GCODE_CURSOR_MODEL` (default: `composer-1.5`)
  - `CURSOR_API_KEY` (optional; overrides saved key)

### GitHub Copilot
- Login: `gcode login --provider copilot`
  - Headless / SSH: `gcode login --provider copilot --no-browser`
  - Scriptable remote flow: `gcode login --provider copilot --print-auth-url`, then later `gcode login --provider copilot --complete`
  - gcode uses GitHub device code flow and can print the verification URL/QR without opening a local browser.
- Credential discovery order:
  ...
  8. `gh auth token`
- Env vars:
  - `GCODE_COPILOT_CLI_PATH` (optional override for CLI path)
  - `GCODE_COPILOT_MODEL` (default: `claude-sonnet-4`)

### Antigravity
- Login: `gcode login --provider antigravity` (native Google OAuth flow; does **not** require Antigravity to be installed)
  - Headless / SSH: `gcode login --provider antigravity --no-browser`
  - Scriptable remote flow: `gcode login --provider antigravity --print-auth-url`, then later complete with `--callback-url`
- Tokens: `~/.gcode/antigravity_oauth.json`
- Credential discovery order:
  1. native gcode tokens at `~/.gcode/antigravity_oauth.json`
  2. trusted OpenCode/pi OAuth entries when present
- Runtime:
  - gcode authenticates directly and stores/refreshes Antigravity OAuth tokens itself
  - the provider transport still shells out to the Antigravity CLI for completions if you choose `--provider antigravity`
- Env vars:
  - `GCODE_ANTIGRAVITY_CLIENT_ID` (optional override for OAuth client id)
  - `GCODE_ANTIGRAVITY_CLIENT_SECRET` (optional override for OAuth client secret)
  - `GCODE_ANTIGRAVITY_VERSION` (optional override for Antigravity request fingerprint/version)
  - `GCODE_ANTIGRAVITY_CLI_PATH` (default: `antigravity`, runtime only)
  - `GCODE_ANTIGRAVITY_MODEL` (default: `default`)
  - `GCODE_ANTIGRAVITY_PROMPT_FLAG` (default: `-p`)
  - `GCODE_ANTIGRAVITY_MODEL_FLAG` (default: `--model`)

## Google / Gmail OAuth

### Login steps
1. Run `gcode login --provider google`.
   - For headless / SSH use: `gcode login --provider google --no-browser`
   - For scriptable remote flows after credentials are already configured: `gcode login --provider google --print-auth-url`
2. If Google credentials are not configured yet, gcode first walks you through saving your client ID/client secret or importing the JSON credentials file.
...
4. Complete the printed flow later with `gcode login --provider google --callback-url '<full callback url or query>'`.

### Notes
- Google/Gmail scriptable auth requires saved OAuth client credentials first.
- The callback URL can come from a remote browser session that fails on the loopback redirect. Copy the final URL from the address bar and paste or pass it back to gcode.

## Scriptable auth state lifecycle

- gcode stores temporary scriptable login state in `~/.gcode/pending-login/*.json`
