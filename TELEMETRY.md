...
gcode collects **anonymous, minimal usage statistics** to help understand how many people use gcode, what providers/models are popular, whether onboarding works, which feature families are used, how often sessions succeed, and whether performance/regressions are improving. This data helps prioritize development without collecting prompts or code.
...
| `version` | `"0.9.1"` | Current gcode version |
| `from_version` | `"0.8.1"` | Previously recorded gcode version |
...
The UUID is randomly generated on first run and stored at `~/.gcode/telemetry_id`. It is not derived from your machine, username, email, or any identifiable information.

## How It Works

1. On first launch, gcode generates a random UUID and sends an `install` event
2. When a session begins, gcode sends a `session_start` event
3. When a session ends normally, gcode sends a `session_end` event with coarse session metrics
4. When auth succeeds, gcode sends a coarse `auth_success` event for activation-funnel analysis
5. When gcode detects a version change, it sends an `upgrade` event
6. On best-effort crash/signal handling, gcode sends a `session_crash` event
7. gcode may also send one-off onboarding milestone events and explicit feedback events when triggered
...
9. If a request fails (offline, firewall, etc.), gcode silently continues - no retries, no queuing
