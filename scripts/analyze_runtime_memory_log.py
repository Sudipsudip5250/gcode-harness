...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze gcode runtime memory JSONL logs for growth, spikes, attribution, and optimization hints"
    )
    parser.add_argument("paths", nargs="*", help="Specific JSONL files or directories to analyze")
    parser.add_argument(
        "--log-dir",
        help="Directory containing runtime memory JSONL logs (default: ~/.gcode/logs/memory or $GCODE_HOME/logs/memory)",
    )
...
def default_log_dir() -> Path:
    gcode_home = os.environ.get("GCODE_HOME")
    if gcode_home:
        return Path(gcode_home).expanduser() / "logs" / "memory"
    return Path.home() / ".gcode" / "logs" / "memory"
