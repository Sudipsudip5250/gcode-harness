use super::{
    ...
    escape_shell_single_quotes, launch_command_for_macos_terminal, paused_gcode_shell_command,
...
pub(super) fn install_macos_app_launcher() -> Result<(PathBuf, MacTerminalKind)> {
    ...
    let launcher_path = macos_dir.join("gcode-launcher");
...
<dict>
    ...
    <string>com.gcode.launcher</string>
    <key>CFBundleVersion</key>
    <string>{version}</string>
    <key>CFBundleShortVersionString</key>
    <string>{version}</string>
    <key>CFBundleExecutable</key>
    <string>gcode-launcher</string>
...
fn legacy_macos_app_launcher_dir() -> Result<PathBuf> {
    let home = dirs::home_dir().context("Could not find home directory")?;
    Ok(home.join("Applications").join("gcode.app"))
}
