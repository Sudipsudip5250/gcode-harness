...
# Run from the gcode repo root

set -euo pipefail

ITERATIONS=${1:-5}
RESULTS_FILE="/tmp/gcode_tool_benchmark_$(date +%Y%m%d_%H%M%S).csv"

echo "=== gcode Tool Call Benchmark ==="
...
# Find debug socket
DEBUG_SOCK="${GCODE_DEBUG_SOCK:-/run/user/$(id -u)/gcode-debug.sock}"

if [ ! -S "$DEBUG_SOCK" ]; then
    echo "ERROR: Debug socket not found at $DEBUG_SOCK"
    echo "Make sure gcode is running with debug control enabled."
    exit 1
fi
