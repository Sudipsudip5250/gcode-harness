...
pub fn selfdev_build_command_for_target(
    ...
    let specs = match target {
        SelfDevBuildTarget::Tui => vec![("gcode", "gcode")],
        SelfDevBuildTarget::Desktop => vec![("gcode-desktop", "gcode-desktop")],
        SelfDevBuildTarget::All | SelfDevBuildTarget::Auto => {
            vec![("gcode", "gcode"), ("gcode-desktop", "gcode-desktop")]
        }
    };
...
fn infer_selfdev_build_target(repo_dir: &Path) -> SelfDevBuildTarget {
    ...
    for line in text.lines() {
        ...
        if path == "Cargo.toml" || path == "Cargo.lock" || path.starts_with(".cargo/") {
            desktop = true;
            other = true;
        } else if path.starts_with("crates/gcode-desktop/") {
            desktop = true;
...
///
/// Defaults to `~/.local/bin` on Unix, `%LOCALAPPDATA%\gcode\bin` on Windows.
/// Overridable with `GCODE_INSTALL_DIR`.
pub fn launcher_dir() -> Result<PathBuf> {
    if let Some(custom) = non_empty_env_path("GCODE_INSTALL_DIR") {
        return Ok(custom);
    }

    if let Some(sandbox_home) = non_empty_env_path("GCODE_HOME") {
    ...
    #[cfg(windows)]
    {
        if let Ok(local) = std::env::var("LOCALAPPDATA") {
            return Ok(PathBuf::from(local).join("gcode").join("bin"));
        }
        Ok(home_dir()?
            .join("AppData")
            .join("Local")
...
/// Path to the launcher binary (`~/.local/bin/gcode` by default).
...
/// Check if a directory is the gcode repository
pub fn is_gcode_repo(dir: &Path) -> bool {
    // Check for Cargo.toml with name = "gcode"
