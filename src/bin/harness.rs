...
use clap::Parser;
use gcode::id::new_id;
use gcode::message::{Message, ToolDefinition};
use gcode::provider::{EventStream, Provider};
use gcode::tool::{Registry, ToolContext, ToolExecutionMode};
use serde_json::json;
use std::path::PathBuf;
use std::sync::Arc;

#[derive(Parser)]
#[command(name = "gcode-harness")]
...
fn create_temp_workspace() -> Result<PathBuf> {
    ...
    path.push(format!("gcode-harness-{}", new_id("run")));
    Ok(path)
}
