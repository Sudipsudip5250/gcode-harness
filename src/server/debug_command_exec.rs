...
use anyhow::Result;
use gcode_agent_runtime::{InterruptSignal, SoftInterruptSource};
...
pub(super) fn debug_message_timeout_secs() -> Option<u64> {
    let raw = std::env::var("GCODE_DEBUG_MESSAGE_TIMEOUT_SECS").ok()?;
...
pub(super) async fn execute_debug_command(
    ...
    if trimmed == "mcp" || trimmed == "mcp:servers" {
        ...
        let config_path = if let Ok(gcode_dir) = crate::storage::gcode_dir() {
            let path = gcode_dir.join("mcp.json");
            if path.exists() {
    ...
    if trimmed == "reload" {
        let repo_dir = crate::build::get_repo_dir()
            .ok_or_else(|| anyhow::anyhow!("Could not find gcode repository directory"))?;

        let target_binary = crate::build::find_dev_binary(&repo_dir)
            .unwrap_or_else(|| build::release_binary_path(&repo_dir));
        if !target_binary.exists() {
            return Err(anyhow::anyhow!(format!(
                "No binary found at {}. Run 'gcode self-dev --build' first, or build with 'scripts/dev_cargo.sh build --profile selfdev -p gcode --bin gcode' and publish current.",
        ...
        let gcode_dir = crate::storage::gcode_dir()?;
        let info_path = gcode_dir.join("reload-info");
...
mod tests {
    ...
    use gcode_agent_runtime::InterruptSignal;
    ...
    async fn debug_tool_selfdev_reload_returns_promptly_for_direct_execution() {
        ...
        let _test_session = EnvGuard::set("GCODE_TEST_SESSION", "1");
        let _debug_control = EnvGuard::set("GCODE_DEBUG_CONTROL", "1");
