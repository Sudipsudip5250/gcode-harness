<#
.SYNOPSIS
    Install gcode on Windows.
.DESCRIPTION
    Downloads the latest gcode release and installs it to %LOCALAPPDATA%\gcode\bin.

    One-liner install:
      irm https://raw.githubusercontent.com/bitan-del/gcode-harness/master/scripts/install.ps1 | iex

    Or download and run (allows parameters):
      & ([scriptblock]::Create((irm https://raw.githubusercontent.com/bitan-del/gcode-harness/master/scripts/install.ps1)))
.PARAMETER InstallDir
    Override the installation directory (default: $env:LOCALAPPDATA\gcode\bin)
.PARAMETER Version
    Override the version tag to install. Required when using a local artifact path.
.PARAMETER ArtifactExePath
    Use a local gcode.exe artifact instead of downloading from GitHub.
.PARAMETER ArtifactTgzPath
    Use a local gcode .tar.gz artifact instead of downloading from GitHub.
...
if (-not $InstallDir) {
    $InstallDir = Join-Path $env:LOCALAPPDATA "gcode\bin"
}

$JcodeHome = if ($env:GCODE_HOME) {
    $env:GCODE_HOME
...
function Invoke-ProcessWithTimeout {
    ...
    $stderrPath = $null
    if ($CaptureOutput) {
        $stdoutPath = Join-Path $env:TEMP ("gcode-{0}-{1}-stdout.log" -f $FriendlyName, [guid]::NewGuid().ToString('N'))
        $stderrPath = Join-Path $env:TEMP ("gcode-{0}-{1}-stderr.log" -f $FriendlyName, [guid]::NewGuid().ToString('N'))
...
function Stop-JcodeHotkeyListeners {
    try {
        Get-CimInstance Win32_Process -Filter "Name = 'powershell.exe' OR Name = 'pwsh.exe'" -ErrorAction SilentlyContinue |
            Where-Object { $_.CommandLine -like '*gcode-hotkey*' } |
...
function Install-JcodeHotkey([string]$JcodeExePath) {
    ...
    $ps1Path = Join-Path $HotkeyDir "gcode-hotkey.ps1"
    $ps1Lines = @(
        '# gcode Alt+; global hotkey listener',
    ...
    $vbsPath = Join-Path $HotkeyDir "gcode-hotkey-launcher.vbs"
    $vbsContent = @(
    ...
    $startupShortcutPath = (Join-Path $startupDir "gcode-hotkey.lnk").Replace("'", "''")
    ...
    $shortcutLines = @(
        ...
        "`$shortcut.Description = 'gcode Alt+; hotkey listener'",
    ...
    Write-Info "Configured Alt+; to launch gcode in Alacritty"
    return $true
}
...
function Get-JcodeWindowsArtifact {
    ...
    foreach ($arch in $candidates) {
        switch -Regex ($arch.Trim()) {
            '^(X64|AMD64|x86_64)$' { return "gcode-windows-x86_64" }
            '^(Arm64|ARM64|AARCH64|aarch64)$' { return "gcode-windows-aarch64" }
        }
    }
...
$BuildsDir = Join-Path $env:LOCALAPPDATA "gcode\builds"
...
if ($Existing) {
    if ($Existing -match [regex]::Escape($VersionNum)) {
        Write-Info "gcode $Version is already installed - reinstalling"
    } else {
        Write-Info "Updating gcode $Existing -> $Version"
    }
} else {
    Write-Info "Installing gcode $Version"
}
...
$TempDir = Join-Path $env:TEMP "gcode-install-$(Get-Random)"
New-Item -ItemType Directory -Path $TempDir -Force | Out-Null

$DownloadMode = ""
$DownloadPath = Join-Path $TempDir "gcode.download"
...
$DestBin = Join-Path $VersionDir "gcode.exe"
...
if ($DownloadMode -eq "tar") {
    ...
    $SrcDir = Join-Path $TempDir "gcode-src"
    ...
    Write-Info "Building gcode from source (this can take several minutes)..."
    ...
    $BuiltBin = Join-Path $SrcDir "target\release\gcode.exe"
    if (-not (Test-Path $BuiltBin)) { Write-Err "Built binary not found at $BuiltBin" }
    Copy-Item -Path $BuiltBin -Destination $DestBin -Force
}

Copy-Item -Path $DestBin -Destination (Join-Path $StableDir "gcode.exe") -Force
Set-Content -Path (Join-Path $BuildsDir "stable-version") -Value $VersionNum
Copy-Item -Path (Join-Path $StableDir "gcode.exe") -Destination $LauncherPath -Force
...
Write-Host ""
Write-Info "gcode $Version installed successfully!"
Write-Host ""
...
if ($configuredHotkey) {
    Write-Info "Global hotkey ready: Alt+; opens gcode in Alacritty"
    Write-Host ""
}

if (Get-Command gcode -ErrorAction SilentlyContinue) {
    Write-Info "Run 'gcode' to get started."
} else {
    Write-Host "  Open a new terminal window, then run:"
    Write-Host ""
    Write-Host "    gcode" -ForegroundColor Green
}
