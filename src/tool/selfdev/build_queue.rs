...
impl SelfDevTool {
    ...
    pub(super) async fn do_build(
        ...
        let repo_dir =
            SelfDevTool::resolve_repo_dir(ctx.working_dir.as_deref()).ok_or_else(|| {
                anyhow::anyhow!("Could not find the gcode repository directory for selfdev build")
            })?;
    ...
    pub(super) async fn do_test(
        ...
        let repo_dir =
            SelfDevTool::resolve_repo_dir(ctx.working_dir.as_deref()).ok_or_else(|| {
                anyhow::anyhow!("Could not find the gcode repository directory for selfdev test")
            })?;
        let requested_source = SelfDevTool::requested_source_state(&repo_dir)?;
