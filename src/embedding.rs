//! Embedding facade for gcode.
//!
//! The heavy ONNX/tokenizer implementation lives in the `gcode-embedding`
//! workspace crate so unchanged embedding code can stay cached across self-dev
//! builds. This module keeps gcode's process-wide cache, stats, and local path /
//! logging integration stable.

use anyhow::Result;
use gcode_embedding as backend;
use serde::Serialize;
...
pub fn models_dir() -> Result<PathBuf> {
    let dir = gcode_dir()?.join("models").join(backend::MODEL_NAME);
