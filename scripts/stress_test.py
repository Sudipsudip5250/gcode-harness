#!/usr/bin/env python3
"""
gcode Stress Test: Spawn 40 sessions via debug socket, measure performance.
...
MAIN_SOCK = f"/run/user/{os.getuid()}/gcode.sock"
DEBUG_SOCK = f"/run/user/{os.getuid()}/gcode-debug.sock"
...
def get_server_pid():
    ...
    try:
        ...
        for line in result.stdout.splitlines():
            if "gcode.sock" in line and "LISTEN" in line:
                parts = line.split()
                return int(parts[1])
    except:
        pass
    
    # Fallback: find the oldest gcode process (likely the server)
    try:
        result = subprocess.run(
            ["pgrep", "-o", "gcode"], capture_output=True, text=True, timeout=5
        )
...
print_header(f"gcode Stress Test: {NUM_INSTANCES} sessions")
...
if not os.path.exists(MAIN_SOCK):
    ...
    print("  Start a server with: gcode serve &")
    sys.exit(1)

if not os.path.exists(DEBUG_SOCK):
    print_err(f"No debug socket at {DEBUG_SOCK}")
    print("  Enable with: touch ~/.gcode/debug_control")
    sys.exit(1)
...
baseline_procs = int(subprocess.run(["pgrep", "-c", "gcode"], capture_output=True, text=True).stdout.strip() or "0")
...
print_stat("Total gcode procs", baseline_procs)
...
for i in range(1, NUM_INSTANCES + 1):
    t0 = time.monotonic()
    resp = debug_cmd(f"create_session:/tmp/gcode-stress-{i}", timeout=30)
    t1 = time.monotonic()
...
peak_procs = int(subprocess.run(["pgrep", "-c", "gcode"], capture_output=True, text=True).stdout.strip() or "0")
...
print_stat("Total gcode procs", peak_procs)
...
final_procs = int(subprocess.run(["pgrep", "-c", "gcode"], capture_output=True, text=True).stdout.strip() or "0")
