...
A human-in-the-loop safety layer for unmonitored agent operations. Designed as an independent subsystem that any gcode feature can integrate with. Currently the only consumer is ambient mode, but the system is intentionally decoupled so it can be reused for future features.
...
sequenceDiagram
    ...
    NF->>US: Email: "gcode ambient wants to create a PR"
...
url = "https://example.com/gcode-safety"
secret = "..."
...
- Extracted 3 memories from crashed session gcode-red-fox-1234
...
```bash
gcode safety review           # Interactive review of pending requests
gcode safety list             # List all pending requests
gcode safety approve <id>     # Approve a specific request
gcode safety deny <id>        # Deny a specific request
gcode safety log              # View decision history
```
...
The safety system exposes a simple API for any gcode feature to use:
...
- [ ] TUI review panel
- [ ] CLI commands (`gcode safety review/list/approve/deny/log`)
