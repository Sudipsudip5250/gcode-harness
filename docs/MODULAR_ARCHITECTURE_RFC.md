...
This RFC describes a modular target architecture for gcode that matches the current codebase, preserves the existing product model, and gives us a safe migration path from today's mostly-monolithic root crate to a layered workspace.
...
Today, gcode is best described as a **modular monolith with a growing workspace shell**:

- The root `gcode` crate still owns most runtime orchestration and product behavior.
...
4. **Composition layer** where the top-level `gcode` package wires the product together.
...
- `gcode` is a **single-server, multi-client** application.
...
- **Root crate `gcode`** still contains most product logic.
...
- root package: `gcode`
- foundation/runtime support: `gcode-agent-runtime`, `gcode-core`, `gcode-storage`, `gcode-terminal-launch`, `gcode-tool-core`
- data-contract crates: `gcode-ambient-types`, `gcode-auth-types`, `gcode-background-types`, `gcode-batch-types`, `gcode-config-types`, `gcode-gateway-types`, `gcode-memory-types`, `gcode-message-types`, `gcode-selfdev-types`, `gcode-session-types`, `gcode-side-panel-types`, `gcode-task-types`, `gcode-tool-types`, `gcode-usage-types`
- protocol and planning: `gcode-protocol`, `gcode-plan`
- heavy or optional integrations: `gcode-embedding`, `gcode-pdf`, `gcode-notify-email`
- auth and providers: `gcode-azure-auth`, `gcode-provider-core`, `gcode-provider-metadata`, `gcode-provider-openrouter`, `gcode-provider-gemini`
- TUI extraction seams: `gcode-tui-core`, `gcode-tui-markdown`, `gcode-tui-mermaid`, `gcode-tui-render`, `gcode-tui-workspace`
- product surfaces outside the main TUI binary: `gcode-desktop`, `gcode-mobile-core`, `gcode-mobile-sim`
...
- provider implementation composition and runtime provider wiring; the shared `Provider` trait now lives in `gcode-provider-core`
...
| Crate | Current role |
|---|---|
| `gcode-agent-runtime` | shared interrupt and lightweight runtime primitives for agent execution |
| `gcode-ambient-types` | usage and rate-limit records shared by ambient/background flows |
| `gcode-auth-types` | provider-neutral auth state and credential metadata |
| `gcode-background-types` | background-task status and progress DTOs |
| `gcode-batch-types` | batch tool progress DTOs, currently depending only on message types internally |
| `gcode-config-types` | stable configuration data contracts |
| `gcode-core` | low-level utilities such as IDs, env helpers, fs helpers, stdin detection, and formatting |
| `gcode-gateway-types` | gateway-facing data contracts |
| `gcode-memory-types` | memory subsystem data contracts |
| `gcode-message-types` | message content and transport-adjacent data contracts |
| `gcode-protocol` | client/server protocol surface built from stable type crates and provider-core values |
| `gcode-plan` | plan/task graph data model shared across coordination flows |
| `gcode-selfdev-types` | self-development request/status data contracts |
| `gcode-session-types` | session DTOs, currently depending only on message types internally |
| `gcode-side-panel-types` | side-panel page and update data contracts |
| `gcode-task-types` | task/tool scheduling data contracts |
| `gcode-tool-core` | runtime tool contracts such as the `Tool` trait and execution context |
| `gcode-tool-types` | stable tool output/image DTOs |
| `gcode-usage-types` | usage accounting data contracts |
| `gcode-storage` | storage helpers layered on `gcode-core` |
| `gcode-embedding` | ONNX/tokenizer-based embedding implementation and heavy inference deps |
| `gcode-pdf` | PDF text extraction |
| `gcode-azure-auth` | Azure bearer token retrieval |
| `gcode-notify-email` | SMTP/IMAP/mail transport |
| `gcode-provider-metadata` | provider/login catalog and profile metadata |
| `gcode-provider-core` | shared provider contract (`Provider`/`EventStream`), value types, route/cost/model helpers, shared HTTP client, schema helpers |
| `gcode-provider-openrouter` | OpenRouter-specific catalog/cache/support helpers |
| `gcode-provider-gemini` | Gemini schema/model/support helpers |
| `gcode-tui-core` | low-level terminal UI primitives that do not need full app state |
| `gcode-tui-markdown` | markdown wrapping/rendering, layered on mermaid/workspace support |
| `gcode-tui-mermaid` | mermaid parsing, rendering, caching, viewport, and widget support |
| `gcode-tui-render` | reusable TUI layout/render helpers |
| `gcode-tui-workspace` | workspace-map data/model/widget rendering |
| `gcode-terminal-launch` | terminal process launch helpers |
| `gcode-mobile-core` | shared headless mobile simulator state/protocol/visual model |
| `gcode-mobile-sim` | mobile simulator CLI/app surface layered on `gcode-mobile-core` |
| `gcode-desktop` | desktop app surface and session/workspace rendering experiments |
...
flowchart TD
  ...
  J --> AR[gcode-agent-runtime]
  J --> Emb[gcode-embedding]
  J --> PDF[gcode-pdf]
  J --> Azure[gcode-azure-auth]
  J --> Mail[gcode-notify-email]
  J --> PMeta[gcode-provider-metadata]
  J --> PCore[gcode-provider-core]
  J --> POR[gcode-provider-openrouter]
  J --> PGem[gcode-provider-gemini]
  J --> TW[gcode-tui-workspace]
```
...
```mermaid
flowchart TD
  App[gcode top-level package]

  subgraph L2[Layer 2: interfaces and product surfaces]
    TUI[gcode-tui]
    SelfDev[gcode-selfdev]
    CLI[gcode-cli or root CLI modules]
  end

  subgraph L1[Layer 1: domain/runtime]
    Server[gcode-server]
    Agent[gcode-agent]
    Provider[gcode-provider]
    Session[gcode-session]
  end

  subgraph L0[Layer 0: foundation and support]
    Core[gcode-core]
    AR[gcode-agent-runtime]
    Emb[gcode-embedding]
    PDF[gcode-pdf]
    Azure[gcode-azure-auth]
    Mail[gcode-notify-email]
    PMeta[gcode-provider-metadata]
    PCore[gcode-provider-core]
    POR[gcode-provider-openrouter]
    PGem[gcode-provider-gemini]
    TW[gcode-tui-workspace]
  end
...
- `gcode-provider`: provider composition, provider routing, streaming contract adapters, and concrete runtime implementations layered on the `gcode-provider-core` trait.
- `gcode-agent`: turn loop, compaction orchestration, provider/tool interaction, recovery logic.
- `gcode-session`: session model, state transitions, persistence-facing session operations.
- `gcode-server`: daemon lifecycle, client attachment, swarm/background coordination, service registries.
- `gcode-tools` or narrower `gcode-tool-core` plus `gcode-tool-impl`: tool registry contracts and tool implementations.
- `gcode-auth`: root auth orchestration after provider-neutral data lives in `gcode-auth-types` and heavy leaf SDKs stay separate.
- `gcode-memory`: memory graph/log/search orchestration once its contracts are stable enough.
...
- `gcode-cli`: parsing and command dispatch if CLI keeps growing.
- `gcode-tui`: app state, reducers, key handling, command/input handling, UI orchestration.
- `gcode-desktop`: already a separate surface.
- `gcode-mobile-*`: already split.
- `gcode-selfdev`: self-dev build/reload/customization workflows if they remain a substantial product surface.
...
- `gcode-notify-email`
- `gcode-tui-mermaid`
- provider support crates such as `gcode-provider-openrouter` and `gcode-provider-gemini`
...
The top-level `gcode` package should eventually become mostly:
...
```text
gcode binary/composition
  -> gcode-cli, gcode-tui, gcode-server, gcode-selfdev

gcode-cli / gcode-tui
  -> gcode-protocol, gcode-*-types, gcode-server-client contracts

gcode-server
  -> gcode-agent, gcode-session, gcode-provider, gcode-tools, gcode-storage

gcode-agent
  -> gcode-provider, gcode-tools, gcode-session, gcode-agent-runtime

gcode-provider
  -> gcode-provider-core, gcode-provider-* leaves, gcode-auth-types

gcode-session
  -> gcode-session-types, gcode-message-types, gcode-storage, optional leaf adapters
...
- **One mega `gcode-common` crate.** It becomes the new root crate and invalidates everything.
...
1. **Provider contracts:** keep shrinking `src/provider/mod.rs` until a `gcode-provider` trait/runtime crate can depend only on `gcode-message-types`, `gcode-provider-core`, and small runtime primitives.
...
6. **Auth facade:** keep provider-neutral auth data in `gcode-auth-types`, heavy SDKs in leaf crates, and move root auth orchestration only after provider contracts stabilize.
...
- `cargo tree -p gcode --edges normal --depth 1` before/after for dependency surprises
...
- If introducing `gcode-session` feels too early, the same boundary should still be established internally first and extracted later.
...
- Existing crates `gcode-provider-core`, `gcode-provider-metadata`, `gcode-provider-openrouter`, and `gcode-provider-gemini` remain useful under this layer.
...
- `gcode-agent-runtime` remains the low-level runtime primitive crate below it.
...
- session persistence internals that belong in `gcode-session`
...
- `gcode-tui-workspace` can remain a leaf crate or become a child dependency of `gcode-tui`.
...
- socket/session attachment state, fanout senders, debug socket helpers, and daemon lifecycle code must not appear in `gcode-core`, `gcode-provider-core`, or provider leaf crates

### Rule 4: Provider implementation crates depend on contracts, not on the server or TUI

- provider leaf crates may depend on `gcode-core`, `gcode-provider`, and `gcode-provider-core`
- they must not depend on `gcode-server` or `gcode-tui`

### Rule 5: Async/network-heavy dependencies do not belong in `gcode-core`

`gcode-core` should stay cheap to compile and highly reusable.
...
- protocol/data types -> `gcode-core`
- session persistence -> `gcode-session`
- provider route/schema helpers -> provider crates
- rendering helpers -> `gcode-tui`

### Rule 8: The root package may compose many crates, but peer crates should stay narrow

The top-level `gcode` package can wire multiple domains together. Peer crates should not casually depend on each other sideways when a lower-level contract would do.
...
|---|---|
| `src/id.rs`, protocol/message/config primitives | `gcode-core` |
| `src/session.rs`, parts of `storage`, restart snapshot concerns | `gcode-session` |
| `src/agent/*`, parts of `compaction`, tool orchestration seams | `gcode-agent` |
| `src/server/` + shrinking `src/server.rs` facade | `gcode-server` |
| `src/provider/mod.rs` trait/contracts plus provider composition seams | `gcode-provider` |
| existing provider helper crates | remain leaf/provider support crates |
| `src/tui/*` + `gcode-tui-workspace` | `gcode-tui` + leaf workspace widget crate |
| `src/cli/*` | stay in root initially or become `gcode-cli` later if justified |
| `src/tool/selfdev/*`, self-dev workflow/productization | `gcode-selfdev` |
...
- Should `gcode-session` become an explicit crate, or remain an internal boundary until later?
- Should CLI remain in the top-level package permanently, or eventually become `gcode-cli`?
- Should `message` and `protocol` remain together in `gcode-core`, or split into separate contract crates if they evolve at different rates?
- Should `gcode-tui-workspace` remain a separate leaf crate long-term, or fold into `gcode-tui` once the larger TUI extraction lands?
