...
/// Default system prompt for gcode (embedded at compile time)
...
pub fn build_system_prompt_full(
    ...
    // Add optional prompt overlays from ~/.gcode/ and ./.gcode/
...
/// Load optional prompt overlay markdown from ~/.gcode/ and ./.gcode/
fn load_prompt_overlay_files_from_dir(working_dir: Option<&Path>) -> (Option<String>, usize) {
    ...
    let project_dir = working_dir.unwrap_or(Path::new("."));
    if let Some((content, size)) = load_file(
        &project_dir.join(".gcode").join("prompt-overlay.md"),
        "Project Prompt Overlay (.gcode/prompt-overlay.md)",
    ) {
        total_chars += size;
        contents.push(content);
    }

    if let Ok(global_overlay) = crate::storage::gcode_dir().map(|dir| dir.join("prompt-overlay.md"))
        && let Some((content, size)) = load_file(
            &global_overlay,
            "Global Prompt Overlay (~/.gcode/prompt-overlay.md)",
        )
    {
        total_chars += size;
        contents.push(content);
