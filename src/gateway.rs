...
//! This lets iOS/web clients interact with gcode sessions identically
//! to TUI clients.
//!
//! Architecture:
...
pub use gcode_gateway_types::{PairedDevice, PairingCode};
...
async fn handle_pair_request(
    ...
    // Reload from disk - pairing codes are generated by `gcode pair` CLI
    *reg = DeviceRegistry::load();
    ...
    let body = serde_json::json!({
        "token": token,
        "server_name": "gcode",
        "server_version": env!("GCODE_VERSION"),
    });
    http_response(200, "OK", &body.to_string())
}
