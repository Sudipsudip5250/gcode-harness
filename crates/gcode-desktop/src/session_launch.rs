...
pub fn launch_resume_session(session_id: &str, title: &str) -> Result<()> {
    let title = format!("gcode · {}", compact_title(title));
    let candidates = terminal_candidates(&title, &["--resume", session_id]);
    launch_first_available_terminal(candidates, &format!("gcode --resume {session_id}"))
}

pub fn launch_new_session() -> Result<()> {
    let candidates = terminal_candidates("gcode · new session", &["--fresh-spawn"]);
    launch_first_available_terminal(candidates, "gcode")
}
...
pub fn send_message_to_session(session_id: &str, _title: &str, message: &str) -> Result<()> {
    ...
    Command::new(gcode_bin())
        ...
        .stderr(Stdio::null())
        .spawn()
        .with_context(|| format!("failed to spawn gcode run for {session_id}"))?;
...
pub fn spawn_fresh_server_session(
    ...
    std::thread::Builder::new()
        .name("gcode-desktop-fresh-session".to_string())
        .spawn(move || {
            if let Err(error) =
...
pub fn spawn_message_to_session(
    ...
    let handle = DesktopSessionHandle { command_tx };
    std::thread::Builder::new()
        .name("gcode-desktop-session-message".to_string())
        .spawn(move || {
...
pub fn spawn_cycle_model(
    ...
    std::thread::Builder::new()
        .name("gcode-desktop-cycle-model".to_string())
        .spawn(move || {
...
pub fn spawn_load_model_catalog(
    ...
    std::thread::Builder::new()
        .name("gcode-desktop-load-model-catalog".to_string())
        .spawn(move || {
            if let Err(error) =
...
pub fn spawn_set_model(
    ...
    std::thread::Builder::new()
        .name("gcode-desktop-set-model".to_string())
        .spawn(move || {
            if let Err(error) =
...
fn ensure_server_running() -> Result<()> {
    ...
    Command::new(gcode_bin())
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to spawn gcode serve")?;
...
fn connect_server_with_retry_path(socket_path: &PathBuf, timeout: Duration) -> Result<UnixStream> {
    ...
    match last_error {
        Some(error) => Err(error).with_context(|| {
            format!(
                "timed out connecting to gcode server at {}",
                socket_path.display()
            )
        }),
        None => anyhow::bail!("timed out connecting to gcode server"),
    }
}
...
fn read_session_id_from_events(
    ...
    while started.elapsed() < timeout {
        ...
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("gcode server disconnected before assigning a session"),
            Ok(_) => {
                let value: Value = serde_json::from_str(line.trim())
                    .context("failed to parse gcode server event")?;
                if value.get("type").and_then(Value::as_str) == Some("session") {
                    let Some(session_id) = value.get("session_id").and_then(Value::as_str) else {
                        anyhow::bail!("gcode server sent malformed session event");
                    };
                ...
                if value.get("type").and_then(Value::as_str) == Some("error") {
                    ...
                    anyhow::bail!("gcode server rejected fresh session: {message}");
                }
            ...
            Err(error) => return Err(error).context("failed to read gcode server event"),
        }
    }

    anyhow::bail!("timed out waiting for gcode server session id")
}
...
fn read_session_id_from_state(
    ...
    while started.elapsed() < timeout {
        line.clear();
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("gcode server disconnected before returning state"),
            Ok(_) => {
                let value: Value = serde_json::from_str(line.trim())
                    .context("failed to parse gcode server event")?;
                ...
                {
                    let Some(session_id) = value.get("session_id").and_then(Value::as_str) else {
                        anyhow::bail!("gcode server sent malformed state event");
                    };
                    ...
                    && value.get("id").and_then(Value::as_u64) == Some(state_request_id)
                {
                    let message = value
                        ...
                        .and_then(Value::as_str)
                        .unwrap_or("unknown server error");
                    anyhow::bail!("gcode server rejected state request: {message}");
                }
            }
            Err(error)
                if matches!(
    ...
    anyhow::bail!("timed out waiting for gcode server state")
}
...
fn read_model_changed(
    ...
    while started.elapsed() < timeout {
        line.clear();
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("gcode server disconnected before switching model"),
            Ok(_) => {
                let value: Value = serde_json::from_str(line.trim())
                    .context("failed to parse gcode server event")?;
                    ...
                    && value.get("id").and_then(Value::as_u64) == Some(request_id)
                {
                    let message = value
                        ...
                        .and_then(Value::as_str)
                        .unwrap_or("unknown server error");
                    anyhow::bail!("gcode server rejected model switch: {message}");
                }
            }
            Err(error)
                if matches!(
    ...
    anyhow::bail!("timed out waiting for gcode server model switch")
}
...
fn read_model_catalog(
    ...
    while started.elapsed() < timeout {
        line.clear();
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("gcode server disconnected before loading model catalog"),
            Ok(_) => {
                let value: Value = serde_json::from_str(line.trim())
                    .context("failed to parse gcode server event")?;
                    ...
                    && value.get("id").and_then(Value::as_u64) == Some(request_id)
                {
                    ...
                    anyhow::bail!("gcode server returned malformed model catalog");
                }
                if let Some(event) = desktop_event_from_server_value(&value) {
                    if !matches!(event, DesktopSessionEvent::Done) {
                        send_desktop_event_ref(event_tx, event);
                    ...
                    && value.get("id").and_then(Value::as_u64) == Some(request_id)
                {
                    let message = value
                        ...
                        .and_then(Value::as_str)
                        .unwrap_or("unknown server error");
                    anyhow::bail!("gcode server rejected model catalog request: {message}");
                }
            }
            Err(error)
                if matches!(
    ...
    anyhow::bail!("timed out waiting for gcode server model catalog")
}
...
fn socket_path() -> PathBuf {
    if let Ok(custom) = std::env::var("GCODE_SOCKET") {
        return PathBuf::from(custom);
    }
    if let Ok(dir) = std::env::var("GCODE_RUNTIME_DIR") {
        return PathBuf::from(dir).join("gcode.sock");
    }
    if let Ok(dir) = std::env::var("XDG_RUNTIME_DIR") {
        return PathBuf::from(dir).join("gcode.sock");
    }
    std::env::temp_dir()
        .join(format!("gcode-{}", runtime_user_discriminator()))
        .join("gcode.sock")
}
...
fn terminal_candidates(title: &str, gcode_args: &[&str]) -> Vec<Command> {
    let mut candidates = Vec::new();

    if let Ok(program) = std::env::var("GCODE_DESKTOP_TERMINAL") {
        candidates.push(terminal_command(program, &[], gcode_args));
    }
    ...
    candidates.push(terminal_command("foot", &["-T", title, "--"], gcode_args));
    candidates.push(terminal_command("kitty", &["--title", title], gcode_args));
    candidates.push(terminal_command(
        "alacritty",
        &["-t", title, "-e"],
        gcode_args,
    ));
    candidates.push(terminal_command("wezterm", &["start", "--"], gcode_args));
    candidates.push(terminal_command(
        "x-terminal-emulator",
        &["-T", title, "-e"],
        gcode_args,
...
fn gcode_bin() -> String {
    std::env::var("GCODE_BIN").unwrap_or_else(|_| "gcode".to_string())
}
...
mod tests {
    ...
    fn desktop_event_parser_maps_streaming_server_events() {
        ...
        assert_eq!(
            desktop_event_from_server_value(&json!({
                "type": "reloading",
                "new_socket": "/tmp/gcode-new.sock"
            })),
            Some(DesktopSessionEvent::Reloading {
                new_socket: Some("/tmp/gcode-new.sock".to_string())
            })
        );
        assert_eq!(
            desktop_event_from_server_value(&json!({
    ...
    fn desktop_worker_roundtrips_message_with_fake_server() -> Result<()> {
        ...
        let socket_path = std::env::temp_dir().join(format!(
            "gcode-desktop-worker-smoke-{}-{}.sock",
            std::process::id(),
        ...
        let previous_socket = std::env::var_os("GCODE_SOCKET");
        unsafe {
            std::env::set_var("GCODE_SOCKET", &socket_path);
        }
        ...
        restore_env_var("GCODE_SOCKET", previous_socket);
        let _ = std::fs::remove_file(&socket_path);
