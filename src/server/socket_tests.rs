...
fn sibling_socket_path_roundtrip() {
    let main = std::path::PathBuf::from("/tmp/gcode.sock");
    let debug = std::path::PathBuf::from("/tmp/gcode-debug.sock");
...
fn cleanup_socket_pair_removes_main_and_debug_files() {
    ...
    let main = dir.join(format!("gcode-test-{}.sock", stamp));
    let debug = dir.join(format!("gcode-test-{}-debug.sock", stamp));
...
fn daemon_lock_serializes_server_processes() {
    ...
    let temp = tempfile::tempdir().expect("tempdir");
    let prev_runtime = std::env::var_os("GCODE_RUNTIME_DIR");
    crate::env::set_var("GCODE_RUNTIME_DIR", temp.path());
    ...
    if let Some(prev_runtime) = prev_runtime {
        crate::env::set_var("GCODE_RUNTIME_DIR", prev_runtime);
    } else {
...
fn existing_server_start_errors_are_detected() {
    assert!(server_start_matches_existing_server(
        "Error: Another gcode server process is already running for runtime dir /run/user/1000"
    ));
    assert!(server_start_matches_existing_server(
        "Error: Refusing to replace active server socket at /run/user/1000/gcode.sock"
    ));
    assert!(!server_start_matches_existing_server(
