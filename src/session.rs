...
fn default_is_test_session() -> bool {
    env_flag_enabled("GCODE_TEST_SESSION")
}
...
pub fn derive_session_provider_key(provider_name: &str) -> Option<String> {
    ...
    if normalized_name == "gcode" {
        return Some("gcode".to_string());
    }
    ...
    if let Ok(active) = std::env::var("GCODE_ACTIVE_PROVIDER") {
...
impl Session {
    ...
    /// Check if this session is working on the gcode repository
    pub fn is_self_dev(&self) -> bool {
        if let Some(ref dir) = self.working_dir {
            // Check if working dir contains gcode source
            let path = std::path::Path::new(dir);
            path.join("Cargo.toml").exists()
                && path.join("src/main.rs").exists()
                && std::fs::read_to_string(path.join("Cargo.toml"))
                    .map(|s| s.contains("name = \"gcode\""))
                    .unwrap_or(false)
        } else {
            false
        }
