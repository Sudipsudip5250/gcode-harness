...
fn test_generated_default_config_uses_low_openai_reasoning_effort() {
    ...
    if let Some(prev) = prev_home {
        crate::env::set_var("GCODE_HOME", prev);
    } else {
...
fn test_env_override_auto_server_reload() {
    let _guard = crate::storage::lock_test_env();
    let prev = std::env::var_os("GCODE_AUTO_SERVER_RELOAD");
    crate::env::set_var("GCODE_AUTO_SERVER_RELOAD", "false");
    ...
    if let Some(prev) = prev {
        crate::env::set_var("GCODE_AUTO_SERVER_RELOAD", prev);
    } else {
        crate::env::remove_var("GCODE_AUTO_SERVER_RELOAD");
    }
}
...
fn test_env_override_native_scrollbars() {
    let _guard = crate::storage::lock_test_env();
    let prev_chat = std::env::var_os("GCODE_CHAT_NATIVE_SCROLLBAR");
    let prev_side = std::env::var_os("GCODE_SIDE_PANEL_NATIVE_SCROLLBAR");
    crate::env::set_var("GCODE_CHAT_NATIVE_SCROLLBAR", "true");
    crate::env::set_var("GCODE_SIDE_PANEL_NATIVE_SCROLLBAR", "false");
    ...
    if let Some(prev) = prev_chat {
        crate::env::set_var("GCODE_CHAT_NATIVE_SCROLLBAR", prev);
    } else {
        crate::env::remove_var("GCODE_CHAT_NATIVE_SCROLLBAR");
    }
    if let Some(prev) = prev_side {
        crate::env::set_var("GCODE_SIDE_PANEL_NATIVE_SCROLLBAR", prev);
    } else {
        crate::env::remove_var("GCODE_SIDE_PANEL_NATIVE_SCROLLBAR");
    }
}
...
fn test_env_override_diff_mode_full_inline() {
    let _guard = crate::storage::lock_test_env();
    let prev = std::env::var_os("GCODE_DIFF_MODE");
    crate::env::set_var("GCODE_DIFF_MODE", "full-inline");
    ...
    if let Some(prev) = prev {
        crate::env::set_var("GCODE_DIFF_MODE", prev);
    } else {
        crate::env::remove_var("GCODE_DIFF_MODE");
    }
}
...
fn test_env_override_trusted_external_auth_splits_source_and_path_entries() {
    let _guard = crate::storage::lock_test_env();
    let prev = std::env::var_os("GCODE_TRUSTED_EXTERNAL_AUTH_SOURCES");
    crate::env::set_var(
        "GCODE_TRUSTED_EXTERNAL_AUTH_SOURCES",
    ...
    if let Some(prev) = prev {
        crate::env::set_var("GCODE_TRUSTED_EXTERNAL_AUTH_SOURCES", prev);
    } else {
        crate::env::remove_var("GCODE_TRUSTED_EXTERNAL_AUTH_SOURCES");
    }
}
