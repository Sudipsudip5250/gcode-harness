...
/// Mutate the process environment for gcode runtime configuration.
///
/// Rust 2024 makes environment mutation unsafe because it can race with
/// concurrent environment access in foreign code. gcode intentionally mutates
...
where
    ...
    V: AsRef<OsStr>,
{
    // SAFETY: gcode treats these mutations as process-global configuration.
...
/// Remove a process environment variable used by gcode runtime configuration.
