...
This document defines the intended direction for the gcode mobile simulator.

## Product definition

The simulator is a **Linux-native simulator for the gcode mobile application itself**.
...
- **App simulator**: the Linux-native, agent-controllable simulator for the gcode mobile app.
...
graph TB
    subgraph Core["Rust mobile app core"]
        ...
        Protocol["gcode protocol adapters"]
    ...
    subgraph Agents["AI agents and tests"]
        CLI["sim CLI"]
        Debug["gcode debug/tester integration"]
        CI["Linux CI"]
    end
...
Lock the simulator definition as a Linux-native app simulator for gcode mobile, with Rust-first app implementation and AI-agent-first automation.
...
Audit `ios/Sources/GcodeMobile` and `ios/Sources/GcodeKit` to extract concepts that must move into Rust.

### M4: Real mobile core

Expand `crates/gcode-mobile-core` from a small mock simulator into the actual shared mobile state machine.
...
### M8: Fake gcode backend

Implement a simulated gcode server backend for health, pairing, token auth, WebSocket lifecycle, sessions, streaming deltas, text replacement, tool calls, errors, and reconnects.
...
### M14: gcode debug/tester integration

Expose simulator lifecycle through gcode tooling so agents can spawn, drive, inspect, capture, and clean up simulator instances.
...
- `crates/gcode-mobile-core`: basic state, typed actions, reducer/store, semantic UI tree, transition/effect log, baseline scenarios
- `crates/gcode-mobile-sim`: headless daemon, Unix socket automation protocol, CLI for state/tree/dispatch/tap/log/reset
