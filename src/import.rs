...
/// Convert Claude Code content blocks to gcode ContentBlocks
...
pub fn resolve_resume_target_to_gcode(
    target: &crate::tui::session_picker::ResumeTarget,
...
pub fn import_session_from_file(path: &Path, session_id: &str) -> Result<Session> {
    ...
    // Create gcode session
    let gcode_session_id = imported_claude_code_session_id(session_id);
    let mut session = Session::create_with_id(gcode_session_id, None, title);
