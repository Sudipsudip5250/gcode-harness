...
fn with_temp_home<T>(f: impl FnOnce(&Path) -> T) -> T {
    ...
    let previous_home = std::env::var("GCODE_HOME").ok();
    ...
    if let Some(previous_home) = previous_home {
        crate::env::set_var("GCODE_HOME", previous_home);
    } else {
...
#[test]
fn external_codex_sessions_are_searchable_without_gcode_session_dir() {
    with_temp_home(|home| {
        ...
        std::fs::remove_dir_all(home.join("sessions")).expect("remove gcode sessions dir");
        ...
        assert_eq!(report.scanned_gcode_sessions, 0);
