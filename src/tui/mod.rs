...
pub use gcode_tui_workspace::workspace_map;
pub use gcode_tui_workspace::workspace_map_widget;
...
pub(crate) fn subscribe_metadata() -> (Option<String>, Option<bool>) {
    ...
    if !selfdev && let Some(ref dir) = working_dir {
        ...
        while let Some(path) = current {
            if crate::build::is_gcode_repo(path) {
                selfdev = true;
                break;
            }
            current = path.parent();
