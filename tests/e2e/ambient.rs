...
fn test_ambient_state_lifecycle() {
    use gcode::ambient::{AmbientCycleResult, AmbientState, AmbientStatus, CycleStatus};
...
fn test_ambient_scheduled_queue() {
    use gcode::ambient::{Priority, ScheduledItem, ScheduledQueue};

    let tmp = std::env::temp_dir().join("gcode-test-queue.json");
    ...
    queue.push(ScheduledItem {
        ...
        target: gcode::ambient::ScheduleTarget::Ambient,
...
fn test_adaptive_scheduler_intervals() {
    use gcode::ambient_scheduler::{AdaptiveScheduler, AmbientSchedulerConfig};
...
async fn test_ambient_end_cycle_tool() -> Result<()> {
    ...
    let result = gcode::tool::ambient::take_cycle_result();
...
async fn test_ambient_request_permission_tool() -> Result<()> {
    ...
    gcode::tool::ambient::register_ambient_session(ambient_session_id.clone());

    let response = agent.run_once_capture("Request permission").await?;
    gcode::tool::ambient::unregister_ambient_session(&ambient_session_id);
...
async fn test_ambient_runner_status() {
    use gcode::ambient_runner::AmbientRunnerHandle;
...
async fn test_ambient_runner_trigger_and_stop() {
    use gcode::ambient::AmbientStatus;
    use gcode::ambient_runner::AmbientRunnerHandle;
    use gcode::safety::SafetySystem;
...
fn test_memory_reinforcement_provenance() {
    use gcode::memory::{MemoryCategory, MemoryEntry};
...
fn test_ambient_config_defaults() {
    use gcode::config::AmbientConfig;
...
fn test_ambient_lock() {
    use gcode::ambient::AmbientLock;
    let _env = setup_test_env().expect("failed to setup isolated GCODE_HOME");
...
async fn test_full_ambient_cycle_simulation() -> Result<()> {
    ...
    agent.set_system_prompt("You are the gcode ambient maintenance agent.");
