#!/bin/bash
# gcode demo recording orchestrator
# Usage: ./scripts/record_demo.sh <demo_name> <prompt>
#
# This script:
# 1. Opens a fresh gcode in a new kitty window
# 2. Starts wf-recorder on that window
# 3. Sends the prompt to gcode
...
DEMO_DIR="/tmp/gcode-demo/$DEMO_NAME"
OUTPUT_DIR="$repo_root/assets/demos"
SOCK=$(ls /tmp/kitty.sock* 2>/dev/null | head -1)

mkdir -p "$DEMO_DIR" "$OUTPUT_DIR"

echo "=== gcode Demo Recorder ==="
echo "Demo: $DEMO_NAME"
echo "Prompt: $PROMPT"
echo "Working dir: $DEMO_DIR"
echo ""

# Step 1: Launch gcode in a new kitty OS window
echo "[1/5] Launching gcode..."
kitten @ --to unix:$SOCK launch --type=os-window \
    --cwd "$DEMO_DIR" \
    --title "gcode-demo-$DEMO_NAME" \
    "$repo_root/target/release/gcode"

sleep 3  # Let gcode fully start

# Step 2: Find the window
DEMO_WIN_ID=$(niri msg windows 2>/dev/null | grep -B5 "gcode-demo-$DEMO_NAME" | grep "Window ID" | awk '{print $3}' | tr -d ':')
...
# Step 4: Type the prompt into gcode
...
for os_win in data:
    for tab in os_win.get('tabs', []):
        for win in tab.get('windows', []):
            if 'gcode-demo-$DEMO_NAME' in win.get('title', ''):
                print(win['id'])
                sys.exit(0)
")
...
if [ -n "$KITTY_WIN_ID" ]; then
    ...
    kitten @ --to unix:$SOCK send-text --match "title:gcode-demo-$DEMO_NAME" "$PROMPT"
    sleep 0.5
    kitten @ --to unix:$SOCK send-text --match "title:gcode-demo-$DEMO_NAME" $'\r'
fi
...
# Poll for completion - check if gcode is still processing
