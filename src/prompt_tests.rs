...
#[test]
fn test_prompt_overlay_files_are_loaded_from_project_and_global_gcode_dirs() {
    let _guard = crate::storage::lock_test_env();
    let prev_home = std::env::var_os("GCODE_HOME");
    let temp = tempfile::TempDir::new().unwrap();
    crate::env::set_var("GCODE_HOME", temp.path());
    ...
    let project_dir = tempfile::TempDir::new().unwrap();
    std::fs::create_dir_all(project_dir.path().join(".gcode")).unwrap();
    std::fs::write(
        project_dir.path().join(".gcode/prompt-overlay.md"),
...
fn test_selfdev_prompt_uses_full_selfdev_instructions() {
    ...
    assert!(prompt.contains("You are working on the gcode codebase itself."));
    assert!(!prompt.contains("Self-Development Access"));
}
...
fn test_selfdev_prompt_prefers_publish_flow_for_active_builds() {
    ...
    assert!(prompt.contains("scripts/dev_cargo.sh build --profile selfdev -p gcode --bin gcode"));
