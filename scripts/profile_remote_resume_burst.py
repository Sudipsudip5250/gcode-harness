...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Profile resumed gcode PTY burst startup")
    p.add_argument("--binary", default="./target/release/gcode")
    p.add_argument("--burst", type=int, default=20)
    p.add_argument("--timeout", type=float, default=15.0)
    p.add_argument("--stagger-ms", type=float, default=0.0)
    p.add_argument("--json-out", default="/tmp/gcode_remote_burst_profile.json")
    return p.parse_args()
...
def run_burst(
    ...
    while time.perf_counter() < deadline and (
        ...
        for fd in rlist:
            ...
            if b"loading session" in lower or b"gcode" in lower or len(client.buffer) > 2048:
                client.done = True
...
def main() -> None:
    args = parse_args()
    root = Path(tempfile.mkdtemp(prefix="gcode-remote-burst-"))
    home = root / "home"
    run = root / "run"
    home.mkdir(parents=True, exist_ok=True)
    run.mkdir(parents=True, exist_ok=True)
    env = os.environ.copy()
    env["GCODE_HOME"] = str(home)
    env["GCODE_RUNTIME_DIR"] = str(run)
    env["GCODE_SOCKET"] = str(run / "gcode.sock")
    env["GCODE_NO_TELEMETRY"] = "1"
    env["GCODE_DEBUG_CONTROL"] = "1"
    env["GCODE_TEMP_SERVER"] = "1"
    env["GCODE_SERVER_OWNER_PID"] = str(os.getpid())
    debug_socket = run / "gcode-debug.sock"

    server = subprocess.Popen(
        [args.binary, "serve", "--socket", env["GCODE_SOCKET"], "--debug-socket"],
        env=env,
    ...
    try:
        wait_for_socket(Path(env["GCODE_SOCKET"]))
