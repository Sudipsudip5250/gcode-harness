...
impl CursorCliProvider {
    ...
    pub fn new() -> Self {
        let model = std::env::var("GCODE_CURSOR_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.into());
        let provider = Self {
...
async fn run_native_text_command_via_curl(
    ...
    let body_path = std::env::temp_dir().join(format!("gcode-cursor-{}.bin", Uuid::new_v4()));
...
fn encode_metadata() -> Vec<u8> {
    ...
    bytes.extend(encode_field(4, 2, b"gcode".to_vec()));
    bytes.extend(encode_field(
        5,
        2,
