...
impl MultiProvider {
    ...
    pub(super) fn new_with_auth_status(auth_status: auth::AuthStatus) -> Self {
            ...
            && let Some(pref) = cfg.provider.default_provider.as_deref()
        {
                ...
                crate::provider_catalog::resolve_openai_compatible_profile_selection(pref)
            {
                ...
                match crate::provider_catalog::apply_named_provider_profile_env_from_config(
                    ...
                    Ok(profile_name) => {
                        crate::env::set_var("GCODE_PROVIDER_PROFILE_NAME", &profile_name);
        ...
        let use_claude_cli = std::env::var("GCODE_USE_CLAUDE_CLI")
            .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
            .unwrap_or(false);
        if use_claude_cli {
            crate::logging::warn(
                "GCODE_USE_CLAUDE_CLI is deprecated and will be removed. Direct Anthropic API transport is the default.",
            );
        }

        let claude = if has_claude_creds && use_claude_cli {
            crate::logging::info(
                "Using deprecated Claude CLI provider (forced by GCODE_USE_CLAUDE_CLI=1)",
            );
        ...
        let openrouter = if has_openrouter_creds {
            let named_profile = std::env::var("GCODE_NAMED_PROVIDER_PROFILE")
                .ok()
        ...
        let copilot_premium_zero = matches!(
            std::env::var("GCODE_COPILOT_PREMIUM").ok().as_deref(),
            Some("0")
        );
