...
use chrono::Utc;
use gcode_agent_runtime::{SoftInterruptMessage, SoftInterruptQueue, SoftInterruptSource};
use std::sync::Arc;
...
impl AmbientRunnerHandle {
    ...
    pub async fn log_json(&self) -> String {
        let transcripts_dir = match crate::storage::gcode_dir() {
    ...
    async fn run_cycle_visible(
        ...
        let gcode_bin =
            std::env::current_exe().unwrap_or_else(|_| std::path::PathBuf::from("gcode"));

        // Spawn kitty with `gcode ambient run-visible`
        logging::info("Ambient visible: spawning kitty with gcode TUI");
        let child = std::process::Command::new("kitty")
            .args([
                "--title",
                "🤖 gcode ambient cycle",
                "-e",
                &gcode_bin.to_string_lossy(),
                "ambient",
                "run-visible",
            ])
