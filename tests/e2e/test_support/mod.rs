//! End-to-end tests for gcode using a mock provider
//!
...
pub(crate) use gcode::agent::Agent;
pub(crate) use gcode::message::{ContentBlock, Message, Role, StreamEvent, ToolDefinition};
pub(crate) use gcode::protocol::{Request, ServerEvent};
pub(crate) use gcode::provider::{EventStream, Provider};
pub(crate) use gcode::server;
pub(crate) use gcode::session::{Session, StoredCompactionState};
pub(crate) use gcode::tool::Registry;
...
static GCODE_HOME_LOCK: std::sync::OnceLock<Mutex<()>> = std::sync::OnceLock::new();
...
fn lock_gcode_home() -> std::sync::MutexGuard<'static, ()> {
    let mutex = GCODE_HOME_LOCK.get_or_init(|| Mutex::new(()));
...
impl TestEnvGuard {
    pub(crate) fn new() -> Result<Self> {
        let lock = lock_gcode_home();
        let temp_home = tempfile::Builder::new()
            .prefix("gcode-e2e-home-")
            .tempdir()?;
        let prev_home = std::env::var_os("GCODE_HOME");
        let prev_runtime_dir = std::env::var_os("GCODE_RUNTIME_DIR");
        let prev_test_session = std::env::var_os("GCODE_TEST_SESSION");
        let prev_debug_control = std::env::var_os("GCODE_DEBUG_CONTROL");
        let runtime_dir = temp_home.path().join("runtime");
        std::fs::create_dir_all(&runtime_dir)?;

        gcode::env::set_var("GCODE_HOME", temp_home.path());
        gcode::env::set_var("GCODE_RUNTIME_DIR", &runtime_dir);
        gcode::env::set_var("GCODE_TEST_SESSION", "1");
        gcode::env::set_var("GCODE_DEBUG_CONTROL", "1");
...
impl Drop for TestEnvGuard {
    fn drop(&mut self) {
        if let Some(prev_home) = &self.prev_home {
            gcode::env::set_var("GCODE_HOME", prev_home);
        } else {
            gcode::env::remove_var("GCODE_HOME");
        }

        if let Some(prev_runtime_dir) = &self.prev_runtime_dir {
            gcode::env::set_var("GCODE_RUNTIME_DIR", prev_runtime_dir);
        } else {
            gcode::env::remove_var("GCODE_RUNTIME_DIR");
        }

        if let Some(prev_test_session) = &self.prev_test_session {
            gcode::env::set_var("GCODE_TEST_SESSION", prev_test_session);
        } else {
            gcode::env::remove_var("GCODE_TEST_SESSION");
        }

        if let Some(prev_debug_control) = &self.prev_debug_control {
            gcode::env::set_var("GCODE_DEBUG_CONTROL", prev_debug_control);
        } else {
            gcode::env::remove_var("GCODE_DEBUG_CONTROL");
        }
    }
}
...
impl Drop for EnvVarGuard {
    fn drop(&mut self) {
        if let Some(prev) = &self.prev {
            gcode::env::set_var(self.name, prev);
        } else {
            gcode::env::remove_var(self.name);
        }
    }
}
...
fn pair_test_device(token: &str) -> Result<()> {
    let mut registry = gcode::gateway::DeviceRegistry::load();
    ...
    registry.devices.push(gcode::gateway::PairedDevice {
...
pub(crate) async fn run_unix_transport_scenario() -> Result<TransportScenarioResult> {
    ...
    std::fs::create_dir_all(&runtime_dir)?;
    let socket_path = runtime_dir.join("gcode.sock");
    let debug_socket_path = runtime_dir.join("gcode-debug.sock");
    ...
    let provider: Arc<dyn gcode::provider::Provider> = Arc::new(provider);
    let server_instance =
    ...
    let result = async {
        ...
        if !saw_message_done {
            ...
            if !response_persisted {
                ...
                let logs = std::env::var_os("GCODE_HOME")
...
pub(crate) async fn run_websocket_transport_scenario() -> Result<TransportScenarioResult> {
    ...
    let server_instance =
        server::Server::new_with_paths(provider, socket_path.clone(), debug_socket_path.clone())
            .with_gateway_config(gcode::gateway::GatewayConfig {
                port: gateway_port,
...
pub(crate) async fn wait_for_selfdev_reload_cycle(
    ...
    while Instant::now() < deadline {
        let marker_active = gcode::server::reload_marker_active(Duration::from_secs(30));
