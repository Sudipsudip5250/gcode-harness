...
#[cfg(test)]
pub(crate) const OAUTH_BETA_HEADERS_1M: &str = gcode_provider_core::ANTHROPIC_OAUTH_BETA_HEADERS_1M;
...
/// Override with GCODE_ANTHROPIC_MAX_TOKENS env var.
...
impl AnthropicProvider {
    ...
    pub fn new() -> Self {
        let model = std::env::var("GCODE_ANTHROPIC_MODEL").unwrap_or_else(|_| {
        ...
        let max_tokens = std::env::var("GCODE_ANTHROPIC_MAX_TOKENS")
            .ok()
    ...
    async fn get_access_token(&self) -> Result<(String, bool)> {
        ...
        {
            anyhow::bail!(
                "Claude OAuth credentials are missing the required user:inference scope (scopes: {}). Run `gcode login --provider claude` to mint a fresh Claude.ai OAuth token, or import/use a fresh Claude Code login.",
        ...
        .await
        {
            ...
            Err(e) => {
                        ...
                        Err(refresh_err) => {
                            let _ = tx
                                .send(Err(anyhow::anyhow!(
                                    "{}\n\nAutomatic Claude OAuth refresh failed: {}\nRun `gcode login --provider claude` (preferred) or `claude`, then retry.",
                                    e,
                                    refresh_err
                                )))
                                .await;
                ...
                if is_oauth && is_oauth_auth_error(&error_str) {
                    let _ = tx
                        .send(Err(anyhow::anyhow!(
                            "{}\n\nClaude OAuth authentication failed. Run `gcode login --provider claude` (preferred) or `claude`, then retry.",
                            e
                        )))
                        .await;
                } else {
...
async fn stream_response(
    ...
    if std::env::var("GCODE_ANTHROPIC_DEBUG")
        .map(|v| v == "1")
        .unwrap_or(false)
