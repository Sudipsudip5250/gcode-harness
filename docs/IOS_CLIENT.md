...
A native iOS application that connects to a gcode server running on the user's laptop or desktop. The phone is a rich, touch-optimized client; all heavy lifting (LLM calls, tool execution, file I/O, git, MCP) stays on the server.
...
graph TB
    ...
    subgraph Laptop ["💻 Laptop / Desktop"]
        ...
        subgraph Srv ["gcode Server (Rust)"]
            AG[🤖 Agent Engine]
            LLM["☁️ LLM Providers\n(Claude / OpenRouter)"]
            TOOLS["🔧 Tools\n(bash, files, git)"]
            MEM[🧠 Memory Graph]
...
gcode's value is **tool execution**: running shell commands, editing files, managing git repos, connecting to MCP servers. None of that is possible inside iOS's sandbox. So the server must exist regardless.

What the phone adds:
- **Mobility** - interact with gcode from the couch, on the bus, in a meeting
...
The gcode server currently speaks newline-delimited JSON over Unix sockets. The iOS client needs the same protocol over a network transport. Changes required:
...
The iOS app connects to the gcode server over **Tailscale** as the primary transport. No LAN-only discovery, no mDNS fragility, no port forwarding.
...
(Tailscale app)            (WireGuard mesh)               (tailscaled)
     │                            │                           │
     │  gcode iOS app connects to laptop.tail1234.ts.net:7643 │
...
2. gcode server binds to Tailscale IP (or `0.0.0.0` and Tailscale handles routing)
...
**No cloud relay needed** - Tailscale is peer-to-peer. Traffic goes directly between phone and laptop, even across networks. No gcode server in the cloud.
...
```
gcode server                     Apple APNs              iPhone
(your laptop)                    (Apple cloud)           (gcode app)

Event fires ───► HTTP/2 POST ──► Routes push ──► 🔔 Native push
                 to APNs with    to device        notification
                 device token                     in gcode app
                 + JWT signing
```

**How it works:**
- Apple Developer Account provides an APNs key (.p8 file)
- The .p8 key is stored on the gcode server (`~/.gcode/apns/`)
- iOS app registers for push on launch, gets a device token from Apple
- Device token is sent to gcode server during pairing (stored in `devices.json`)
- To send a push: gcode server signs a JWT with the .p8 key, POSTs to `api.push.apple.com`
...
The killer feature for iOS. Shows what gcode is doing autonomously.
...
1. Add WebSocket listener to gcode server (`src/gateway.rs`)
...
2. Add token-based authentication
   - Pairing command: `gcode pair`
   - Device registry: `~/.gcode/devices.json`
...
**Deliverable:** Any WebSocket client can connect to gcode server over Tailscale, authenticate, and interact with sessions. Testable from Linux with CLI tools.
...
**Deliverable:** Working iOS app that can chat with gcode.
...
- **Device revocation** via `gcode pair --revoke <name-or-id>`
...
For the current implementation, the iOS side in this repo is `GcodeKit` (networking + protocol layer), and the server-side gateway/pairing flow is live. Use this sequence to get reliable access to your Mac from iPhone:

1. On `yashmacbook`, enable gateway in `~/.gcode/config.toml`:
...
2. Restart gcode server on `yashmacbook`.
...
5. In iOS client, connect to the host printed by `gcode pair` (or set `GCODE_GATEWAY_HOST` on Mac to force the exact hostname shown).
6. Pair with the 6-digit code, then connect over WebSocket.
7. Ask gcode to run Xcode workflows on the Mac via tools, for example:
   - `xcodebuild -list`
   - `xcodebuild -scheme <Scheme> -destination 'platform=iOS Simulator,name=iPhone 15' build`
   - `xed .` (open current project in Xcode)

Because gcode executes tools on `yashmacbook`, this gives you "use Xcode through iPhone" behavior: the phone is the control surface, Mac runs Xcode/build commands.

### Current in-repo iOS implementation (Phase 1)

The repo now includes both:

- `GcodeKit` (`ios/Sources/GcodeKit`) - transport/protocol SDK
- `GcodeMobile` (`ios/Sources/GcodeMobile`) - SwiftUI app shell for pairing + chat
...
3. Open `ios/GcodeMobile.xcodeproj` in Xcode.

If you don't want to install XcodeGen, manually create an iOS app target in Xcode and add `../ios` as a local Swift Package dependency (product: `GcodeKit`).

4. Select the `GcodeMobile` scheme and an iPhone simulator or your device.
5. Build and run.

`project.yml` already wires the app target (`GcodeMobile`) to the local `GcodeKit` package product.


### End-to-end checklist for your goal (iPhone -> yashmacbook -> Xcode commands)

1. On Mac: enable and restart gcode gateway.
2. On Mac: run `gcode pair` and copy the code.
