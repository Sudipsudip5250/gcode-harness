...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Profile a single resumed gcode spawn")
    parser.add_argument("--binary", default="./target/selfdev/gcode")
...
def latest_log_file(log_dir: Path) -> Path:
    logs = sorted(log_dir.glob("gcode-*.log"), key=lambda p: p.stat().st_mtime)
    if not logs:
...
def profile_single_spawn(binary: str, cwd: str, timeout_s: float) -> dict:
    root = Path(tempfile.mkdtemp(prefix="gcode-single-profile-"))
    home = root / "home"
    runtime_dir = root / "run"
    socket_path = runtime_dir / "gcode.sock"
    debug_socket_path = runtime_dir / "gcode-debug.sock"
    ...
    env.update(
        {
            "GCODE_HOME": str(home),
            "GCODE_RUNTIME_DIR": str(runtime_dir),
            "GCODE_SOCKET": str(socket_path),
            "GCODE_DEBUG_SOCKET": str(debug_socket_path),
            "GCODE_SWARM_ENABLED": "0",
            "GCODE_NO_TELEMETRY": "1",
            "GCODE_TRACE": "1",
            "GCODE_TEMP_SERVER": "1",
            "GCODE_SERVER_OWNER_PID": str(os.getpid()),
        }
    )
    ...
    try:
        ...
        while time.perf_counter() < deadline:
            ...
            if rlist:
                ...
                lower = buffer.lower()
                if b"loading session" in lower or b"gcode" in lower or len(buffer) > 4096:
