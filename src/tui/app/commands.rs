        ...
        Ok(result) => {
            ...
            match result {
                Ok(prepared) => {
                    ...
                    let socket = std::env::var("GCODE_SOCKET").ok();
...
fn handle_subagent_model_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if app.is_remote {
        app.push_display_message(DisplayMessage::error(
            "`/subagent-model` requires a live gcode server connection in remote mode.".to_string(),
        ));
        return true;
    }
...
fn handle_subagent_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if app.is_remote {
        app.push_display_message(DisplayMessage::error(
            "`/subagent` requires a live gcode server connection in remote mode.".to_string(),
        ));
        return true;
    }
...
fn git_command_repo_dir(app: &App) -> Result<PathBuf, String> {
    if let Some(path) = active_working_dir(app) {
        ...
        return Err(format!(
            "Unable to run `/git`: session working directory `{}` is not accessible from this gcode client.",
            path.display()
        ));
    }
...
pub(super) fn handle_dictation_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed.starts_with("/dictate ") || trimmed.starts_with("/dictation ") {
        app.push_display_message(DisplayMessage::error(
            "Usage: `/dictate`\nConfigure `[dictation]` in `~/.gcode/config.toml` to customize command, mode, hotkey, and timeout."
                .to_string(),
        ));
        return true;
    }
...
pub(super) fn handle_config_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed == "/subscription" || trimmed == "/subscription status" {
        app.show_gcode_subscription_status();
        return true;
    }
    ...
    if trimmed == "/config edit" {
        ...
        if let Some(path) = Config::path() {
            ...
            app.push_display_message(DisplayMessage {
                role: "system".to_string(),
                content: format!(
                    "Opening config in editor...\n`{} {}`\n\n*Restart gcode after editing for changes to take effect.*",
                    editor,
                    path.display()
                ),
                tool_calls: vec![],
