...
use gcode::message::{ContentBlock, Role, ToolCall};
use gcode::perf::{SyntheticSystemProfile, TuiPerfPolicy, tui_policy_for};
use gcode::prompt::ContextInfo;
use gcode::session::{Session, StoredDisplayRole};
use gcode::side_panel::{
    SidePanelPage, SidePanelPageFormat, SidePanelPageSource, SidePanelSnapshot,
};
use gcode::tui::{DisplayMessage, ProcessingStatus, TuiState, info_widget::InfoWidgetData};
...
struct BenchState {
    ...
    diff_mode: gcode::config::DiffDisplayMode,
    queue_mode: bool,
...
impl BenchState {
    fn new(
        ...
        Ok(Self {
            ...
            is_processing,
            status,
            diff_mode: gcode::config::DiffDisplayMode::Off,
            queue_mode: true,
    ...
    fn from_session(
        ...
        let session = gcode::replay::load_session(id_or_path)
            .with_context(|| format!("failed to load session '{}'", id_or_path))?;
        let mut side_panel =
            gcode::side_panel::snapshot_for_session(&session.id).unwrap_or_default();
        ...
        Ok(Self {
            ...
            diff_mode: if matches!(mode, BenchMode::FileDiff) {
                gcode::config::DiffDisplayMode::File
            } else {
                gcode::config::DiffDisplayMode::Off
            },
            queue_mode: true,
            context_info: ContextInfo::default(),
            info_widget: InfoWidgetData::default(),
    ...
    fn simulate_linked_refresh(&mut self) -> Result<()> {
        ...
        let _ = gcode::side_panel::refresh_linked_page_content(&mut self.side_panel, None);
        Ok(())
    }

    fn prewarm_side_panel(&self, width: u16, height: u16) -> bool {
        gcode::tui::prewarm_focused_side_panel(
            &self.side_panel,
            width,
            height,
            40,
            gcode::tui::mermaid::protocol_type().is_some(),
            false,
        )
    }
}
...
fn stored_message_visible_text(message: &gcode::session::StoredMessage) -> String {
...
impl TuiState for BenchState {
    ...
    fn side_pane_images(&self) -> Vec<gcode::session::RenderedImage> {
        Vec::new()
    }
    ...
    fn batch_progress(&self) -> Option<gcode::bus::BatchProgress> {
        None
    }
    ...
    fn diff_mode(&self) -> gcode::config::DiffDisplayMode {
        self.diff_mode
    }
    ...
    fn context_limit(&self) -> Option<usize> {
        Some(gcode::provider::DEFAULT_CONTEXT_LIMIT)
    }
    ...
    fn render_streaming_markdown(&self, width: usize) -> Vec<ratatui::text::Line<'static>> {
        ...
        gcode::tui::markdown::render_markdown_with_width(&self.streaming_text, Some(width))
    }
    ...
    fn auth_status(&self) -> gcode::auth::AuthStatus {
        gcode::auth::AuthStatus::default()
    }

    fn diagram_mode(&self) -> gcode::config::DiagramDisplayMode {
        gcode::config::DiagramDisplayMode::Pinned
    }
    ...
    fn diagram_pane_position(&self) -> gcode::config::DiagramPanePosition {
        gcode::config::DiagramPanePosition::default()
    }
    ...
    fn side_panel(&self) -> &gcode::side_panel::SidePanelSnapshot {
        &self.side_panel
    }
    ...
    fn chat_native_scrollbar(&self) -> bool {
        gcode::config::config().display.native_scrollbars.chat
    }

    fn side_panel_native_scrollbar(&self) -> bool {
        gcode::config::config().display.native_scrollbars.side_panel
    }

    fn diff_line_wrap(&self) -> bool {
        true
    }
    fn inline_interactive_state(&self) -> Option<&gcode::tui::InlineInteractiveState> {
        None
    }
    ...
    fn session_picker_overlay(
        &self,
    ) -> Option<&std::cell::RefCell<gcode::tui::session_picker::SessionPicker>> {
        None
    }

    fn login_picker_overlay(
        &self,
    ) -> Option<&std::cell::RefCell<gcode::tui::login_picker::LoginPicker>> {
        None
    }

    fn account_picker_overlay(
        &self,
    ) -> Option<&std::cell::RefCell<gcode::tui::account_picker::AccountPicker>> {
        None
    }

    fn usage_overlay(
        &self,
    ) -> Option<&std::cell::RefCell<gcode::tui::usage_overlay::UsageOverlay>> {
        None
    }
    ...
    fn copy_badge_ui(&self) -> gcode::tui::CopyBadgeUiState {
        gcode::tui::CopyBadgeUiState::default()
    }
    ...
    fn copy_selection_range(&self) -> Option<gcode::tui::CopySelectionRange> {
        None
    }

    fn copy_selection_status(&self) -> Option<gcode::tui::CopySelectionStatus> {
        None
    }
    ...
    fn cache_ttl_status(&self) -> Option<gcode::tui::CacheTtlInfo> {
        None
    }
}
...
fn main() -> Result<()> {
    if std::env::var("GCODE_TUI_PROFILE").is_ok() {
        gcode::logging::init();
        if let Some(path) = gcode::logging::log_path() {
    ...
    if matches!(args.mode, BenchMode::MermaidFlicker) {
        let result = gcode::tui::mermaid::debug_flicker_benchmark(args.frames.max(4));
    ...
    if matches!(args.mode, BenchMode::FileDiff) {
        state.diff_mode = gcode::config::DiffDisplayMode::File;
    }
    ...
    if profile_side_panel {
        gcode::tui::mermaid::init_picker();
        gcode::tui::mermaid::clear_active_diagrams();
        gcode::tui::mermaid::clear_streaming_preview_diagram();
        gcode::tui::clear_side_panel_render_caches();
        gcode::tui::reset_side_panel_debug_stats();
        gcode::tui::markdown::reset_debug_stats();
        gcode::tui::mermaid::reset_debug_stats();
        if !args.keep_mermaid_cache {
            let _ = gcode::tui::mermaid::clear_cache();
        }
    ...
    for frame in 0..args.frames {
        ...
        let markdown_before = profile_side_panel.then(gcode::tui::markdown::debug_stats);
        let mermaid_before = profile_side_panel.then(gcode::tui::mermaid::debug_stats);
        let side_panel_before = profile_side_panel.then(gcode::tui::side_panel_debug_stats);
        let frame_start = Instant::now();
        terminal.draw(|f| gcode::tui::render_frame(f, &state))?;
        ...
        {
            let markdown_after = gcode::tui::markdown::debug_stats();
            let mermaid_after = gcode::tui::mermaid::debug_stats();
            let side_panel_after = gcode::tui::side_panel_debug_stats();
    ...
    let side_panel_final_stats = profile_side_panel.then(gcode::tui::side_panel_debug_stats);
    let markdown_final_stats = profile_side_panel.then(gcode::tui::markdown::debug_stats);
    let mermaid_final_stats = profile_side_panel.then(gcode::tui::mermaid::debug_stats);
    let mermaid_ui_summary = if profile_mermaid_ui {
        Some(summarize_mermaid_ui(
            &side_panel_profiles,
            gcode::tui::mermaid::protocol_type().is_some(),
            gcode::tui::mermaid::protocol_type().map(|p| format!("{:?}", p)),
        ))
    } else {
        None
    };
    let actual_policy = summarize_policy("detected", gcode::perf::tui_policy());
    let synthetic_policy = args.synthetic_profile.map(|kind| {
        let synthetic = gcode::perf::synthetic_profile(kind.to_system_profile());
        summarize_policy(
            kind.to_system_profile().label(),
            tui_policy_for(&synthetic, &gcode::config::config().display),
        )
    });
