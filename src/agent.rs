...
const GCODE_NATIVE_TOOLS: &[&str] = &["selfdev", "communicate"];
static RECOVERED_TEXT_WRAPPED_TOOL_CALLS: std::sync::atomic::AtomicU64 =
    std::sync::atomic::AtomicU64::new(0);
static GCODE_REPO_SOURCE_STATE: LazyLock<(Option<String>, Option<bool>)> = LazyLock::new(|| {
...
impl Agent {
    fn should_track_client_cache(&self) -> bool {
        match std::env::var("GCODE_TRACK_CLIENT_CACHE") {
            Ok(value) => {
    ...
    fn messages_for_provider(&mut self) -> (Vec<Message>, Option<CompactionEvent>) {
        if self.provider.uses_gcode_compaction() || self.session.compaction.is_some() {
            let compaction = self.registry.compaction();
            match compaction.try_write() {
                Ok(mut manager) => {
                    ...
                    let messages = {
                        ...
                        if self.provider.uses_gcode_compaction() {
                            let action =
                    ...
                    let event = if self.provider.uses_gcode_compaction() {
    ...
    fn record_client_cache_request(&mut self, messages: &[Message]) {
        ...
        let fast_snapshot =
            if !self.provider.uses_gcode_compaction() && self.session.compaction.is_none() {
