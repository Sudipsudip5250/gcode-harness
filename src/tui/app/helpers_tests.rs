...
fn shell_command_quotes_single_quotes_for_handterm_exec() {
    let command = shell_command(&[
        "/tmp/gcode binary".to_string(),
        "--resume".to_string(),
        "session'quote".to_string(),
    ]);
    assert_eq!(
        command,
        "'/tmp/gcode binary' '--resume' 'session'\"'\"'quote'"
    );
}

#[test]
fn resume_invocation_args_includes_socket_when_present() {
    let args = resume_invocation_args("ses_123", Some("/tmp/gcode-test.sock"));
    assert_eq!(
        args,
        vec![
...
#[test]
fn build_resume_command_uses_imported_gcode_session_for_claude_code() {
...
#[test]
fn build_resume_command_uses_imported_gcode_session_for_codex() {
    let (program, args, title) = build_resume_command(
...
fn gather_ambient_info_filters_to_session_reminders_when_ambient_disabled() {
    ...
    let _home = EnvVarGuard::set_path("GCODE_HOME", temp.path());
