use super::{
    ClientCandidate, extract_session_short_name_from_window_title, focused_gcode_session,
...
fn select_candidate_prefers_title_match() {
    ...
    let selected = select_candidate(&candidates, Some("🦀 gcode/sleeping Crab [self-dev]"))
...
#[test]
fn extract_session_short_name_from_gcode_window_title() {
    assert_eq!(
        extract_session_short_name_from_window_title("🦢 gcode/cliff Swan [self-dev]"),
        Some("swan".to_string())
    );
    assert_eq!(
        extract_session_short_name_from_window_title("🦊 gcode Fox"),
        Some("fox".to_string())
    );
}
...
fn remember_and_read_last_focused_session() {
    ...
    let prev = std::env::var_os("GCODE_HOME");
...
#[cfg(target_os = "linux")]
#[test]
fn focused_gcode_session_uses_niri_window_title_when_process_name_is_generic() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().expect("tempdir");
    let _home = EnvVarGuard::set("GCODE_HOME", temp.path());

    let active_dir = temp.path().join("active_pids");
    std::fs::create_dir_all(&active_dir).expect("create active_pids");
    std::fs::write(active_dir.join("session_swan_123"), "12345").expect("write active pid");

    let focused_process = ChildGuard::spawn_named("gcode");
    let bin_dir = temp.path().join("bin");
    install_fake_niri(
        &bin_dir,
        focused_process.pid(),
        "🦢 gcode/cliff Swan [self-dev]",
    );
    ...
    assert_eq!(
        focused_gcode_session().expect("resolve focused session"),
