...
fn matrix_openrouter_like_sources_include_all_static_profiles() {
    ...
    crate::env::remove_var("GCODE_OPENROUTER_API_KEY_NAME");
    crate::env::remove_var("GCODE_OPENROUTER_ENV_FILE");
    crate::env::remove_var("GCODE_OPENAI_COMPAT_API_KEY_NAME");
    crate::env::remove_var("GCODE_OPENAI_COMPAT_ENV_FILE");
...
fn matrix_openrouter_like_sources_accept_valid_overrides() {
    ...
    crate::env::set_var("GCODE_OPENROUTER_API_KEY_NAME", "ALT_OPENROUTER_KEY");
    crate::env::set_var("GCODE_OPENROUTER_ENV_FILE", "alt-openrouter.env");
    crate::env::set_var("GCODE_OPENAI_COMPAT_API_KEY_NAME", "ALT_COMPAT_KEY");
    crate::env::set_var("GCODE_OPENAI_COMPAT_ENV_FILE", "alt-compat.env");
...
fn named_provider_profile_maps_to_openai_compatible_runtime_env() {
    ...
    assert_eq!(
        std::env::var("GCODE_OPENROUTER_MODEL_CATALOG")
            .ok()
            .as_deref(),
        Some("0")
    );
    assert_eq!(
        std::env::var("GCODE_OPENROUTER_MODEL").ok().as_deref(),
        Some("opaque/model@id")
    );
    assert_eq!(
        std::env::var("GCODE_OPENROUTER_STATIC_MODELS")
            .ok()
            .as_deref(),
        Some("opaque/model@id\nanother-local-id")
    );
    assert_eq!(
        std::env::var("GCODE_OPENROUTER_AUTH_HEADER")
            .ok()
            .as_deref(),
        Some("api-key")
    );
    assert_eq!(
        std::env::var("GCODE_OPENROUTER_AUTH_HEADER_NAME")
            .ok()
            .as_deref(),
        Some("x-api-key")
    );
    assert_eq!(
        std::env::var("GCODE_NAMED_PROVIDER_PROFILE")
            .ok()
            .as_deref(),
        Some("my-gateway")
    );
...
fn named_provider_inline_api_key_is_private_runtime_fallback() {
    ...
    let _guard = EnvGuard::save(&[
        ...
        "GCODE_OPENROUTER_MODEL_CATALOG",
        "GCODE_NAMED_PROVIDER_PROFILE",
        "GCODE_PROVIDER_MY_GATEWAY_API_KEY",
    ]);
    ...
    assert_eq!(
        std::env::var("GCODE_OPENROUTER_API_KEY_NAME")
            .ok()
            .as_deref(),
        Some("GCODE_PROVIDER_MY_GATEWAY_API_KEY")
    );
    assert_eq!(
        std::env::var("GCODE_PROVIDER_MY_GATEWAY_API_KEY")
            .ok()
            .as_deref(),
        Some("inline-secret")
    );
...
fn matrix_openrouter_like_sources_reject_invalid_overrides() {
    ...
    crate::env::set_var("GCODE_OPENROUTER_API_KEY_NAME", "bad-key-name");
    crate::env::set_var("GCODE_OPENROUTER_ENV_FILE", "../bad.env");
    crate::env::set_var("GCODE_OPENAI_COMPAT_API_KEY_NAME", "bad key");
    crate::env::set_var("GCODE_OPENAI_COMPAT_ENV_FILE", "../bad-compat.env");
...
fn matrix_openai_compatible_profile_overrides_apply_when_valid() {
    ...
    crate::env::set_var("GCODE_OPENAI_COMPAT_API_KEY_NAME", "GROQ_API_KEY");
    crate::env::set_var("GCODE_OPENAI_COMPAT_ENV_FILE", "groq.env");
    crate::env::set_var("GCODE_OPENAI_COMPAT_DEFAULT_MODEL", "openai/gpt-oss-120b");
...
fn matrix_openai_compatible_profile_overrides_reject_invalid_values() {
    ...
    crate::env::set_var("GCODE_OPENAI_COMPAT_API_BASE", "http://example.com/v1");
    crate::env::set_var("GCODE_OPENAI_COMPAT_API_KEY_NAME", "bad-key-name");
    crate::env::set_var("GCODE_OPENAI_COMPAT_ENV_FILE", "../bad.env");
...
fn matrix_openai_compatible_profile_overrides_read_from_env_file() {
    ...
    crate::env::remove_var("GCODE_OPENAI_COMPAT_API_BASE");
    crate::env::remove_var("GCODE_OPENAI_COMPAT_API_KEY_NAME");
    crate::env::remove_var("GCODE_OPENAI_COMPAT_ENV_FILE");
    crate::env::remove_var("GCODE_OPENAI_COMPAT_DEFAULT_MODEL");
    std::fs::write(
        config_root.join(OPENAI_COMPAT_PROFILE.env_file),
        concat!(
            "GCODE_OPENAI_COMPAT_API_BASE=https://api.example.com/v1\n",
            "GCODE_OPENAI_COMPAT_API_KEY_NAME=EXAMPLE_API_KEY\n",
            "GCODE_OPENAI_COMPAT_ENV_FILE=example.env\n",
            "GCODE_OPENAI_COMPAT_DEFAULT_MODEL=example/model\n",
        ),
    )
    .expect("env file");
...
fn matrix_openai_compatible_localhost_override_allows_no_auth() {
    ...
    crate::env::set_var("GCODE_OPENAI_COMPAT_API_BASE", "http://localhost:11434/v1");
    crate::env::remove_var("GCODE_OPENAI_COMPAT_API_KEY_NAME");
    crate::env::remove_var("GCODE_OPENAI_COMPAT_ENV_FILE");
    crate::env::remove_var("GCODE_OPENAI_COMPAT_LOCAL_ENABLED");
...
fn matrix_load_api_key_from_env_or_config_prefers_env() {
    ...
    let _guard = EnvGuard::save(&["GCODE_HOME", "OPENCODE_API_KEY"]);
    crate::env::set_var("GCODE_HOME", temp.path());
...
fn load_api_key_accepts_legacy_zai_key_name() {
    ...
    let _guard = EnvGuard::save(&["GCODE_HOME", "ZHIPU_API_KEY", "ZAI_API_KEY"]);
    crate::env::set_var("GCODE_HOME", temp.path());
