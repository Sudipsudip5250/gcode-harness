...
fn test_safety_classification() {
    use gcode::safety::SafetySystem;

    let safety = SafetySystem::new();

    // Tier 1: auto-allowed
    assert!(safety.classify("read") == gcode::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("glob") == gcode::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("grep") == gcode::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("memory") == gcode::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("todoread") == gcode::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("todowrite") == gcode::safety::ActionTier::AutoAllowed);

    // Tier 2: requires permission
    assert!(safety.classify("bash") == gcode::safety::ActionTier::RequiresPermission);
    assert!(safety.classify("edit") == gcode::safety::ActionTier::RequiresPermission);
    assert!(safety.classify("write") == gcode::safety::ActionTier::RequiresPermission);
    assert!(
        safety.classify("create_pull_request") == gcode::safety::ActionTier::RequiresPermission
    );
    assert!(safety.classify("send_email") == gcode::safety::ActionTier::RequiresPermission);
...
fn test_safety_permission_flow() {
    use gcode::safety::{PermissionRequest, PermissionResult, SafetySystem, Urgency};
...
fn test_safety_transcript() {
    use gcode::safety::{AmbientTranscript, SafetySystem, TranscriptStatus};
...
fn test_safety_summary_generation() {
    use gcode::safety::{ActionLog, ActionTier, SafetySystem};
