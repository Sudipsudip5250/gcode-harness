...
impl ToolDefinition {
    ...
    ///
    /// This uses gcode's standard chars/4 heuristic, matching other token
...
pub enum ContentBlock {
    ...
    /// compaction state across turns/saves when gcode explicitly triggers it.
    OpenAICompaction {
...
pub enum StreamEvent {
    ...
    /// Native tool call from a provider bridge that needs execution by gcode
    NativeToolCall {
        request_id: String,
        tool_name: String,
        input: serde_json::Value,
