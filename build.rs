...
fn main() {
    ...
    let dirty = match std::env::var("GCODE_BUILD_GIT_DIRTY") {
        Ok(value) => matches!(
    ...
    let raw_log = std::env::var("GCODE_BUILD_CHANGELOG_RAW")
        .ok()
    ...
    if std::env::var("GCODE_RELEASE_BUILD").is_ok() {
        println!("cargo:rustc-env=GCODE_RELEASE_BUILD=1");
    }
    ...
    println!("cargo:rerun-if-env-changed=GCODE_RELEASE_BUILD");
    println!("cargo:rerun-if-env-changed=GCODE_BUILD_SEMVER");
}
...
fn explicit_build_semver_override() -> Option<String> {
    std::env::var("GCODE_BUILD_SEMVER")
        .ok()
...
fn build_counter_file() -> PathBuf {
    if let Some(target_root) = target_root_from_out_dir() {
        return target_root.join("gcode-build").join("patch-counters.txt");
    }
...
fn metadata_value(key: &str) -> Option<String> {
    let path = std::env::var("GCODE_BUILD_METADATA_FILE").ok()?;
