...
#[derive(Parser, Debug)]
#[command(name = "gcode")]
#[command(version = env!("GCODE_VERSION"))]
#[command(about = "J-Code: A coding agent using Claude Max or ChatGPT Pro subscriptions")]
pub(crate) struct Args {
    /// Provider to use (gcode, claude, openai, openai-api, openrouter, azure, opencode, opencode-go, zai, 302ai, baseten, cortecs, comtegra, deepseek, firmware, huggingface, moonshotai, nebius, scaleway, stackit, groq, mistral, perplexity, togetherai, deepinfra, xai, lmstudio, ollama, chutes, cerebras, alibaba-coding-plan, openai-compatible, cursor, copilot, gemini, antigravity, google, or auto-detect)
    ...
    /// Disable auto-detection of gcode repository and self-dev mode
    #[arg(long, global = true)]
...
pub(crate) enum Command {
    ...
    Login {
        ...
        /// OpenAI-compatible API key. If omitted, gcode prompts securely when needed.
        #[arg(long)]
    ...
    /// Update gcode to the latest version
    Update,
    ...
    /// Debug socket CLI - interact with running gcode server
    Debug {
    ...
    /// Run configured dictation: send to last-focused gcode client or type raw text
    Dictate {
        /// Type the transcript into the focused app instead of sending to gcode
        #[arg(long)]
        r#type: bool,
    },

    /// Set up a global hotkey (Alt+;) to launch gcode
    SetupHotkey {
    ...
    /// Install a launcher so gcode appears in your app launcher
    SetupLauncher,
    ...
    /// Save or restore the current set of open gcode windows across a system reboot
    Restart {
        #[command(subcommand)]
...
pub(crate) enum RestartCommand {
    /// Save a reboot snapshot of currently active gcode windows
    Save {
        /// Restore this reboot snapshot automatically the next time plain `gcode` starts
        #[arg(long)]
        auto_restore: bool,
    },
...
pub(crate) enum ProviderCommand {
    ...
    Add {
        ...
        /// API key value to store in gcode's private provider env file. Prefer --api-key-stdin for shell history safety.
        #[arg(long, conflicts_with_all = ["api_key_stdin", "no_api_key"])]
        api_key: Option<String>,

        /// Read the API key from stdin and store it in gcode's private provider env file
        #[arg(long, conflicts_with = "no_api_key")]
        api_key_stdin: bool,
        ...
        /// Private env file name under gcode's app config directory for stored API keys
        #[arg(long)]
