...
//! - macOS: detect suboptimal terminal and offer guided Ghostty setup via gcode.
//! - Linux: create a .desktop launcher file.
//!
//! Each nudge can be dismissed permanently with "Don't ask again".
//! State is persisted in `~/.gcode/setup_hints.json`.
...
use macos_terminal::{
    ...
    launch_command_for_macos_terminal, paused_gcode_shell_command, save_preferred_macos_terminal,
};
#[cfg(windows)]
use windows_setup::{
...
impl SetupHintsState {
    fn path() -> Result<PathBuf> {
        Ok(storage::gcode_dir()?.join("setup_hints.json"))
    }
...
fn mac_hotkey_support_dir() -> Result<PathBuf> {
    Ok(storage::gcode_dir()?.join("hotkey"))
}
...
fn install_macos_hotkey_listener(
    ...
    let shell_command = paused_gcode_shell_command(&exe_path);

    let launch_script_path = hotkey_dir.join("launch_gcode.sh");
    std::fs::write(
        &launch_script_path,
...
<dict>
    <key>Label</key>
    <string>com.gcode.hotkey</string>
    ...
    <dict>
        <key>GCODE_PREFERRED_TERMINAL</key>
...
"#,
    ...
    let status = std::process::Command::new("launchctl")
        ...
        .status()
        .context("failed to load gcode LaunchAgent")?;
    if !status.success() {
...
fn startup_hints_for_launch(state: &SetupHintsState) -> Option<StartupHints> {
    ...
    let spawn_notice = if !state.hotkey_configured || state.startup_spawn_hint_dismissed {
        ...
        Some(format!(
            "Press Alt+; from anywhere to open gcode in {}.",
    ...
    if state.launch_count == 1 {
        let mut message = "Tip: gcode is left-aligned by default. Use `/alignment centered` or press `Alt+C` to toggle left/centered for the current session.".to_string();
    ...
    if state.launch_count <= 3 {
        let config_path = crate::config::Config::path()
            ...
            .unwrap_or_else(|| "~/.gcode/config.toml".to_string());
...
fn macos_guided_ghostty_message(current_terminal: MacTerminalKind) -> String {
    format!(
        "I want to upgrade my macOS terminal setup for gcode. Please guide me step-by-step, wait for confirmation between steps, and keep each step concise.\n\nCurrent terminal: {}\nGoal: install Ghostty and use it for gcode.\n\nPlease help me with:\n1) Detecting if Homebrew is installed (and installing it if missing)\n2) Installing Ghostty\n3) Launching Ghostty and setting it as my preferred terminal for gcode\n4) Optional: adding a macOS keyboard shortcut/launcher flow for gcode\n5) Verifying gcode runs in Ghostty and that inline images/graphics work\n\nAssume I am not an expert; provide exact commands and where to click in macOS settings when needed.",
...
fn nudge_macos_ghostty(state: &mut SetupHintsState) -> Option<String> {
    ...
    eprintln!(
        "\x1b[36m│\x1b[0m \x1b[1m💡 Better macOS terminal for gcode: Ghostty\x1b[0m                \x1b[36m│\x1b[0m"
    );
    eprintln!(
    ...
    if ghostty_installed {
        ...
        eprintln!(
            "\x1b[36m│\x1b[0m    Ghostty offers fast rendering and great gcode UX.         \x1b[36m│\x1b[0m"
        );
    }
    eprintln!(
        "\x1b[36m│\x1b[0m                                                             \x1b[36m│\x1b[0m"
    ...
    eprintln!(
        "\x1b[36m│\x1b[0m    Let gcode guide you through setup right now?             \x1b[36m│\x1b[0m"
    );
    eprintln!(
...
/// Manual `gcode setup-hotkey` command.
///
...
pub fn run_setup_hotkey(_listen_macos_hotkey: bool) -> Result<()> {
    #[cfg(target_os = "macos")]
    {
        ...
        let terminal = effective_macos_terminal();
        eprintln!("\x1b[1mgcode setup-hotkey\x1b[0m");
        eprintln!();
        eprintln!("  Preferred terminal: {}", terminal.label());
        eprintln!("  Installing a LaunchAgent so Alt+; opens gcode from anywhere.");
        eprintln!();
        ...
        match install_macos_hotkey_listener(Some(terminal)) {
            Ok(installed_terminal) => {
                ...
                let _ = state.save();
                eprintln!(
                    "  \x1b[32m✓\x1b[0m Created hotkey (\x1b[1mAlt+;\x1b[0m) → {} + gcode",
                    installed_terminal.label()
                );
                eprintln!();
                eprintln!(
                    "  Press \x1b[1mAlt+;\x1b[0m from anywhere to open gcode in {}.",
                    installed_terminal.label()
                );
                return Ok(());
            }
...
fn run_macos_hotkey_listener() -> Result<()> {
    ...
    let launch_script = mac_hotkey_support_dir()?.join("launch_gcode.sh");
    let manager =
...
pub fn maybe_show_setup_hints() -> Option<StartupHints> {
    ...
    // `gcode setup-launcher` explicitly.
...
/// Manual `gcode setup-launcher` command.
pub fn run_setup_launcher() -> Result<()> {
    #[cfg(target_os = "macos")]
    {
        let mut state = SetupHintsState::load();
        eprintln!("\x1b[1mgcode setup-launcher\x1b[0m");
        eprintln!();
        ...
        match install_macos_app_launcher() {
            Ok((app_dir, terminal)) => {
                ...
                eprintln!(
                    "  \x1b[32m✓\x1b[0m Spotlight/Launchpad/Dock will launch gcode in {}",
                    terminal.label()
                );
                eprintln!();
...
/// Create a desktop shortcut/launcher for gcode.
///
/// - Windows: creates a .lnk shortcut on the Desktop
/// - macOS: creates a gcode.app bundle in ~/Applications/
