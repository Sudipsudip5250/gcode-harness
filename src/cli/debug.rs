...
pub async fn run_debug_command(
    ...
    if !crate::transport::is_socket_path(&debug_socket) {
        ...
        eprintln!("\nMake sure:");
        eprintln!("  1. A gcode server is running (gcode or gcode serve)");
        eprintln!("  2. debug_socket is enabled in ~/.gcode/config.toml");
        eprintln!("     [display]");
        eprintln!("     debug_socket = true");
        eprintln!("\nOr use 'gcode debug start' to start a server.");
        eprintln!("Use 'gcode debug list' to see running servers.");
...
async fn debug_list_servers() -> Result<()> {
    ...
    if servers.is_empty() {
        println!("No running gcode servers found.");
        println!("\nStart one with: gcode debug start");
        return Ok(());
    }

    println!("Running gcode servers:\n");
    ...
    println!("  gcode debug -s /path/to/socket.sock sessions");
...
async fn debug_start_server(arg: &str, socket_path: Option<String>) -> Result<()> {
            ...
            .is_ok()
    {
        ...
        eprintln!("Use 'gcode debug list' to see all servers.");
        return Ok(());
    }
    ...
    eprintln!("Starting gcode server...");
    ...
    if crate::transport::is_socket_path(&debug_socket) {
        ...
        eprintln!("⚠ Debug socket not enabled. Add to ~/.gcode/config.toml:");
        eprintln!("  [display]");
        eprintln!("  debug_socket = true");
    }
