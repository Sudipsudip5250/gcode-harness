...
> Any references below to `/tmp/gcode-selfdev.sock`, `canary-wrapper`, or
> `GCODE_SELFDEV_MODE` describe the pre-merge architecture or transition steps,
...
### Normal mode
- Main socket: runtime `gcode.sock`
- Debug socket: runtime `gcode-debug.sock`
- Startup path: `gcode` -> default client flow -> spawn `gcode serve` if needed

### Self-dev mode
- Main socket: `/tmp/gcode-selfdev.sock`
- Debug socket: `/tmp/gcode-selfdev-debug.sock`
- Startup path:
  - repo auto-detection or `gcode self-dev`
...
- explicit `gcode self-dev`
- current working directory is the gcode repo (auto-detected)
...
Expected result:
- opening gcode inside the repo uses the shared server path by default
- session still becomes canary/self-dev
- explicit `gcode self-dev` command may still use legacy wrapper temporarily

### Phase 2 - Move explicit `gcode self-dev` onto shared server path
...
Expected result:
- both auto-detected self-dev and explicit `gcode self-dev` share one server
...
Changes:
- deprecate `/tmp/gcode-selfdev.sock` and `/tmp/gcode-selfdev-debug.sock`
...
There are multiple `GCODE_SELFDEV_MODE` checks across startup, hot reload, and server behavior; these need to be separated into:
...
2. Land explicit `gcode self-dev` shared-path behavior
