...
const GITHUB_REPO: &str = "bitan-del/gcode-harness";
...
pub fn is_release_build() -> bool {
    option_env!("GCODE_RELEASE_BUILD").is_some()
}

fn current_update_semver() -> &'static str {
    env!("GCODE_UPDATE_SEMVER")
}
...
fn metadata_path() -> Result<PathBuf> {
    Ok(storage::gcode_dir()?.join("update_metadata.json"))
}

fn source_build_root() -> Result<PathBuf> {
    Ok(storage::gcode_dir()?.join("builds").join("source"))
}

fn source_build_repo_dir() -> Result<PathBuf> {
    Ok(source_build_root()?.join("gcode"))
}
...
pub fn should_auto_update() -> bool {
    if std::env::var("GCODE_NO_AUTO_UPDATE").is_ok() {
        return false;
    }
...
fn prepare_stable_update_blocking() -> Result<PreparedUpdate> {
    let current_version = env!("GCODE_VERSION");
...
/// Build gcode from source by cloning/pulling the repo and running cargo build
fn build_from_source() -> Result<PathBuf> {
    let started = Instant::now();
    let build_dir = source_build_root()?;
    fs::create_dir_all(&build_dir)?;

    let repo_dir = build_dir.join("gcode");
    ...
    if repo_dir.join(".git").exists() {
        ...
        let output = std::process::Command::new("git")
            .args([
                "clone", "--depth", "1", "--branch", "main", &clone_url, "gcode",
            ])
...
pub fn download_and_install_blocking_with_progress(
    ...
    let temp_path = temp_dir.join(format!("gcode-update-{}", std::process::id()));
    ...
    if asset.name.ends_with(".tar.gz") {
        ...
        for entry in archive.entries()? {
            ...
            entry.unpack(&dest)?;
            if file_name.starts_with("gcode") && !file_name.ends_with(".bin") {
                extracted_binary = Some(dest);
            }
        }
        let Some(extracted_binary) = extracted_binary else {
            anyhow::bail!("Could not find gcode binary inside tar.gz archive");
        };
...
pub fn check_and_maybe_update(auto_install: bool) -> UpdateCheckResult {
    ...
    match check_for_update_blocking() {
        Ok(Some(release)) => {
            let current = env!("GCODE_VERSION").to_string();
...
mod tests {
    use super::*;
    use gcode_update_core::parse_sha256sums;
    ...
    fn test_parse_sha256sums_accepts_standard_and_binary_lines() {
        ...
        let contents = format!(
            "# generated by release workflow\n{}  gcode-linux-x86_64.tar.gz\r\n{} *gcode-windows-x86_64.exe\n",
            digest_a, digest_b
        );
        let parsed = parse_sha256sums(&contents).unwrap();
        assert_eq!(
            parsed.get("gcode-linux-x86_64.tar.gz").map(String::as_str),
            Some(digest_a.as_str())
        );
        assert_eq!(
            parsed.get("gcode-windows-x86_64.exe").map(String::as_str),
    ...
    fn test_verify_asset_checksum_text_accepts_matching_digest() {
        ...
        let contents = format!("{}  gcode-linux-x86_64.tar.gz\n", digest);
        verify_asset_checksum_text(&contents, "gcode-linux-x86_64.tar.gz", bytes).unwrap();
    }
    ...
    fn test_verify_asset_checksum_text_rejects_mismatch() {
        ...
        let contents = format!("{}  gcode-linux-x86_64.tar.gz\n", wrong);
        let err = verify_asset_checksum_text(&contents, "gcode-linux-x86_64.tar.gz", b"actual")
            .unwrap_err()
            .to_string();
    ...
    fn test_parse_sha256sums_rejects_invalid_digest() {
        let err = parse_sha256sums("not-a-sha  gcode-linux-x86_64.tar.gz\n")
            .unwrap_err()
            .to_string();
