...
This document audits the current provider, session, and shared-contract seams in the gcode workspace and recommends the next **realistic** crate moves that improve modularity without creating high-churn dependency cycles.
...
1. **Add a small `gcode-shared-contracts` crate** for the serde-only protocol/session overlap types that already act like shared contracts.
2. **After that, add a narrow `gcode-session-contracts` crate** for session metadata/replay/view structs that are widely reused but do not need the full `Session` runtime.
3. **If we want one more provider-side move before a larger provider refactor, extract the pure provider identity/selection layer** into `gcode-provider-core` or a small `gcode-provider-selection` crate.
...
- `gcode-provider-core`: no local workspace deps
- `gcode-provider-metadata`: no local workspace deps
- `gcode-provider-openrouter`: no local workspace deps
- `gcode-provider-gemini`: no local workspace deps
...
- `gcode-provider-metadata` holds stable login/profile catalog data
- `gcode-provider-core` holds route/cost/shared HTTP client/core value types
- `gcode-provider-openrouter` holds OpenRouter-specific catalog/cache/ranking/model-spec support
- `gcode-provider-gemini` holds Gemini Code Assist schema/types/support helpers
...
- either a new `crates/gcode-provider-selection`
- or a small `provider_identity` / `selection` module inside `gcode-provider-core`
...
### Option B: narrow `gcode-session-contracts`
...
### Option C: add `gcode-shared-contracts`
...
Create `crates/gcode-shared-contracts`.
...
Create `crates/gcode-session-contracts`.

Do this only after Phase 1, so session replay types can point at `gcode_shared_contracts::*` instead of `crate::protocol::*` or `crate::plan::*`.
...
1. Add `crates/gcode-shared-contracts` with serde-only types from `plan.rs` and the small protocol/session overlap set.
...
3. Re-measure touched-file compile times for:
   ...
   - `src/protocol.rs`
   - `src/provider/mod.rs`
4. If the new seam stays clean, follow with a narrow `gcode-session-contracts` extraction.
