...
pub(crate) fn resumed_window_title(session_id: &str) -> String {
    ...
    if let Some(server_info) = crate::registry::find_server_by_socket_sync(&server::socket_path()) {
        format!("{} gcode/{} {}", icon, server_info.name, session_name)
    } else {
        format!("{} gcode {}", icon, session_name)
    }
}
...
fn focus_title_best_effort(title: &str) {
    ...
    cmd.arg("-c")
        .arg(
            "sleep 0.4; \
             if command -v wmctrl >/dev/null 2>&1; then wmctrl -a \"$GCODE_WINDOW_TITLE\" >/dev/null 2>&1 && exit 0; fi; \
             if command -v xdotool >/dev/null 2>&1; then xdotool search --name \"$GCODE_WINDOW_TITLE\" windowactivate >/dev/null 2>&1 && exit 0; fi; \
             exit 0",
        )
        .env("GCODE_WINDOW_TITLE", title)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
...
pub async fn run_tui_client(
    ...
    if let Some(ref session_id) = resume_session {
        ...
        let _ = crossterm::execute!(std::io::stdout(), crossterm::terminal::SetTitle("gcode"));
    }
        ...
        if let Some(output) = video_output {
            let output_path = if output == "auto" {
                ...
                let safe_name = session_id_or_path
                    ...
                    .collect::<String>();
                std::path::PathBuf::from(format!("gcode_swarm_replay_{}_{}.mp4", safe_name, date))
            } else {
    ...
    if let Some(output) = video_output {
        let output_path = if output == "auto" {
            ...
            let safe_name = session_name
                ...
                .collect::<String>();
            std::path::PathBuf::from(format!("gcode_replay_{}_{}.mp4", safe_name, date))
        } else {
            std::path::PathBuf::from(output)
        };
        eprintln!(
...
fn resume_terminal_candidates_windows() -> Vec<String> {
    std::env::var("GCODE_RESUME_TERMINAL")
        .ok()
        .map(|value| {
            value
                .split(',')
    ...
    match tui::session_picker::pick_session()? {
        Some(
            ...
            if targets.len() == 1 {
                ...
                let resolved_target = crate::import::resolve_resume_target_to_gcode(target)?;
                ...
                for target in targets {
                    let resolved_target =
                        match crate::import::resolve_resume_target_to_gcode(&target) {
                            Ok(target) => target,
                            Err(e) => {
        ...
        Some(tui::session_picker::PickerResult::SelectedInNewTerminal(targets)) => {
            ...
            for target in targets {
                let resolved_target = match crate::import::resolve_resume_target_to_gcode(&target) {
                    Ok(target) => target,
                    Err(e) => {
                        eprintln!("Failed to import selected session: {}", e);
                        continue;
            ...
            for session_id in recovered {
                ...
                match spawn_resume_in_new_terminal(&exe, &session_id, &session_cwd) {
                    ...
                    Ok(false) => {
                        ...
                        eprintln!("  gcode --resume {}", session_id);
                    }
                    Err(e) => {
