#!/bin/bash
# Replay a gcode recording as video
#
# This script:
# 1. Starts a fresh gcode instance in a new terminal
...
# Start gcode in a new kitty window
echo "🚀 Starting gcode..."
kitty --title "gcode-replay" -e bash -c "cd $(pwd) && ~/.cargo/bin/gcode; read -p 'Press Enter to close...'" &
KITTY_PID=$!
sleep 2  # Wait for gcode to start

# Focus the new window
sleep 0.5
# Find and focus the gcode-replay window
WINDOW_ID=$(niri msg windows 2>/dev/null | grep -B5 "gcode-replay" | grep -oP 'Window ID \K\d+' | head -1)
if [ -n "$WINDOW_ID" ]; then
