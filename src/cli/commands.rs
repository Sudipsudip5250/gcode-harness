...
async fn run_ambient_visible() -> Result<()> {
    ...
    let _ = crossterm::execute!(
        std::io::stdout(),
        crossterm::terminal::SetTitle("🤖 gcode ambient cycle")
    );
...
pub fn run_memory_command(cmd: MemorySubcommand) -> Result<()> {
    ...
    match cmd {
        ...
        MemorySubcommand::ClearTest => {
            let test_dir = storage::gcode_dir()?.join("memory").join("test");
            if test_dir.exists() {
...
pub fn run_pair_command(list: bool, revoke: Option<String>) -> Result<()> {
    ...
    if !gw_config.enabled {
        eprintln!("\x1b[33m⚠\x1b[0m  Gateway is disabled. Enable it in ~/.gcode/config.toml:\n");
        eprintln!("    \x1b[2m[gateway]\x1b[0m");
        eprintln!("    \x1b[2menabled = true\x1b[0m");
        eprintln!("    \x1b[2mport = {}\x1b[0m\n", gw_config.port);
        eprintln!("  Then restart the gcode server.\n");
    }

    let code = registry.generate_pairing_code();
    let connect_host = resolve_connect_host(&gw_config.bind_addr);
    let pair_uri = format!(
        "gcode://pair?host={}&port={}&code={}",
        connect_host, gw_config.port, code
    );

    eprintln!();
    eprintln!("  \x1b[1mScan with the gcode iOS app:\x1b[0m\n");
    ...
    if connect_host == "<your-mac-hostname>" {
        eprintln!(
            "\n  \x1b[33mTip:\x1b[0m set GCODE_GATEWAY_HOST to your reachable Tailscale hostname."
        );
    }
...
pub fn resolve_connect_host(bind_addr: &str) -> String {
    if bind_addr == "0.0.0.0" || bind_addr == "::" {
        if let Some(host) = std::env::var("GCODE_GATEWAY_HOST")
            .ok()
    ...
    match action {
        ...
        "status" => {
            ...
            if status.ready {
                ...
                println!(
                    "\nThe browser bridge is connected, but the installed Firefox extension is out of date for this gcode build. Run `gcode browser setup` to repair or update it."
                );
            } else {
                println!("\nRun `gcode browser setup` to install or repair it.");
            }
        }
        other => {
...
fn run_command_auto_poke_enabled() -> bool {
    std::env::var("GCODE_RUN_AUTO_POKE")
        .ok()
        .map(|value| {
...
fn run_command_auto_poke_max_turns() -> Option<usize> {
    std::env::var("GCODE_RUN_AUTO_POKE_MAX_TURNS")
        .ok()
...
async fn run_single_message_command_plain_with_auto_poke(
    ...
    loop {
        ...
        eprintln!(
            "Auto-poking: {} incomplete todo(s). Set GCODE_RUN_AUTO_POKE=0 to disable.",
            incomplete.len()
        );
    }
    Ok(())
