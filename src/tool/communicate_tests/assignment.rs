...
async fn communicate_assign_task_can_spawn_fallback_agent() {
    ...
    let socket_path = runtime_dir.path().join("gcode.sock");
    let _runtime = EnvGuard::set("GCODE_RUNTIME_DIR", runtime_dir.path());
    let _socket = EnvGuard::set("GCODE_SOCKET", &socket_path);
    let _debug = EnvGuard::set("GCODE_DEBUG_CONTROL", "1");
