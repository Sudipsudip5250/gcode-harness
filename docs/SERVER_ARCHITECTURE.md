...
gcode uses a **single-server, multi-client** architecture. One server process
...
│                                                                             │
│  gcode serve                                                                │
│  ├── Unix socket:  /run/user/$UID/gcode.sock                                │
│  ├── Debug socket: /run/user/$UID/gcode-debug.sock                          │
│  ├── Registry:     ~/.gcode/servers.json                                    │
...
The server name persists across reloads via the registry (`~/.gcode/servers.json`).
...
```
  ...
  ─────                          ───────                     ──────
  gcode (first run)              gcode (subsequent)          /reload
       │                              │                          │
...
When you run `gcode`, it checks if a server is already running:

1. **Server exists**: connect directly as a client
2. **No server**: spawn `gcode serve` as a detached daemon (with `setsid`),
...
```
/run/user/$UID/
├── gcode.sock          # Main communication socket
└── gcode-debug.sock    # Debug/testing socket
```

## Self-Dev Mode

When running `gcode` inside the gcode repository:

1. Auto-detects the repo and enables self-dev mode
2. Connects to the normal shared gcode server
...
| Scenario | Behavior |
|----------|----------|
| First `gcode` run | Spawns server daemon, connects |
| Subsequent `gcode` | Connects to existing server |
...
| Resume session | `gcode --resume fox` reconnects to existing session |
