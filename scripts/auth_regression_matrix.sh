...
bin=${GCODE_AUTH_MATRIX_BIN:-}
out_dir=${GCODE_AUTH_MATRIX_OUT:-"$repo_root/target/auth-test-reports"}
prompt=${GCODE_AUTH_MATRIX_PROMPT:-"Reply with exactly AUTH_TEST_OK and nothing else. Do not call tools."}
providers=${GCODE_AUTH_MATRIX_PROVIDERS:-"claude copilot openrouter deepseek zai alibaba-coding-plan openai-compatible"}
mode=${GCODE_AUTH_MATRIX_MODE:-configured}
keep_going=${GCODE_AUTH_MATRIX_KEEP_GOING:-1}
per_command_timeout=${GCODE_AUTH_MATRIX_TIMEOUT:-90}

usage() {
  cat <<'EOF'
Usage: scripts/auth_regression_matrix.sh [options]

Runs gcode auth-test across the auth/provider matrix and writes one JSON report per provider.
...
Options:
  ...
  --bin PATH            gcode binary to run (default: cargo run --bin gcode --)
...
Environment equivalents:
  GCODE_AUTH_MATRIX_BIN=/path/to/gcode
  GCODE_AUTH_MATRIX_OUT=target/auth-test-reports
  GCODE_AUTH_MATRIX_PROVIDERS="claude deepseek zai"
  GCODE_AUTH_MATRIX_MODE=configured|all
  GCODE_AUTH_MATRIX_LOGIN=1
  GCODE_AUTH_MATRIX_NO_SMOKE=1
  GCODE_AUTH_MATRIX_NO_TOOL_SMOKE=1
  GCODE_AUTH_MATRIX_KEEP_GOING=0
  GCODE_AUTH_MATRIX_TIMEOUT=90

Examples:
  scripts/auth_regression_matrix.sh --configured --no-smoke
  scripts/auth_regression_matrix.sh --provider deepseek --provider zai
  GCODE_AUTH_MATRIX_BIN=target/selfdev/gcode scripts/auth_regression_matrix.sh --all
EOF
}
...
if [[ "${GCODE_AUTH_MATRIX_LOGIN:-0}" == "1" ]]; then
  extra_args+=(--login)
fi
if [[ "${GCODE_AUTH_MATRIX_NO_SMOKE:-0}" == "1" ]]; then
  extra_args+=(--no-smoke)
fi
if [[ "${GCODE_AUTH_MATRIX_NO_TOOL_SMOKE:-0}" == "1" ]]; then
  extra_args+=(--no-tool-smoke)
fi
...
run_gcode() {
  if [[ -n "$bin" ]]; then
    ...
    timeout "$per_command_timeout" cargo run --quiet --bin gcode -- "$@"
  fi
}
...
if [[ "$mode" == "configured" ]]; then
  ...
  if ! run_gcode auth-test --all-configured --no-smoke --no-tool-smoke --json --output "$configured_json" >/tmp/gcode-auth-matrix-discovery.out 2>/tmp/gcode-auth-matrix-discovery.err; then
    if [[ -s "$configured_json" ]]; then
      ...
      cat /tmp/gcode-auth-matrix-discovery.err >&2 || true
