...
Compares gcode's swarm (multi-agent coordination) with single-agent performance
...
Environment:
    Requires gcode server running with debug_control enabled:
        touch ~/.gcode/debug_control
        gcode serve
"""
...
def main():
    ...
    if not os.path.exists(DEBUG_SOCKET):
        ...
        print("Make sure gcode server is running with debug_control enabled:")
        print("  touch ~/.gcode/debug_control")
        print("  gcode serve")
        sys.exit(1)
