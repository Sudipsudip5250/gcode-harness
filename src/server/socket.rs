...
pub fn socket_path() -> PathBuf {
    ...
    crate::storage::runtime_dir().join("gcode.sock")
}
...
pub async fn connect_socket(path: &std::path::Path) -> Result<Stream> {
    match Stream::connect(path).await {
        ...
        Err(err) if err.kind() == std::io::ErrorKind::ConnectionRefused && path.exists() => {
            anyhow::bail!(
                "Socket exists but refused the connection at {}. Retry, or remove it after confirming no gcode server is running.",
                path.display()
            )
        }
...
pub(super) fn daemon_lock_path() -> PathBuf {
    crate::storage::runtime_dir().join("gcode-daemon.lock")
}
...
pub(super) fn acquire_daemon_lock() -> Result<DaemonLockGuard> {
    ...
    try_acquire_daemon_lock(&path)?.ok_or_else(|| {
        anyhow::anyhow!(
            "Another gcode server process is already running for runtime dir {}",
...
pub fn set_socket_path(path: &str) {
    crate::env::set_var("GCODE_SOCKET", path);
}
...
/// `GCODE_READY_FD`, and awaits a single byte on the read end. The server
...
pub async fn spawn_server_notify(cmd: &mut std::process::Command) -> Result<std::process::Child> {
    ...
    cmd.env("GCODE_READY_FD", write_fd.to_string());
...
pub(super) fn server_start_matches_existing_server(stderr_output: &str) -> bool {
    stderr_output.contains("Another gcode server process is already running")
...
pub(super) fn format_server_start_error(
    ...
    if stderr_output.trim().is_empty() {
        format!(
            "Server exited before signalling ready ({}). Check logs at ~/.gcode/logs/",
            status
        )
    } else {
        format!(
...
/// Write a single byte to the fd in `GCODE_READY_FD` and close it.
...
pub(super) fn signal_ready_fd() {
    #[cfg(unix)]
    {
        ...
        if let Ok(fd_str) = std::env::var("GCODE_READY_FD") {
            crate::env::remove_var("GCODE_READY_FD");
