...
IN_CONTAINER_HOME = "/tmp/gcode-home"
IN_CONTAINER_RUNTIME = "/tmp/gcode-runtime"
IN_CONTAINER_INPUT = "/tmp/gcode-input"
IN_CONTAINER_OUTPUT = "/tmp/gcode-output"
IN_CONTAINER_BINARY = "/usr/local/bin/gcode"
IN_CONTAINER_LIB_DIR = f"{IN_CONTAINER_RUNTIME}/lib"
IN_CONTAINER_CA_BUNDLE = f"{IN_CONTAINER_HOME}/ca-certificates.crt"
DEFAULT_BINARY_PATH = "/tmp/gcode-compat-dist/gcode-linux-x86_64"
DEFAULT_OPENAI_AUTH_PATH = "~/.gcode/openai-auth.json"
CA_BUNDLE_CANDIDATES = (
    os.environ.get("GCODE_HARBOR_CA_BUNDLE"),
...
OPENSSL_RUNTIME_LIBS = tuple(
    ...
    for lib in (
        _resolve_optional_existing_file(
            candidates=(
                os.environ.get("GCODE_HARBOR_LIBSSL"),
                *_sibling_runtime_lib_candidates(GCODE_BINARY, "libssl"),
                "/usr/lib/libssl.so.3",
        ...
        _resolve_optional_existing_file(
            candidates=(
                os.environ.get("GCODE_HARBOR_LIBCRYPTO"),
                *_sibling_runtime_lib_candidates(GCODE_BINARY, "libcrypto"),
...
def _benchmark_instruction_preamble() -> str:
    # Keep Harbor runs aligned with normal TUI/gcode-run prompting by default.
    # The legacy preamble can still be enabled explicitly for reproducing older
    # runs, but new benchmark runs should rely on gcode's normal system prompt
    # and the official Terminal-Bench task instruction.
    if os.environ.get("GCODE_HARBOR_LEGACY_PREAMBLE"):
        return LEGACY_BENCHMARK_INSTRUCTION_PREAMBLE
    return os.environ.get("GCODE_HARBOR_EXTRA_PREAMBLE", "")


def _load_task_hint() -> str:
    if not os.environ.get("GCODE_HARBOR_ENABLE_HINTS"):
        return ""
    task_name = os.environ.get("GCODE_HARBOR_CURRENT_TASK", "").strip()
    hints_path = os.environ.get("GCODE_HARBOR_TASK_HINTS_FILE", "").strip()
    extra = os.environ.get("GCODE_HARBOR_EXTRA_PREAMBLE", "").strip()
    parts: list[str] = []
    if extra:
        parts.append(extra)
...
class JcodeHarborAgent(BaseAgent):
    def __init__(self, logs_dir: Path, model_name: str | None = None, *args, **kwargs):
        ...
        if "/" in self._model_arg:
            self._provider_arg, self._gcode_model = self._model_arg.split("/", 1)
        else:
            self._provider_arg, self._gcode_model = "openai", self._model_arg
    ...
    async def setup(self, environment: BaseEnvironment) -> None:
        ...
        await environment.upload_file(GCODE_BINARY, IN_CONTAINER_BINARY)
        ...
        version_result = await environment.exec(
            ...
            env={
                ...
                "GCODE_HOME": IN_CONTAINER_HOME,
                "GCODE_RUNTIME_DIR": IN_CONTAINER_RUNTIME,
    ...
    async def run(self, instruction: str, environment: BaseEnvironment, context: AgentContext) -> None:
        ...
        env = {
            ...
            "GCODE_RUNTIME_DIR": IN_CONTAINER_RUNTIME,
            "GCODE_NO_TELEMETRY": "1",
            "GCODE_PROVIDER": self._provider_arg,
            "GCODE_MODEL": self._gcode_model,
            "GCODE_OPENAI_REASONING_EFFORT": os.environ.get("GCODE_OPENAI_REASONING_EFFORT", "high"),
            "GCODE_OPENAI_SERVICE_TIER": os.environ.get("GCODE_OPENAI_SERVICE_TIER", "priority"),
        ...
        result = await environment.exec(
            command=(
                ...
                '--provider "$GCODE_PROVIDER" --model "$GCODE_MODEL" '
        ...
        try:
            await environment.download_dir(IN_CONTAINER_OUTPUT, self.logs_dir / "gcode-output")
        ...
        metadata: dict[str, Any] = {
            ...
            "model": self._gcode_model,
            "gcode_binary": str(GCODE_BINARY),
        }

        output_dir = self.logs_dir / "gcode-output"
