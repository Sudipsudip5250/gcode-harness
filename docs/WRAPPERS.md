# gcode wrapper / scripting guide

This document describes the non-interactive CLI surface intended for wrappers, scripts, and other tools that invoke `gcode`.
...
```bash
gcode --quiet --no-update --no-selfdev ...
```
...
```bash
gcode --quiet model list
gcode --quiet model list --json
gcode --quiet --provider openai model list --json
```
...
```bash
gcode --quiet provider list
gcode --quiet provider list --json
```

Inspect the currently requested and resolved provider/model selection:

```bash
gcode --quiet provider current
gcode --quiet --provider openai --model gpt-5.4 provider current --json
```

Verbose human summary:

```bash
gcode --quiet model list --verbose
```

## Run one prompt and return JSON

```bash
gcode --quiet run --json "Reply with exactly OK"
```

## Stream one prompt as NDJSON

```bash
gcode --quiet run --ndjson "Reply with exactly OK"
```
...
```bash
gcode --quiet auth status
gcode --quiet auth status --json
```
...
- `gcode model list` and `gcode run --json` do not require the TUI
- `gcode model list` does not require an already-running shared server
