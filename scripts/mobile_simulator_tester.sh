...
# Agent-friendly wrapper for the Linux-native gcode mobile simulator.
...
state_dir="${GCODE_MOBILE_TESTER_DIR:-${TMPDIR:-/tmp}/gcode-mobile-tester-${USER:-user}}"
...
Commands:
  ...
  wait [sim args...]        Forward to gcode-mobile-sim wait
  assert-screen <screen>    Assert current screen
  assert-text <text>        Assert text exists in state
  assert-node <args...>     Forward to gcode-mobile-sim assert-node
...
sim() {
  cargo run -q -p gcode-mobile-sim -- "$@"
}
...
case "$cmd" in
  ...
  smoke)
    ...
    "$0" wait --screen chat --contains "Connected to simulated gcode server." >/dev/null
