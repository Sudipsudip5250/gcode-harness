...
case "$OS" in
  Linux)
    case "$ARCH" in
      x86_64)  ARTIFACT="gcode-linux-x86_64" ;;
      aarch64|arm64) ARTIFACT="gcode-linux-aarch64" ;;
      *)       err "Unsupported Linux architecture: $ARCH" ;;
    esac
    ;;
  Darwin)
    case "$ARCH" in
      arm64)   ARTIFACT="gcode-macos-aarch64" ;;
      x86_64)  ARTIFACT="gcode-macos-x86_64" ;;
  ...
  MINGW*|MSYS*|CYGWIN*)
    IS_WINDOWS=true
    case "$ARCH" in
      x86_64|AMD64)  ARTIFACT="gcode-windows-x86_64" ;;
      aarch64|arm64|ARM64) ARTIFACT="gcode-windows-aarch64" ;;
...
if [ "$IS_WINDOWS" = true ]; then
  INSTALL_DIR="${GCODE_INSTALL_DIR:-$LOCALAPPDATA/gcode/bin}"
else
...
if [ "$IS_WINDOWS" = true ]; then
  EXE=".exe"
  builds_dir="$LOCALAPPDATA/gcode/builds"
else
  EXE=""
  builds_dir="$HOME/.gcode/builds"
fi
stable_dir="$builds_dir/stable"
current_dir="$builds_dir/current"
version_dir="$builds_dir/versions"
launcher_path="$INSTALL_DIR/gcode${EXE}"
...
if [ -n "$EXISTING" ]; then
  if echo "$EXISTING" | grep -qF "${VERSION#v}"; then
    info "gcode $VERSION is already installed — reinstalling"
  else
    info "Updating gcode $EXISTING → $VERSION"
  fi
else
  info "Installing gcode $VERSION"
fi
...
download_mode=""
if curl -fsSL "$URL_TGZ" -o "$tmpdir/gcode.download" 2>/dev/null; then
  download_mode="tar"
elif curl -fsSL "$URL_BIN" -o "$tmpdir/gcode.download" 2>/dev/null; then
  download_mode="bin"
fi
...
if [ "$download_mode" = "tar" ]; then
  tar xzf "$tmpdir/gcode.download" -C "$tmpdir"
  ...
  mv "$tmpdir/gcode.download" "$dest_version_dir/$bin_name"
else
...
if [ "$IS_WINDOWS" = true ]; then
  ...
  echo ""
  info "✅ gcode $VERSION installed successfully!"
  echo ""
  if command -v gcode >/dev/null 2>&1; then
    info "Run 'gcode' to get started."
  else
    echo "  To start using gcode right now, run:"
    echo ""
    printf '    \033[1;32mexport PATH="%s:$PATH" && gcode\033[0m\n' "$INSTALL_DIR"
    echo ""
    echo "  To add gcode to PATH permanently (PowerShell):"
    echo ""
  ...
  if ! echo "$PATH" | tr ':' '\n' | grep -qx "$INSTALL_DIR"; then
    ...
    if [ "$(uname -s)" = "Darwin" ] || [ "$SHELL_NAME" = "zsh" ]; then
      ...
      # `ssh host 'gcode --version'`, without depending on .zshrc/.zprofile.
  ...
  if [ "$(uname -s)" = "Darwin" ]; then
    if [ "${mac_hotkey_ready:-false}" = true ]; then
      info "Global hotkey ready: Alt+; opens gcode in your preferred terminal"
    else
      info "Tip: run 'gcode setup-hotkey' to enable Alt+; launch on macOS"
    fi
  fi
  ...
  if command -v gcode >/dev/null 2>&1; then
    ...
    printf '    \033[1;32mexport PATH="%s:\$PATH" && gcode\033[0m\n' "$INSTALL_DIR"
    echo ""
    echo "  Future terminal sessions will have gcode on PATH automatically."
  fi
fi
