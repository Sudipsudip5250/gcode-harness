...
/// Represents a named Anthropic OAuth account stored in gcode's auth.json.
...
/// Multi-account gcode auth.json format.
...
pub fn gcode_path() -> Result<PathBuf> {
    Ok(crate::storage::gcode_dir()?.join("auth.json"))
}

// ---- Multi-account helpers ----

/// Read the gcode auth file, auto-migrating from legacy format if needed.
pub fn load_auth_file() -> Result<JcodeAuthFile> {
    ...
    let mut auth: JcodeAuthFile = crate::storage::read_json(&path)
        .with_context(|| format!("Could not read gcode credentials from {:?}", path))?;
...
/// Write the gcode auth file (multi-account format).
pub fn save_auth_file(auth: &JcodeAuthFile) -> Result<()> {
    let auth_path = gcode_path()?;
...
/// Falls through Claude Code -> gcode accounts -> OpenCode, preferring non-expired tokens.
pub fn load_credentials() -> Result<ClaudeCredentials> {
    ...
    if let Ok(creds) = load_gcode_credentials() {
        if creds.expires_at > now_ms {
            return Ok(creds);
        }
        expired_candidates.push(("gcode", creds));
    }
    ...
    anyhow::bail!("No Claude OAuth credentials found (checked Claude Code, gcode, OpenCode)")
}

/// Load credentials for a specific gcode account by label.
...
/// Load credentials from the active gcode account (multi-account aware).
fn load_gcode_credentials() -> Result<ClaudeCredentials> {
    let auth = load_auth_file()?;
    if auth.anthropic_accounts.is_empty() {
        anyhow::bail!("No anthropic accounts configured in gcode auth.json");
    }
    ...
    let account = auth
        ...
        .context("No anthropic accounts in gcode auth.json")?;
