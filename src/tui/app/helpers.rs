...
pub(super) fn launch_client_executable() -> PathBuf {
    crate::build::client_update_candidate(crate::cli::selfdev::client_selfdev_requested())
        ...
        .unwrap_or_else(|| PathBuf::from("gcode"))
}
...
pub(super) fn debug_cmd_path() -> PathBuf {
    if let Ok(path) = std::env::var("GCODE_DEBUG_CMD_PATH") {
        return PathBuf::from(path);
    }
    std::env::temp_dir().join("gcode_debug_cmd")
}

/// Debug response file path
pub(super) fn debug_response_path() -> PathBuf {
    if let Ok(path) = std::env::var("GCODE_DEBUG_RESPONSE_PATH") {
        return PathBuf::from(path);
    }
    std::env::temp_dir().join("gcode_debug_response")
}
...
/// Spawn a new terminal window that resumes a gcode session.
...
pub(super) fn clipboard_image() -> Option<(String, String)> {
    ...
    #[cfg(target_os = "macos")]
    {
        let temp_path = std::env::temp_dir().join("gcode_clipboard.png");
        let script = format!(
