...
pub fn resume_terminal_candidates() -> Vec<String> {
    ...
    if let Ok(term) = std::env::var("GCODE_TERMINAL") {
...
fn build_spawn_command(term: &str, command: &TerminalCommand, cwd: &Path) -> Option<Command> {
    let title = command.title.as_deref().unwrap_or("gcode");
    ...
    match term {
        ...
        "terminal" => {
            ...
            cmd.args([
                "-a",
                "Terminal",
                command.program.to_str().unwrap_or("gcode"),
                "--args",
            ]);
            cmd.args(&command.args);
        }
...
mod tests {
    ...
    fn shell_command_quotes_arguments() {
        let shell = shell_command(&["gcode".to_string(), "it's ok".to_string()]);
        #[cfg(unix)]
        assert_eq!(shell, "'gcode' 'it'\"'\"'s ok'");
    }
}
