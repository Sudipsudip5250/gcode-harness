...
// These tests run the actual gcode binary and require real credentials.
// Run with: cargo test --test e2e binary_integration -- --ignored
// ============================================================================

/// Test that the gcode binary can run independent with Claude provider
#[tokio::test]
...
/// Test that the gcode binary can run with OpenAI provider
#[tokio::test]
...
/// Test that gcode version command works
#[tokio::test]
async fn binary_version_command() -> Result<()> {
    ...
    assert!(
        stdout.contains("gcode") || stdout.contains("20"),
        "Version should contain 'gcode' or date. Got: {}",
        stdout
    );
...
///
/// Requires a built release binary at target/release/gcode because the reload
...
async fn binary_integration_reload_handoff() -> Result<()> {
    ...
    let release_binary =
        gcode::build::release_binary_path(std::path::Path::new(env!("CARGO_MANIFEST_DIR")));
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_gcode"))
        ...
        .env_remove("GCODE_TEST_SESSION")
        .env("GCODE_HOME", &home_dir)
        .env("GCODE_RUNTIME_DIR", &runtime_dir)
        .env("GCODE_INSTALL_DIR", &install_dir)
        .env("GCODE_DEBUG_CONTROL", "1")
        .env("GCODE_TEMP_SERVER", "1")
        .env("GCODE_SERVER_OWNER_PID", std::process::id().to_string())
        .stdin(Stdio::null())
        .stdout(Stdio::null())
    ...
    let test_result = async {
        ...
        while gcode::server::reload_marker_active(Duration::from_secs(30)) {
...
///
/// Requires a built release binary at target/release/gcode because the
...
async fn binary_integration_selfdev_reload_reconnects_quickly() -> Result<()> {
    ...
    let temp_root = tempfile::Builder::new()
        .prefix("gcode-selfdev-reload-e2e-")
        .tempdir()?;
    let runtime_dir = temp_root.path().join("runtime");
    let home_dir = temp_root.path().join("home");
    let install_dir = temp_root.path().join("install");
    ...
    let _home_guard = EnvVarGuard::set("GCODE_HOME", &home_dir);
    let _runtime_guard = EnvVarGuard::set("GCODE_RUNTIME_DIR", &runtime_dir);
    let _install_guard = EnvVarGuard::set("GCODE_INSTALL_DIR", &install_dir);
...
async fn binary_integration_selfdev_client_reload_resumes_session() -> Result<()> {
    ...
    let temp_root = tempfile::Builder::new()
        .prefix("gcode-selfdev-client-reload-e2e-")
        .tempdir()?;
    let runtime_dir = temp_root.path().join("runtime");
    let home_dir = temp_root.path().join("home");
    let install_dir = temp_root.path().join("install");
    ...
    let socket_path = runtime_dir.join("gcode.sock");
    let debug_socket_path = runtime_dir.join("gcode-debug.sock");
    let starter_binary = temp_root.path().join("gcode-selfdev-client-starter");
    std::fs::copy(env!("CARGO_BIN_EXE_gcode"), &starter_binary)?;
...
async fn binary_integration_selfdev_full_reload_resumes_session_quickly() -> Result<()> {
    ...
    let temp_root = tempfile::Builder::new()
        .prefix("gcode-selfdev-full-reload-e2e-")
        .tempdir()?;
    let runtime_dir = temp_root.path().join("runtime");
    let home_dir = temp_root.path().join("home");
    let install_dir = temp_root.path().join("install");
    ...
    let socket_path = runtime_dir.join("gcode.sock");
    let debug_socket_path = runtime_dir.join("gcode-debug.sock");
    let starter_binary = temp_root.path().join("gcode-selfdev-full-reload-starter");
    std::fs::copy(env!("CARGO_BIN_EXE_gcode"), &starter_binary)?;
    let starter_mtime = std::fs::metadata(&release_binary)?
        .modified()?
        .checked_sub(Duration::from_secs(60))
