...
#
# Defaults:
# - Host: must be provided via GCODE_REMOTE_HOST or --host
# - Remote dir: .cache/remote-builds/gcode/<repo-name> (override with GCODE_REMOTE_DIR or --remote-dir)
#
# Examples:
#   scripts/remote_build.sh --release
#   scripts/remote_build.sh test
#   scripts/remote_build.sh check --all-targets
#   scripts/remote_build.sh --host mybox --remote-dir ~/src/gcode test -- --nocapture
...
Options:
  ...
  --host HOST          Remote SSH host (default: $GCODE_REMOTE_HOST; required if unset)
  --remote-dir DIR     Remote project directory (default: $GCODE_REMOTE_DIR or .cache/remote-builds/gcode/<repo-name>)
...
Behavior:
  ...
  - For build sync-back, copies target/{debug|release}/<artifact> from remote to local
    (artifact defaults to 'gcode', or '--bin <name>' when provided)
EOF
}

LOCAL_DIR="$(cd "$(dirname "$0")/.." && pwd)"
REPO_NAME="$(basename "$LOCAL_DIR")"
REMOTE="${GCODE_REMOTE_HOST:-}"
REMOTE_DIR="${GCODE_REMOTE_DIR:-.cache/remote-builds/gcode/${REPO_NAME}}"
SSH_BIN="${GCODE_REMOTE_SSH_BIN:-ssh}"
RSYNC_BIN="${GCODE_REMOTE_RSYNC_BIN:-rsync}"
...
if [[ -z "$REMOTE" ]]; then
    echo "error: remote host not configured; set GCODE_REMOTE_HOST or pass --host HOST" >&2
    exit 2
fi
...
if [[ "$SYNC_SOURCE" -eq 1 ]]; then
    ...
    "$RSYNC_BIN" -avz "$metadata_file" "$REMOTE:$REMOTE_DIR/.gcode-build-meta"
else
    echo ""
...
printf -v REMOTE_INNER_CMD 'cd %q && env GCODE_BUILD_METADATA_FILE=.gcode-build-meta %s' "$REMOTE_DIR" "$REMOTE_CARGO_CMD"
