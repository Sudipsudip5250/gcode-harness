...
const PROGRESS_MARKER_PREFIX: &str = "GCODE_PROGRESS ";
const CHECKPOINT_MARKER_PREFIX: &str = "GCODE_CHECKPOINT ";
const BACKGROUND_PROGRESS_GUIDANCE: &str = "For long-running background commands, prefer scripts or commands that periodically print progress updates. Best format: print lines starting with `GCODE_PROGRESS ` followed by JSON like {\"percent\":42,\"message\":\"Running\"} or {\"current\":120,\"total\":1000,\"unit\":\"batches\",\"message\":\"Epoch 2/5\",\"eta_seconds\":30}. Supported JSON fields are `percent`, `message`, `current`, `total`, `unit`, `eta_seconds`, and optional `kind`=`indeterminate` or `kind`=`checkpoint`. For milestone-style wakeups, print `GCODE_CHECKPOINT {\"message\":\"Unit tests passed\"}`. Generic fallback output that can be parsed includes `42%`, `3/10 tests`, `3 of 10 steps`, `1.5/3.0 GiB`, or phase lines like `Compiling ...`, `Downloading ...`, `Running ...`, and `Building ...`. If you are writing the script yourself, add these progress/checkpoint lines explicitly.";
const BASH_TOOL_DESCRIPTION: &str = "Run a bash command. For long-running background commands, prefer scripts that emit progress/checkpoint lines. Print `GCODE_PROGRESS {json}` or `GCODE_CHECKPOINT {json}` lines for reliable reporting, or at least output parseable progress like `42%`, `3/10 tests`, `3 of 10 steps`, `1.5/3.0 GiB`, or `Running ...`.";
...
async fn handle_background_output_line(
    ...
    match parse_heuristic_progress(raw_line) {
        ...
        Ok(None) => {}
        Err(err) => {
            let warning = format!("[gcode warning] failed to parse background progress: {err}\n");
...
fn build_detached_shell_wrapper(command: &str) -> StdCommand {
    ...
    cmd.arg("-lc")
        .arg(
            r#"eval "$GCODE_RELOAD_DETACH_COMMAND"; status=$?; printf '\n--- Command finished with exit code: %s ---\n' "$status"; exit "$status""#,
        )
        .env("GCODE_RELOAD_DETACH_COMMAND", command);
    cmd
}
...
impl Tool for BashTool {
    ...
    fn parameters_schema(&self) -> Value {
        let cmd_desc = if cfg!(windows) {
            "The shell command to execute (via cmd.exe). If you write a long-running script or loop for run_in_background=true, make it print progress lines. Preferred format: `GCODE_PROGRESS {json}`."
        } else {
            "The bash command to execute. If you write a long-running script or loop for run_in_background=true, make it print progress lines. Preferred format: `GCODE_PROGRESS {json}`."
        };
        json!({
            "type": "object",
            "required": ["command"],
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        if crate::browser::is_browser_command(&params.command) {
            ...
            // Start/attach a browser session for this gcode session.
