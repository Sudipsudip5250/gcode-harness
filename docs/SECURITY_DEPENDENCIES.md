...
This file tracks the current `cargo audit` findings for gcode and the intended remediation path.
It is not an allowlist. It is a triage record so advisories are visible and actionable.

## Current advisories

| Advisory | Crate | Dependency path | Affected area in gcode | Triage | Planned action |
|---|---|---|---|---|---|
