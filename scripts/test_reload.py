...
def gcode_debug_socket():
    """Find the active gcode debug socket path."""
    runtime_dir = os.environ.get("XDG_RUNTIME_DIR") or f"/run/user/{os.getuid()}"
    return os.path.join(runtime_dir, "gcode-debug.sock")
...
def dbg(command, session_id=None, timeout=TIMEOUT_SECS):
    ...
    return _send_recv(gcode_debug_socket(), req, timeout=timeout)
...
def test_reload_context_roundtrip():
    gcode_dir = pathlib.Path.home() / ".gcode"
    ctx_path = gcode_dir / "reload-context.json"
...
def test_reload_info_file():
    gcode_dir = pathlib.Path.home() / ".gcode"
    info_path = gcode_dir / "reload-info"
...
def test_canary_binary_path():
    ...
    manifest_path = home / ".gcode" / "build-manifest.json"
    ...
    if canary_hash:
        canary_binary = home / ".gcode" / "builds" / "canary" / "gcode"
...
def test_multiple_sessions_responsive():
    ...
    try:
        for i in range(N):
            s = create_session(f"/tmp/gcode-test-{i}")
            sessions.append(s)
...
def check_server_up():
    dbg_sock = gcode_debug_socket()
    if not os.path.exists(dbg_sock):
        raise RuntimeError(
            f"Debug socket not found at {dbg_sock}.\n"
            "  Start gcode server first:\n"
            "    gcode serve   (or just launch gcode in the repo)"
        )
    try:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test gcode selfdev reload mechanism",
    ...
    print("\n╔══════════════════════════════════════════════════════════╗")
    print("║      gcode selfdev reload test suite                    ║")
    print("╚══════════════════════════════════════════════════════════╝\n")
