...
const ALLOW_LEGACY_AUTH_ENV: &str = "GCODE_ALLOW_CODEX_LEGACY_AUTH";
...
fn gcode_auth_path() -> Result<PathBuf> {
    Ok(crate::storage::gcode_dir()?.join("openai-auth.json"))
}
...
fn load_gcode_credentials() -> Result<CodexCredentials> {
    let auth = load_auth_file()?;
    if auth.openai_accounts.is_empty() {
        anyhow::bail!("No OpenAI accounts configured in gcode auth file")
    }
    ...
    let account = auth
        ...
        .context("No OpenAI accounts in gcode auth file")?;
