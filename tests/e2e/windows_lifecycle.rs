...
impl SpawnedWindowsServer {
    fn gcode_binary() -> std::path::PathBuf {
        ...
        std::path::PathBuf::from(env!("CARGO_BIN_EXE_gcode"))
    }
    ...
    fn spawn(prefix: &str) -> Result<Self> {
        ...
        let socket_path = runtime_dir.join("gcode-windows-lifecycle.sock");
        let debug_socket_path = runtime_dir.join("gcode-windows-lifecycle-debug.sock");

        let stdout_file = std::fs::File::create(&stdout_path)?;
        let stderr_file = std::fs::File::create(&stderr_path)?;
        let mut command = Command::new(Self::gcode_binary());
        command
            .arg("--no-update")
            .arg("--socket")
            .arg(&socket_path)
            ...
            .env("GCODE_OPENAI_COMPAT_API_BASE", "http://127.0.0.1:9/v1")
            .env("GCODE_OPENAI_COMPAT_DEFAULT_MODEL", "windows-e2e-model")
            .env("GCODE_OPENAI_COMPAT_LOCAL_ENABLED", "1")
    ...
    fn apply_env<'a>(&self, command: &'a mut Command) -> &'a mut Command {
        command
            .env_remove("GCODE_TEST_SESSION")
            .env("GCODE_HOME", &self.home_dir)
            .env("GCODE_RUNTIME_DIR", &self.runtime_dir)
            .env("GCODE_INSTALL_DIR", &self.install_dir)
            .env("GCODE_NO_TELEMETRY", "1")
            .env("GCODE_OPENAI_COMPAT_API_BASE", "http://127.0.0.1:9/v1")
            .env("GCODE_OPENAI_COMPAT_DEFAULT_MODEL", "windows-e2e-model")
            .env("GCODE_OPENAI_COMPAT_LOCAL_ENABLED", "1")
    ...
    fn dump_logs(&self, label: &str) {
        ...
        if let Ok(artifact_root) = std::env::var("GCODE_E2E_ARTIFACT_DIR") {
            ...
            if let Ok(entries) = std::fs::read_dir(logs_dir) {
                let copied_logs_dir = artifact_dir.join("gcode-logs");
...
async fn windows_binary_server_accepts_clients_and_debug_cli() -> Result<()> {
    ...
    let server = SpawnedWindowsServer::spawn("gcode-windows-lifecycle-")?;
    ...
    let result = async {
        ...
        anyhow::ensure!(
            info.get("debug_control_enabled")
                ...
                == Some(true),
            "server should honor GCODE_DEBUG_CONTROL in Windows e2e"
        );
...
async fn windows_binary_server_rebinds_named_pipe_after_exit() -> Result<()> {
    let _env = setup_test_env()?;
    let mut first = SpawnedWindowsServer::spawn("gcode-windows-rebind-")?;
