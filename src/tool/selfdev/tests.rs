...
fn test_reload_context_save_and_load_for_session_uses_session_scoped_file() {
    ...
    let _home_guard = EnvVarGuard::set("GCODE_HOME", temp_home.path());
...
fn reload_timeout_secs_defaults_to_15() {
    let _storage_guard = crate::storage::lock_test_env();
    let _lock = lock_env();
    let _guard = EnvVarGuard::remove("GCODE_SELFDEV_RELOAD_TIMEOUT_SECS");
...
fn reload_timeout_secs_honors_valid_env_override() {
    let _storage_guard = crate::storage::lock_test_env();
    let _lock = lock_env();
    let _guard = EnvVarGuard::set("GCODE_SELFDEV_RELOAD_TIMEOUT_SECS", "27");
...
fn reload_timeout_secs_ignores_empty_invalid_and_zero_values() {
    let _storage_guard = crate::storage::lock_test_env();
    let _lock = lock_env();
    let _guard = EnvVarGuard::set("GCODE_SELFDEV_RELOAD_TIMEOUT_SECS", "   ");
    assert_eq!(SelfDevTool::reload_timeout_secs(), 15);
    drop(_guard);

    let _guard = EnvVarGuard::set("GCODE_SELFDEV_RELOAD_TIMEOUT_SECS", "abc");
    assert_eq!(SelfDevTool::reload_timeout_secs(), 15);
    drop(_guard);

    let _guard = EnvVarGuard::set("GCODE_SELFDEV_RELOAD_TIMEOUT_SECS", "0");
    assert_eq!(SelfDevTool::reload_timeout_secs(), 15);
}
...
async fn test_action_queues_command_in_test_mode() {
    ...
    let temp_home = tempfile::TempDir::new().expect("temp home");
    let _home_guard = EnvVarGuard::set("GCODE_HOME", temp_home.path());
    let _test_guard = EnvVarGuard::set("GCODE_TEST_SESSION", "1");
    ...
    let output = tool
        .execute(
            json!({
                "action": "test",
                "command": "cargo test -p gcode selfdev_build_command",
    ...
    assert!(
        output
            .output
            .contains("cargo test -p gcode selfdev_build_command")
    );
}
...
async fn enter_creates_selfdev_session_in_test_mode() {
    ...
    let output = tool
        .execute(
            json!({"action": "enter", "prompt": "Work on gcode itself"}),
            ctx,
        )
        .await
...
fn status_output_prunes_stale_pending_requests() {
    ...
    let source = test_source_state(std::path::Path::new("/tmp/gcode"));
    let request = BuildRequest {
        ...
        repo_dir: "/tmp/gcode".to_string(),
        repo_scope: source.repo_scope.clone(),
        worktree_scope: source.worktree_scope.clone(),
        command: "scripts/dev_cargo.sh build --profile selfdev -p gcode --bin gcode".to_string(),
