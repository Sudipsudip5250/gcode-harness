...
pub const GCODE_API_KEY_ENV: &str = "GCODE_API_KEY";
pub const GCODE_API_BASE_ENV: &str = "GCODE_API_BASE";
pub const GCODE_ENV_FILE: &str = "gcode-subscription.env";
pub const GCODE_CACHE_NAMESPACE: &str = "gcode-subscription";
pub const GCODE_SUBSCRIPTION_ACTIVE_ENV: &str = "GCODE_SUBSCRIPTION_ACTIVE";
pub const DEFAULT_GCODE_API_BASE: &str = "https://subscription.gcode.invalid/v1";
...
pub fn routing_policy_detail(model: &CuratedModel) -> String {
    match model.routing_policy {
        UpstreamRoutingPolicy::CacheCapableOnly => {
            "gcode subscription routing · cache-capable upstreams only".to_string()
        }
        UpstreamRoutingPolicy::ProviderAllowlist(providers) => format!(
            "gcode subscription routing · curated upstream: {}",
            providers.join(", ")
        ),
    }
}
...
pub fn configured_api_key() -> Option<String> {
    provider_catalog::load_env_value_from_env_or_config(GCODE_API_KEY_ENV, GCODE_ENV_FILE)
}

pub fn configured_api_base() -> Option<String> {
    provider_catalog::load_env_value_from_env_or_config(GCODE_API_BASE_ENV, GCODE_ENV_FILE)
}
...
pub fn is_runtime_mode_enabled() -> bool {
    std::env::var(GCODE_SUBSCRIPTION_ACTIVE_ENV)
        .ok()
        .map(|value| {
            matches!(
...
pub fn apply_runtime_env() {
    crate::env::set_var(GCODE_SUBSCRIPTION_ACTIVE_ENV, "1");
    crate::env::set_var(
        "GCODE_OPENROUTER_API_BASE",
        configured_api_base().unwrap_or_else(|| DEFAULT_GCODE_API_BASE.to_string()),
    );
    crate::env::set_var("GCODE_OPENROUTER_API_KEY_NAME", GCODE_API_KEY_ENV);
    crate::env::set_var("GCODE_OPENROUTER_ENV_FILE", GCODE_ENV_FILE);
    crate::env::set_var("GCODE_OPENROUTER_CACHE_NAMESPACE", GCODE_CACHE_NAMESPACE);
    crate::env::set_var("GCODE_OPENROUTER_PROVIDER_FEATURES", "0");
    crate::env::remove_var("GCODE_OPENROUTER_ALLOW_NO_AUTH");
    crate::env::remove_var("GCODE_OPENROUTER_PROVIDER");
    crate::env::remove_var("GCODE_OPENROUTER_NO_FALLBACK");
}

pub fn clear_runtime_env() {
    crate::env::remove_var(GCODE_SUBSCRIPTION_ACTIVE_ENV);
    crate::env::remove_var("GCODE_OPENROUTER_API_BASE");
    crate::env::remove_var("GCODE_OPENROUTER_API_KEY_NAME");
    crate::env::remove_var("GCODE_OPENROUTER_ENV_FILE");
    crate::env::remove_var("GCODE_OPENROUTER_CACHE_NAMESPACE");
    crate::env::remove_var("GCODE_OPENROUTER_PROVIDER_FEATURES");
    crate::env::remove_var("GCODE_OPENROUTER_ALLOW_NO_AUTH");
    crate::env::remove_var("GCODE_OPENROUTER_PROVIDER");
    crate::env::remove_var("GCODE_OPENROUTER_NO_FALLBACK");
}
