...
pub use gcode_usage_types::{ErrorCategory, SessionEndReason};
...
const TELEMETRY_ENDPOINT: &str = "https://gcode-telemetry.jeremyhuang55555.workers.dev/v1/event";
...
pub fn is_enabled() -> bool {
    if std::env::var("GCODE_NO_TELEMETRY").is_ok() || std::env::var("DO_NOT_TRACK").is_ok() {
        return false;
    }
    if let Ok(dir) = storage::gcode_dir()
...
fn detect_project_profile() -> ProjectProfile {
    ...
    profile.repo_present = root.join(".git").exists() || crate::build::is_gcode_repo(root);
...
fn show_first_run_notice() {
    eprintln!("\x1b[90m");
    eprintln!("  gcode collects anonymous usage statistics (install count, version, OS,");
    eprintln!("  session activity, tool counts, and crash/exit reasons). No code, filenames,");
    eprintln!("  prompts, or personal data is sent.");
    eprintln!("  To opt out: export GCODE_NO_TELEMETRY=1");
    eprintln!("  Details: https://github.com/bitan-del/gcode-harness/blob/master/TELEMETRY.md");
    eprintln!("\x1b[0m");
}
