...
pub fn auth_failure_recovery_hint(provider_id: &str, reason: AuthFailureReason) -> Option<String> {
    ...
    let hint = match reason {
        ...
        | AuthFailureReason::NonInteractiveTerminal => format!(
            "Try a manual-safe fallback: `gcode login --provider {} --print-auth-url`, then complete with `--callback-url` or `--auth-code`.",
            provider
        ),
        AuthFailureReason::ManualInputMissing => {
        ...
        AuthFailureReason::SaveFailed => {
            "Check whether gcode can write its config directory, or retry inside an isolated sandbox with `bash scripts/onboarding_sandbox.sh fresh`.".to_string()
        }
        AuthFailureReason::PostLoginValidationFailed => format!(
            "Credentials were saved, but runtime verification failed. Run `gcode auth-test --provider {}` and `gcode auth doctor {}` for guided diagnosis.",
            provider, provider
        ),
        AuthFailureReason::ImportUnavailable => {
        ...
        AuthFailureReason::RateLimited => format!(
            "The provider accepted the browser callback but rate-limited the token exchange. Wait before retrying, avoid repeated immediate attempts, and keep using existing credentials if they still validate with `gcode auth doctor {} --validate`.",
            provider
        ),
        AuthFailureReason::OAuthExchangeFailed => format!(
            "Retry the OAuth flow, and if it keeps failing use `gcode login --provider {} --print-auth-url` so the callback can be completed manually.",
            provider
        ),
        AuthFailureReason::Unknown => {
            "Run `gcode auth status`, then `gcode auth doctor` for a structured diagnosis.".to_string()
        }
    };
