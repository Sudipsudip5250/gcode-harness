...
```text
crates/
  gcode-protocol/             # eventually extracted from src/protocol.rs
  gcode-client-core/          # surface-independent client state/reducers/view models
  gcode-desktop-ui/           # custom UI tree, layout, input routing, style tokens
  gcode-desktop-renderer/     # wgpu renderer, display list, glyph/image atlases
  gcode-desktop-platform/     # winit/AppKit/Linux shell abstraction, menus, clipboard
  gcode-desktop/              # product app: windows, panels, protocol client, composition
```

Initial implementation may keep some of these as modules inside `crates/gcode-desktop` to reduce early friction, but the boundaries should be designed as if they were separate crates.
...
```text
gcode-desktop
  -> gcode-desktop-platform
  -> gcode-desktop-renderer
  -> gcode-desktop-ui
  -> gcode-client-core
  -> gcode-protocol
```

`gcode-client-core` must not depend on:
...
`gcode-desktop-ui` should not depend on the Jcode server runtime. It can depend on client-core view models and generic UI types.

`gcode-desktop-renderer` should not know what a Jcode session is. It renders display lists, text runs, images, clips, and primitives.
...
```text
crates/gcode-protocol/src/lib.rs
  Request
  ServerEvent
  HistoryMessage
...
`gcode-desktop` should be product composition, not renderer internals.

Suggested modules:

```text
crates/gcode-desktop/src/
  main.rs
...
`gcode-desktop-ui` is the framework-like internal layer, but it is product-owned and small.

```text
crates/gcode-desktop-ui/src/
  lib.rs
  id.rs
  geometry.rs
  color.rs
...
`gcode-desktop-renderer` should know nothing about Jcode.

```text
crates/gcode-desktop-renderer/src/
  lib.rs
  gpu.rs
  surface.rs
  pipeline.rs
...
```text
crates/gcode-desktop-platform/src/
  lib.rs
  event.rs
  window.rs
  clipboard.rs
...
Once the desktop reducer shape is clear, extract shared pieces from TUI and desktop into `gcode-client-core`:
...
At that point the TUI can gradually become another presentation of `gcode-client-core`, but it does not have to be converted all at once.
...
```text
crates/gcode-desktop-ui       # pure-ish UI/layout model
crates/gcode-desktop-renderer # wgpu display-list renderer
crates/gcode-desktop          # fake-data app shell
```

Then connect to the server protocol.

Only after the desktop reducer/view model shape is proven should shared `gcode-client-core` extraction begin. This avoids prematurely extracting the wrong abstraction from the current TUI.
