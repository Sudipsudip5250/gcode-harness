...
impl LiveOpenAITestEnv {
    fn new() -> Result<Option<Self>> {
        ...
        let gcode_home = EnvVarGuard::set_path("GCODE_HOME", temp.path());
        let transport = EnvVarGuard::set("GCODE_OPENAI_TRANSPORT", "https");

        Ok(Some(Self {
            _lock: lock,
            _gcode_home: gcode_home,
