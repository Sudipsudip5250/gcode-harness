...
```text
packaging/linux/gcode-desktop.desktop
```

It expects a `gcode-desktop` binary to be available on `PATH`. For local testing after installing or copying the binary somewhere your desktop launcher can execute, copy the entry to your user applications directory:

```bash
mkdir -p ~/.local/share/applications
cp packaging/linux/gcode-desktop.desktop ~/.local/share/applications/
