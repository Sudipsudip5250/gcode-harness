use super::*;
pub use gcode_selfdev_types::ReloadRecoveryDirective;
...
impl ReloadContext {
    ...
    pub fn path_for_session(session_id: &str) -> Result<std::path::PathBuf> {
        ...
        Ok(storage::gcode_dir()?.join(format!("reload-context-{}.json", sanitized)))
    }

    fn legacy_path() -> Result<std::path::PathBuf> {
        Ok(storage::gcode_dir()?.join("reload-context.json"))
    }
    ...
    pub(super) async fn do_reload(
        ...
        if !target_binary.exists() {
            return Ok(ToolOutput::new(
                format!(
                    "No binary found at {}.\n\
                     Run 'gcode self-dev --build' first, or build with 'scripts/dev_cargo.sh build --profile selfdev -p gcode --bin gcode' and then try reload again.",
        ...
        let version_before = env!("GCODE_VERSION").to_string();
