...
fn version_display_candidates() -> Vec<String> {
    let full = format!("gcode {}", semver());
    let core = format!("gcode {}", semver_core());
    let minor = format!("gcode {}", semver_minor());
...
mod tests {
    ...
    fn ensure_test_gcode_home_if_unset() {
        static TEST_HOME: OnceLock<std::path::PathBuf> = OnceLock::new();

        if std::env::var_os("GCODE_HOME").is_some() {
            return;
        }

        let path = TEST_HOME.get_or_init(|| {
            let path = std::env::temp_dir().join(format!("gcode-test-home-{}", std::process::id()));
            let _ = std::fs::create_dir_all(&path);
            path
        });
        crate::env::set_var("GCODE_HOME", path);
    }

    fn create_test_app() -> crate::tui::app::App {
        ensure_test_gcode_home_if_unset();
    ...
    fn build_persistent_header_prefers_configured_model_during_remote_connect() {
        ...
        let prev_model = std::env::var_os("GCODE_MODEL");
        let prev_provider = std::env::var_os("GCODE_PROVIDER");
        crate::env::set_var("GCODE_MODEL", "gpt-5.4");
        crate::env::set_var("GCODE_PROVIDER", "openai");
        ...
        if let Some(prev_model) = prev_model {
            crate::env::set_var("GCODE_MODEL", prev_model);
        } else {
            crate::env::remove_var("GCODE_MODEL");
        }
        if let Some(prev_provider) = prev_provider {
            crate::env::set_var("GCODE_PROVIDER", prev_provider);
        } else {
            crate::env::remove_var("GCODE_PROVIDER");
        }
    }
