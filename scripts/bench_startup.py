#!/usr/bin/env python3
"""Benchmark and optionally regression-check gcode startup time.

This script runs isolated startup measurements under a temporary GCODE_HOME and
GCODE_RUNTIME_DIR so it does not interfere with the user's real server, logs, or
credentials.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("binary", nargs="?", default="./target/release/gcode")
...
def isolated_env(root: str) -> dict[str, str]:
    env = os.environ.copy()
    env["GCODE_HOME"] = os.path.join(root, "home")
    env["GCODE_RUNTIME_DIR"] = os.path.join(root, "run")
    env["GCODE_SOCKET"] = os.path.join(env["GCODE_RUNTIME_DIR"], "gcode.sock")
...
def measure_server_startup(binary: str, runs: int) -> list[float]:
    times: list[float] = []
    for _ in range(runs):
        root = tempfile.mkdtemp(prefix="gcode-server-bench-")
        env = isolated_env(root)
        socket_path = env["GCODE_SOCKET"]
        proc = None
        try:
            start = time.perf_counter()
...
def measure_cold_client_startup(binary: str, runs: int) -> list[StartupProfile]:
    ...
    for _ in range(runs):
        root = tempfile.mkdtemp(prefix="gcode-cold-bench-")
        env = isolated_env(root)
        log_path = Path(env["GCODE_HOME"]) / "logs" / f"gcode-{time.strftime('%Y-%m-%d')}.log"
        try:
            command = (
                f"{binary} --no-update --debug-socket "
                f"--socket {env['GCODE_SOCKET']}"
            )
            subprocess.run(
