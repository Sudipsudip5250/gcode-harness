...
artifact="${GCODE_COMPAT_ARTIFACT:-gcode-linux-x86_64}"
profile="${GCODE_COMPAT_PROFILE:-release}"
image="${GCODE_COMPAT_IMAGE:-quay.io/pypa/manylinux2014_x86_64}"
cache_root="${GCODE_COMPAT_CACHE_DIR:-$HOME/.cache/gcode-linux-compat}"
...
docker run --rm \
  ...
  -e GCODE_RELEASE_BUILD="${GCODE_RELEASE_BUILD:-1}" \
  -e GCODE_BUILD_SEMVER="${GCODE_BUILD_SEMVER:-}" \
  -e GCODE_COMPAT_PROFILE="$profile" \
  -e GCODE_COMPAT_TARGET="$target" \
  -e HOST_UID="$host_uid" \
  -e HOST_GID="$host_gid" \
  -v "$repo_root:/work" \
  -v "$out_dir:/out" \
  ...
  bash -lc '
    ...
    if [[ ! -x /root/.cargo/bin/cargo ]]; then
      curl https://sh.rustup.rs -sSf | sh -s -- -y --profile minimal --default-toolchain stable
	    ...
	    cargo build --profile "$GCODE_COMPAT_PROFILE" --target "$GCODE_COMPAT_TARGET" -p gcode --bin gcode

	    cp "$CARGO_TARGET_DIR/$GCODE_COMPAT_TARGET/$GCODE_COMPAT_PROFILE/gcode" "/out/'"$artifact"'.bin"
...
WRAPPER
	    ...
	    # these sibling libraries and sets LD_LIBRARY_PATH for the gcode process.
