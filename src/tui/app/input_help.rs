...
impl App {
    pub(super) fn command_help(&self, topic: &str) -> Option<String> {
        ...
        let help = match topic.as_str() {
            ...
            "restart" => {
                "`/restart`\nRestart gcode with the current binary. Session is preserved.\nUseful after config changes, MCP server updates, or env var changes."
            }
            "rebuild" => {
                "`/rebuild`\nRun `git pull --ff-only`, `cargo build --release`, and release tests in the background. gcode stays usable and reloads automatically when the build is ready."
            }
            "selfdev" => {
                "`/selfdev`\nSpawn a new self-dev gcode session in a separate terminal.\n\n`/selfdev <prompt>`\nSpawn a new self-dev session and auto-deliver the prompt to it.\n\n`/selfdev status`\nShow current self-dev/build status."
            }
            "split" => {
            ...
            "subscription" => {
                "`/subscription`\nShow curated gcode subscription status for this session, including router config, runtime mode, curated models, and planned tier budget scaffolding."
            }
            "version" => "`/version`\nShow gcode version/build details.",
