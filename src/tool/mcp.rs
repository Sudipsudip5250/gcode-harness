    ...
    async fn list_servers(&self) -> Result<ToolOutput> {
        ...
        if servers.is_empty() {
            return Ok(ToolOutput::new(
                ...
                Or add servers to ~/.gcode/mcp.json or .gcode/mcp.json and use {\"action\": \"reload\"}.\n\
    ...
    async fn reload_config(&self, session_id: &str) -> Result<ToolOutput> {
        ...
        if config.servers.is_empty() {
            ...
            return Ok(ToolOutput::new(
                ...
                Add servers to ~/.gcode/mcp.json (global) or .gcode/mcp.json (project):\n\
...
mod tests {
    ...
    impl LocalMcpConfigGuard {
        fn new(content: &str) -> std::io::Result<Self> {
            let path = PathBuf::from(".gcode/mcp.json");
            let dir = path
                .parent()
    ...
    async fn test_reload_empty_config() {
        let _guard =
            LocalMcpConfigGuard::new("{\"servers\":{}}").expect("create temporary .gcode/mcp.json");
        let tool = create_test_tool();
        let ctx = create_test_context();
