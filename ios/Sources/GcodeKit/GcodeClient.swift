...
@MainActor
public protocol GcodeClientDelegate: AnyObject {
...
@MainActor
public extension GcodeClientDelegate {
    func clientDidReplaceText(_ text: String) {}
    func clientDidReceiveToolInput(_ delta: String) {}
    func clientDidUpdateTokens(_ update: TokenUpdate) {}
    func clientDidChangeModel(model: String, provider: String?) {}
...
public actor GcodeClient {
    private let connection: GcodeConnection
    private nonisolated(unsafe) weak var _delegate: (any GcodeClientDelegate)?
    private var serverInfo = ServerInfo()
    private var eventTask: Task<Void, Never>?

    public init(host: String, port: UInt16 = 7643, authToken: String) {
        self.connection = GcodeConnection(host: host, port: port, authToken: authToken)
    }

    public func setDelegate(_ delegate: any GcodeClientDelegate) {
    ...
    private func handleEvent(_ event: GcodeConnection.Event) async {
        switch event {
    ...
    private nonisolated func callDelegate(_ block: @MainActor @Sendable (any GcodeClientDelegate) -> Void) async {
        await MainActor.run {
