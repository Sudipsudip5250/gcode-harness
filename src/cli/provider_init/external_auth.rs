...
pub(super) fn can_prompt_for_external_auth() -> bool {
    std::io::stdin().is_terminal()
        ...
        && std::env::var("GCODE_NON_INTERACTIVE").is_err()
}
...
pub(super) fn external_auth_blocked_message(
    ...
    format!(
        "Found existing {} credentials from {} at {} but gcode will not read them without confirmation. Re-run in an interactive terminal to approve this auth source for future gcode sessions, or run `{}`.",
        provider_name,
        source_name,
        path.display(),
        login_hint
...
pub(super) fn prompt_to_trust_external_auth(
    ...
    eprintln!("gcode will only read that source in place after you approve it.");
    eprintln!("It will not move, delete, or rewrite the original auth there.");
    eprint!("Trust this auth source for future gcode sessions? [y/N]: ");
    io::stdout().flush()?;
...
fn prompt_to_review_external_auth_sources(
    ...
    eprintln!();
    eprintln!("Found existing logins that gcode can reuse.");
    eprintln!("Nothing has been imported yet.");
    eprintln!(
        "Approve the sources you want gcode to read in place; rejected sources stay untouched."
    );
    eprintln!();
