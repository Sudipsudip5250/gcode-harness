...
impl SkillRegistry {
    ...
    /// Only runs if ~/.gcode/skills/ doesn't exist yet.
    fn import_from_external() {
        let gcode_skills = match crate::storage::gcode_dir() {
            Ok(dir) => dir.join("skills"),
            Err(_) => return,
        };

        if gcode_skills.exists() {
        ...
        {
            let count = Self::copy_skills_dir(&claude_skills, &gcode_skills);
            if count > 0 {
                sources.push(format!("{} from Claude Code", count));
                copied.extend(Self::list_skill_names(&gcode_skills));
            }
        }
        ...
        if let Ok(codex_skills) = crate::storage::user_home_path(".codex/skills")
            && codex_skills.is_dir()
        {
            let count = Self::copy_skills_dir(&codex_skills, &gcode_skills);
            if count > 0 {
    ...
    pub fn load_for_working_dir(working_dir: Option<&Path>) -> Result<Self> {
        ...
        // Load from ~/.gcode/skills/ (gcode's own global skills)
        if let Ok(gcode_dir) = crate::storage::gcode_dir() {
            let gcode_skills = gcode_dir.join("skills");
            if gcode_skills.exists() {
                registry.load_from_dir(&gcode_skills)?;
            }
        }
    ...
    fn load_project_local_dirs(&mut self, working_dir: Option<&Path>) -> Result<()> {
        // Load from ./.gcode/skills/ (project-local gcode skills)
        let local_gcode = Self::project_local_dir(working_dir, ".gcode");
        if local_gcode.exists() {
            self.load_from_dir(&local_gcode)?;
        }
    ...
    pub fn reload_all_for_working_dir(&mut self, working_dir: Option<&Path>) -> Result<usize> {
        ...
        if let Ok(gcode_dir) = crate::storage::gcode_dir() {
            let gcode_skills = gcode_dir.join("skills");
            if gcode_skills.exists() {
                count += self.load_from_dir_count(&gcode_skills)?;
            }
        }
        ...
        let local_gcode = Self::project_local_dir(working_dir, ".gcode");
        if local_gcode.exists() {
            count += self.load_from_dir_count(&local_gcode)?;
        }
...
mod tests {
    ...
    #[test]
    fn load_for_working_dir_reads_project_local_gcode_skills() {
        let temp = tempfile::tempdir().expect("tempdir");
        write_test_skill(temp.path(), ".gcode", "wd-only");
    ...
    fn reload_all_for_working_dir_replaces_stale_snapshot_with_session_local_skills() {
        let temp = tempfile::tempdir().expect("tempdir");
        write_test_skill(temp.path(), ".gcode", "session-skill");
