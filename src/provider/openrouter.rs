...
//! - Manual pinning: Set GCODE_OPENROUTER_PROVIDER or use model@Provider syntax
...
pub use gcode_provider_openrouter::{
...
fn configured_api_base() -> String {
    let raw = std::env::var("GCODE_OPENROUTER_API_BASE")
        .ok()
    ...
    normalize_api_base(&raw).unwrap_or_else(|| {
        crate::logging::warn(&format!(
            "Ignoring invalid GCODE_OPENROUTER_API_BASE '{}'; using {}",
...
fn configured_api_key_name() -> String {
    let raw = std::env::var("GCODE_OPENROUTER_API_KEY_NAME")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
    ...
    if is_safe_env_key_name(&raw) {
        ...
        crate::logging::warn(&format!(
            "Ignoring invalid GCODE_OPENROUTER_API_KEY_NAME '{}'; using {}",
...
fn configured_env_file_name() -> String {
    let raw = std::env::var("GCODE_OPENROUTER_ENV_FILE")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
    ...
    if is_safe_env_file_name(&raw) {
        ...
        crate::logging::warn(&format!(
            "Ignoring invalid GCODE_OPENROUTER_ENV_FILE '{}'; using {}",
...
fn provider_features_enabled(api_base: &str) -> bool {
    if let Ok(raw) = std::env::var("GCODE_OPENROUTER_PROVIDER_FEATURES") {
        if let Some(value) = parse_env_bool(&raw) {
            return value;
        }
        crate::logging::warn(&format!(
            "Ignoring invalid GCODE_OPENROUTER_PROVIDER_FEATURES '{}'; expected true/false",
            raw
        ));
    }
...
fn model_catalog_enabled() -> bool {
    if let Ok(raw) = std::env::var("GCODE_OPENROUTER_MODEL_CATALOG") {
        if let Some(value) = parse_env_bool(&raw) {
            return value;
        }
        crate::logging::warn(&format!(
            "Ignoring invalid GCODE_OPENROUTER_MODEL_CATALOG '{}'; expected true/false",
            raw
        ));
    }
    true
...
fn configured_auth_header_mode() -> AuthHeaderMode {
    let Some(raw) = std::env::var("GCODE_OPENROUTER_AUTH_HEADER")
        .ok()
    ...
    match raw.as_str() {
        ...
        other => {
            crate::logging::warn(&format!(
                "Ignoring invalid GCODE_OPENROUTER_AUTH_HEADER '{}'; expected authorization-bearer or api-key",
                other
            ));
...
fn configured_auth_header_name() -> HeaderName {
    let raw = std::env::var("GCODE_OPENROUTER_AUTH_HEADER_NAME")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
    ...
    HeaderName::from_bytes(raw.as_bytes()).unwrap_or_else(|_| {
        crate::logging::warn(&format!(
            "Ignoring invalid GCODE_OPENROUTER_AUTH_HEADER_NAME '{}'; using api-key",
            raw
        ));
...
fn configured_dynamic_bearer_provider() -> Option<String> {
    std::env::var("GCODE_OPENROUTER_DYNAMIC_BEARER_PROVIDER")
        .ok()
        .map(|v| v.trim().to_ascii_lowercase())
        .filter(|v| !v.is_empty())
}
...
impl OpenRouterProvider {
    fn configured_max_tokens(profile_id: Option<&str>) -> Option<u32> {
        if let Ok(raw) = std::env::var("GCODE_OPENROUTER_MAX_TOKENS") {
            ...
            match trimmed.parse::<u32>() {
                ...
                Err(_) => crate::logging::warn(&format!(
                    "Ignoring invalid GCODE_OPENROUTER_MAX_TOKENS '{}'; expected a positive integer or auto",
                    raw
                )),
            }
        }
    ...
    fn is_kimi_model(model: &str) -> bool {
        gcode_provider_openrouter::is_kimi_model(model)
    }
    ...
    fn thinking_override() -> Option<bool> {
        let raw = std::env::var("GCODE_OPENROUTER_THINKING").ok()?;
        ...
        match value.as_str() {
            ...
            other => {
                crate::logging::info(&format!(
                    "Warning: Unsupported GCODE_OPENROUTER_THINKING '{}'; expected enabled/disabled/auto",
                    other
                ));
                None
            }
    ...
    pub fn new() -> Result<Self> {
        ...
        let profile_id = std::env::var("GCODE_OPENROUTER_CACHE_NAMESPACE")
            .ok()
        ...
        let static_context_limits = profile_id
            ...
            .unwrap_or_default();
        let static_models = std::env::var("GCODE_OPENROUTER_STATIC_MODELS")
            .ok()
            .map(|raw| {
                raw.lines()
                    .map(str::trim)
        ...
        if std::env::var_os("GCODE_OPENROUTER_CACHE_NAMESPACE").is_none()
            && let Some(profile) = autodetected_profile.as_ref()
        {
            crate::env::set_var("GCODE_OPENROUTER_CACHE_NAMESPACE", &profile.id);
        }

        let model = std::env::var("GCODE_OPENROUTER_MODEL")
            .ok()
    ...
    fn parse_provider_routing() -> ProviderRouting {
        gcode_provider_openrouter::parse_provider_routing_from_env()
    }
    ...
    fn rank_providers_from_endpoints(endpoints: &[EndpointInfo]) -> Vec<String> {
        gcode_provider_openrouter::rank_providers_from_endpoints(endpoints)
    }
            ...
            return match provider.as_str() {
                "azure" => {
                    if crate::auth::azure::has_configuration() {
                        ...
                        anyhow::bail!(
                            "Azure OpenAI is configured for Entra ID, but Azure settings are incomplete. Run `gcode login --provider azure`."
                        )
                    }
                }
                other => anyhow::bail!(
                    "Unsupported GCODE_OPENROUTER_DYNAMIC_BEARER_PROVIDER '{}'.",
                    other
                ),
            };
        }
