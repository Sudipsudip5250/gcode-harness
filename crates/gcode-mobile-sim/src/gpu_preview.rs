...
use gcode_mobile_core::{UiRect, VisualPrimitive, VisualScene};
...
impl<'window> PreviewCanvas<'window> {
    async fn new(window: &'window Window, scene: VisualScene) -> Result<Self> {
        ...
        let (device, queue) = adapter
            .request_device(
                &wgpu::DeviceDescriptor {
                    label: Some("gcode-mobile-preview-device"),
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("gcode-mobile-preview-shader"),
            source: wgpu::ShaderSource::Wgsl(SHADER.into()),
        });
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("gcode-mobile-preview-pipeline-layout"),
            bind_group_layouts: &[],
            push_constant_ranges: &[],
        });
        let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("gcode-mobile-preview-pipeline"),
    ...
    fn render(&mut self) -> std::result::Result<(), SurfaceError> {
        ...
        let mut encoder = self
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("gcode-mobile-preview-render"),
            });
        let vertices = build_preview_vertices_for_size(&self.scene, self.size);
        let vertex_buffer = self
            .device
            .create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("gcode-mobile-preview-vertices"),
                contents: bytemuck::cast_slice(&vertices),
                usage: wgpu::BufferUsages::VERTEX,
            });
        {
            let mut render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("gcode-mobile-preview-pass"),
...
mod tests {
    use super::*;
    use gcode_mobile_core::{ScenarioName, SimulatorState, SimulatorStore};
