...
//! doesn't expose a usage API. Data persists to ~/.gcode/copilot_usage.json.
...
fn usage_path() -> PathBuf {
    crate::storage::gcode_dir()
        .unwrap_or_else(|_| PathBuf::from(".").join(".gcode"))
        .join("copilot_usage.json")
}

pub use gcode_usage_types::{AllTimeUsage, CopilotUsageTracker, DayUsage, MonthUsage};
...
mod tests {
    ...
    #[test]
    fn usage_path_respects_gcode_home() {
        let _env_lock = lock_env();
        clear_tracker();
        let temp = tempfile::tempdir().expect("tempdir");
        let _home = EnvVarGuard::set("GCODE_HOME", temp.path().as_os_str());

        assert_eq!(usage_path(), temp.path().join("copilot_usage.json"));
    }

    #[test]
    fn save_and_load_roundtrip_under_gcode_home() {
        let _env_lock = lock_env();
        clear_tracker();
        let temp = tempfile::tempdir().expect("tempdir");
        let _home = EnvVarGuard::set("GCODE_HOME", temp.path().as_os_str());
