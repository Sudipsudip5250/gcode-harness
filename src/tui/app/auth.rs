...
impl App {
    ...
    fn record_oauth_preflight(
        ...
        if matches!(callback_available, Some(false)) {
            ...
            if let Some(target) = callback_target {
                notices.push(format!(
                    "Local callback target `{}` is unavailable, so gcode is using manual-safe paste completion instead.",
                    target
                ));
            } else {
                notices.push(
                    "The local callback listener is unavailable, so gcode is using manual-safe paste completion instead."
                        .to_string(),
                );
            }
        }
        if !notices.is_empty() {
            notices.push(format!(
                "If login still fails, run `gcode auth doctor {}` for a guided diagnosis.",
                provider_id
            ));
        }
        notices.join("\n")
    ...
    pub(super) fn show_gcode_subscription_status(&mut self) {
        ...
        let configured_base = crate::subscription_catalog::configured_api_base()
            .unwrap_or_else(|| crate::subscription_catalog::DEFAULT_GCODE_API_BASE.to_string());
        ...
        message.push_str(
            "\nUsage/billing reporting is not live yet; this command is a scaffold for the curated gcode-managed subscription path.",
        );
    ...
    pub(super) fn show_auth_status(&mut self) {
        ...
        message.push_str(
            "\nUse `/login <provider>` to authenticate. `/login gcode` is for curated gcode subscription access; `/account` opens the provider/account management center, `/account <provider> settings` shows provider-specific controls, and `/auth doctor` or `/account <provider> doctor` shows recovery steps.",
        );
        self.push_display_message(DisplayMessage::system(message));
    }
    ...
    pub(super) fn start_login_provider(
        ...
        match provider.target {
            ...
            crate::provider_catalog::LoginProviderTarget::Jcode => self.start_gcode_login(),
            ...
            crate::provider_catalog::LoginProviderTarget::Azure => {
                ...
                self.push_display_message(DisplayMessage::error(
                    "Azure OpenAI login is currently CLI-only. Run `gcode login --provider azure`."
                        .to_string(),
                ));
            }
            ...
            crate::provider_catalog::LoginProviderTarget::Google => {
                ...
                self.push_display_message(DisplayMessage::error(
                    "Google/Gmail login is only available from the CLI right now. Run `gcode login --provider google`."
                        .to_string(),
                ));
            }
        }
    ...
    fn start_gcode_login(&mut self) {
        self.push_display_message(DisplayMessage::system(
            "**Jcode Subscription Login**\n\n\
             This doesn't exist yet.\n\n\
             This would be a gcode subscription for a curated list of models chosen for good compatibility with gcode. It would work similarly to OpenRouter, but gcode would pick the best model/provider routes by balancing price, performance, KV cache support, latency, and throughput. Right now, the model of choice would be DeepSeek V4 Pro.\n\n\
             The goal would be to maximize the amount of token usage you get for your subscription. The plan is to stay around zero profit until gcode can beat raw API prices while providing some level of competitive subsidization. This subscription would be required for the mobile app version.\n\n\
             If you are interested in this, please send feedback letting me know."
                .to_string(),
        ));
        self.set_status_notice("Login: gcode unavailable");
    }
    ...
    fn start_api_key_login(
        ...
        let provider_id = openai_compatible_profile
            ...
            .unwrap_or_else(|| match key_name {
                crate::subscription_catalog::GCODE_API_KEY_ENV => "gcode".to_string(),
    ...
    fn start_cursor_login(&mut self) {
        ...
        self.push_display_message(DisplayMessage::system(
            "**Cursor API Key**\n\n\
             ...
             gcode will save it securely and use the native Cursor HTTPS transport.\n\n\
        ...
        match pending {
            ...
            PendingLogin::ApiKeyProfile {
                ...
                let save_result: anyhow::Result<()> =
                    ...
                    } else if key_name == crate::subscription_catalog::GCODE_API_KEY_ENV {
                        (|| {
                            let mut content = format!("{}={}\n", key_name, key);
                            if let Some(base) = crate::subscription_catalog::configured_api_base() {
                                content.push_str(&format!(
                                    "{}={}\n",
                                    crate::subscription_catalog::GCODE_API_BASE_ENV,
                                    base
                                ));
                            }
                ...
                match save_result {
                    Ok(()) => {
                        ...
                        if let Some(profile) = openai_compatible_profile {
                            ...
                            {
                                crate::env::set_var("GCODE_OPENROUTER_MODEL", default_model);
                            }
                        ...
                        let model_hint = effective_default_model
                            ...
                            .unwrap_or_default();
                        let guidance = if key_name == crate::subscription_catalog::GCODE_API_KEY_ENV
                        {
                            format!(
                                "Use `/login gcode` to access curated models via your router. If the model list looks stale, run `/refresh-model-list`.\nDocs: {}",
                                docs_url
                            )
                    ...
                    Ok(()) => {
                        ...
                        Bus::global().publish(BusEvent::LoginCompleted(LoginCompleted {
                            ...
                            message: "**Cursor API key saved.**\n\n\
                             Stored at `~/.config/gcode/cursor.env`.\n\
                             gcode will use it with the native Cursor HTTPS transport."
                                .to_string(),
                        }));
                    }
                    Err(e) => {
                        ...
                        if let Some(model) = selected {
                            ...
                            crate::bus::Bus::global().publish(crate::bus::BusEvent::LoginCompleted(
                                crate::bus::LoginCompleted {
                                    ...
                                    message:
                                        format!(
                                            "Fetched the model catalog, but it contained no selectable {} models. Run `/refresh-model-list` to retry model discovery, then `gcode auth status` and `gcode auth doctor` for a structured diagnosis.",
                                            provider_label
                                        ),
                                },
                            ));
