...
pub fn show_crash_resume_hint() {
    ...
    if crashed.len() == 1 {
        eprintln!(
            "\x1b[33m💥 Session \x1b[1m{}\x1b[0m\x1b[33m crashed. Resume with:\x1b[0m  gcode --resume {}",
            session_label, id
        );
    } else {
        eprintln!(
        ...
        eprintln!("\x1b[33m   Resume with:\x1b[0m  gcode --resume {}", id);
        eprintln!("\x1b[33m   List all:\x1b[0m     gcode --resume");
    }
    eprintln!();
}
...
fn init_tui_terminal() -> Result<ratatui::DefaultTerminal> {
    if !io::stdin().is_terminal() || !io::stdout().is_terminal() {
        anyhow::bail!("gcode TUI requires an interactive terminal (stdin/stdout must be a TTY)");
    }
    let is_resuming = std::env::var("GCODE_RESUMING").is_ok();
    if is_resuming {
...
pub fn init_tui_runtime() -> Result<(ratatui::DefaultTerminal, TuiRuntimeState)> {
    ...
    crate::tui::mermaid::install_gcode_mermaid_hooks();
    crate::tui::markdown::install_gcode_markdown_hooks();
...
mod tests {
    ...
    fn test_session_recovery_message_format() {
        ...
        if let Some(session_id) = get_current_session() {
            let expected_cmd = format!("gcode --resume {}", session_id);
            assert!(expected_cmd.starts_with("gcode --resume "));
