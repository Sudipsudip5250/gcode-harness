# gcode Mobile App Simulator Foundation
...
The simulator is intended to be a **Linux-native simulator for the gcode mobile
...
- integration with gcode debug/tester tooling
...
`gcode-mobile-core` now emits a serializable `VisualScene` contract:
...
`gcode-mobile-sim` now includes the first non-HTML graphics backend:
...
`gcode-mobile-core::protocol` owns the gateway-facing mobile protocol shapes and
...
- `$GCODE_RUNTIME_DIR/gcode-mobile-sim.sock` if `GCODE_RUNTIME_DIR` is set
- otherwise `$XDG_RUNTIME_DIR/gcode-mobile-sim.sock`
...
The simulator includes a deterministic in-process fake gcode backend for effects
...
This lets agents validate pairing and chat behavior without a real gcode server,
...
```bash
cargo run -p gcode-mobile-sim -- start --scenario onboarding
```
...
The wrapper honors `GCODE_MOBILE_TESTER_DIR` so parallel agents can isolate
simulator state.

### Serve in the foreground

```bash
cargo run -p gcode-mobile-sim -- serve --scenario pairing_ready
```

### Query status

```bash
cargo run -p gcode-mobile-sim -- status
```

### Dump full state

```bash
cargo run -p gcode-mobile-sim -- state
```

### Dump semantic UI tree

```bash
cargo run -p gcode-mobile-sim -- tree
```
...
```bash
cargo run -p gcode-mobile-sim -- scene
cargo run -p gcode-mobile-sim -- scene --output /tmp/mobile-scene.json
...
```bash
cargo run -p gcode-mobile-sim -- preview --scenario connected_chat
...
```bash
cargo run -p gcode-mobile-sim -- preview-mesh --scenario connected_chat
cargo run -p gcode-mobile-sim -- preview-mesh --output /tmp/mobile-preview-mesh.json
...
```bash
cargo run -p gcode-mobile-sim -- render
cargo run -p gcode-mobile-sim -- render --output /tmp/mobile-render.txt
```

### Find and assert semantic UI nodes

```bash
cargo run -p gcode-mobile-sim -- find-node pair.submit
cargo run -p gcode-mobile-sim -- assert-screen onboarding
cargo run -p gcode-mobile-sim -- assert-node pair.submit --enabled true --role button
cargo run -p gcode-mobile-sim -- assert-text "Ready to pair"
cargo run -p gcode-mobile-sim -- assert-no-error
```
...
```bash
cargo run -p gcode-mobile-sim -- log
cargo run -p gcode-mobile-sim -- log --limit 10
```
...
```bash
cargo run -p gcode-mobile-sim -- export-replay --name pairing-ready-chat-send --output crates/gcode-mobile-core/tests/golden/pairing_ready_chat_send.json
cargo run -p gcode-mobile-sim -- assert-replay crates/gcode-mobile-core/tests/golden/pairing_ready_chat_send.json
cargo run -p gcode-mobile-sim -- assert-live-replay crates/gcode-mobile-core/tests/golden/pairing_ready_chat_send.json
```

The checked-in golden trace `crates/gcode-mobile-core/tests/golden/pairing_ready_chat_send.json`
...
```bash
cargo run -p gcode-mobile-sim -- screenshot --output /tmp/mobile-screenshot.json
cargo run -p gcode-mobile-sim -- screenshot --format svg --output /tmp/mobile-screenshot.svg
cargo run -p gcode-mobile-sim -- assert-screenshot /tmp/mobile-screenshot.json
```
...
```bash
cargo run -p gcode-mobile-sim -- set-field host devbox.tailnet.ts.net
cargo run -p gcode-mobile-sim -- set-field pair_code 123456
cargo run -p gcode-mobile-sim -- set-field draft "hello simulator"
```
...
```bash
cargo run -p gcode-mobile-sim -- tap pair.submit
cargo run -p gcode-mobile-sim -- tap chat.send
cargo run -p gcode-mobile-sim -- tap chat.interrupt
```
...
```bash
cargo run -p gcode-mobile-sim -- hit-test 195 354
cargo run -p gcode-mobile-sim -- assert-hit 195 354 pair.submit
cargo run -p gcode-mobile-sim -- tap-at 195 354
```
...
```bash
cargo run -p gcode-mobile-sim -- type-text chat.draft "hello from typing"
cargo run -p gcode-mobile-sim -- keypress Enter --node-id chat.draft
cargo run -p gcode-mobile-sim -- wait --screen chat --contains "Simulated response"
cargo run -p gcode-mobile-sim -- scroll chat.messages 120
cargo run -p gcode-mobile-sim -- gesture swipe_up
cargo run -p gcode-mobile-sim -- inject-fault tool_failed
```
...
```bash
cargo run -p gcode-mobile-sim -- load-scenario connected_chat
```

### Reset to default onboarding state

```bash
cargo run -p gcode-mobile-sim -- reset
```

### Dispatch an action directly as JSON

```bash
cargo run -p gcode-mobile-sim -- dispatch-json '{"type":"set_host","value":"devbox.tailnet.ts.net"}'
```

### Shut down the simulator

```bash
cargo run -p gcode-mobile-sim -- shutdown
```
...
- fake gcode backend that exercises real pairing/WebSocket/protocol flows
...
```bash
cargo run -p gcode-mobile-sim -- start --scenario pairing_ready
cargo run -p gcode-mobile-sim -- state
cargo run -p gcode-mobile-sim -- tap pair.submit
cargo run -p gcode-mobile-sim -- assert-screen chat
cargo run -p gcode-mobile-sim -- set-field draft "hello simulator"
cargo run -p gcode-mobile-sim -- tap chat.send
cargo run -p gcode-mobile-sim -- assert-text "Simulated response to: hello simulator"
cargo run -p gcode-mobile-sim -- assert-no-error
cargo run -p gcode-mobile-sim -- log --limit 10
cargo run -p gcode-mobile-sim -- shutdown
```
