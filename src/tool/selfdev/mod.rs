...
//! Self-development tool - manage canary builds when working on gcode itself
...
impl BuildRequest {
    fn requests_dir() -> Result<PathBuf> {
        let dir = storage::gcode_dir()?.join("selfdev-build-requests");
        ...
        json!({
            ...
            "properties": {
                ...
                "target": {
                    ...
                    "description": "Build target for action=build. auto chooses from changed paths; tui builds gcode; desktop builds gcode-desktop; all builds both."
                },
                "command": {
                    "type": "string",
...
impl SelfDevTool {
    fn is_test_session() -> bool {
        std::env::var("GCODE_TEST_SESSION")
            .map(|value| {
    ...
    fn reload_timeout_secs() -> u64 {
        std::env::var("GCODE_SELFDEV_RELOAD_TIMEOUT_SECS")
            .ok()
    ...
    fn resolve_repo_dir(working_dir: Option<&std::path::Path>) -> Option<std::path::PathBuf> {
        if let Some(dir) = working_dir {
            for ancestor in dir.ancestors() {
                if build::is_gcode_repo(ancestor) {
    ...
    fn launch_binary() -> Result<std::path::PathBuf> {
        build::client_update_candidate(true)
            ...
            .ok_or_else(|| anyhow::anyhow!("Could not resolve gcode executable to launch"))
    }
    ...
    fn build_lock_path(worktree_scope: &str) -> Result<PathBuf> {
        let dir = storage::gcode_dir()?.join("selfdev-build-locks");
        storage::ensure_dir(&dir)?;
