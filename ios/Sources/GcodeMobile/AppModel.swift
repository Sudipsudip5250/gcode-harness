...
final class AppModel: ObservableObject {
    ...
    @Published var selectedServer: ServerCredential? {
        didSet {
            guard let selectedServer else {
                UserDefaults.standard.removeObject(forKey: "gcode.selected.host")
                UserDefaults.standard.removeObject(forKey: "gcode.selected.port")
                return
            }
            UserDefaults.standard.set(selectedServer.host, forKey: "gcode.selected.host")
            UserDefaults.standard.set(Int(selectedServer.port), forKey: "gcode.selected.port")
    ...
    private var client: GcodeClient?
    ...
    private let deviceId: String = {
        if let existing = UserDefaults.standard.string(forKey: "gcode.device.id") {
            return existing
        }
        let generated = "ios-" + UUID().uuidString.lowercased()
        UserDefaults.standard.set(generated, forKey: "gcode.device.id")
        return generated
    }()
    ...
    func loadSavedServers() async {
        ...
        let rememberedHost = UserDefaults.standard.string(forKey: "gcode.selected.host")
        let rememberedPort = UserDefaults.standard.integer(forKey: "gcode.selected.port")
    ...
    func pairAndSave() async {
        ...
        guard !code.isEmpty else {
            errorMessage = "Enter the 6-digit pairing code from gcode pair."
            return
        }
    ...
    func connectSelected() async {
        ...
        let newClient = GcodeClient(host: credential.host, port: credential.port, authToken: credential.authToken)
    ...
    private func applyConnectedServerInfo(_ info: ServerInfo) {
        ...
        serverName = info.serverName ?? "gcode"
...
@MainActor
private final class ClientDelegate: GcodeClientDelegate {
