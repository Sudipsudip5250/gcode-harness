...
(
    GCODE_RELEASE_BUILD=1 GCODE_BUILD_SEMVER="$VERSION_NUM" scripts/build_linux_compat.sh "$DIST" >/dev/null
    echo "  ✅ Linux done ($(( $(date +%s) - OVERALL_START ))s)"
) &
LINUX_PID=$!

(
    GCODE_RELEASE_BUILD=1 GCODE_BUILD_SEMVER="$VERSION_NUM" cargo build --release --target aarch64-apple-darwin 2>/dev/null
    cp target/aarch64-apple-darwin/release/gcode "$DIST/gcode-macos-aarch64"
    chmod +x "$DIST/gcode-macos-aarch64"
    (cd "$DIST" && tar czf gcode-macos-aarch64.tar.gz gcode-macos-aarch64)
...
# Verify binaries
file "$DIST/gcode-linux-x86_64.bin" | grep -q 'ELF 64-bit' || { echo "Error: bad Linux binary"; exit 1; }
head -1 "$DIST/gcode-linux-x86_64" | grep -q '^#!/' || { echo "Error: bad Linux wrapper"; exit 1; }
file "$DIST/gcode-macos-aarch64" | grep -q 'Mach-O 64-bit' || { echo "Error: bad macOS binary"; exit 1; }
...
gh release create "$VERSION" \
    "$DIST/gcode-linux-x86_64.tar.gz" \
    "$DIST/gcode-macos-aarch64.tar.gz" \
    --title "$TITLE" \
    --generate-notes
...
echo ""
echo "Users can now: gcode update"
