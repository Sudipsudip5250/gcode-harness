...
pub fn poll_imap_once(host: &str, port: u16, user: &str, pass: &str) -> Result<Vec<ReplyAction>> {
    ...
    let reply_search = session.search("UNSEEN HEADER In-Reply-To \"@gcode>\"")?;
    let button_search = session.search("UNSEEN SUBJECT \"[gcode-perm:\"")?;
    ...
    for message in messages.iter() {
            ...
            && let Some(parsed) = mail_parser::MessageParser::default().parse(body)
        {
            ...
            let cycle_id = if in_reply_to.contains("@gcode>") {
                in_reply_to
                    .trim_start_matches("<ambient-")
                    .trim_end_matches("@gcode>")
                    .to_string()
            } else if let Some(start) = subject.find("[gcode-perm:") {
                let rest = &subject[start + "[gcode-perm:".len()..];
...
mod tests {
    ...
    fn test_build_permission_email_html() {
        ...
        assert!(html.contains("mailto:gcode@example.com"));
    }
}
