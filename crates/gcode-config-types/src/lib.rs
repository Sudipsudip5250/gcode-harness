...
pub struct AuthConfig {
    /// External auth source ids that the user has approved gcode to read/use.
...
pub struct ProviderConfig {
    ...
    /// Whether gcode should automatically try another account on the same provider
...
pub struct SafetyConfig {
    ...
    /// SMTP password (prefer GCODE_SMTP_PASSWORD env var)
