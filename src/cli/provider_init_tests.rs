...
fn test_provider_choice_arg_values() {
    assert_eq!(ProviderChoice::Jcode.as_arg_value(), "gcode");
...
#[test]
fn test_init_provider_gcode_delegates_runtime_profile_to_wrapper() {
    let _guard = lock_env();
    let _env_guard = crate::storage::lock_test_env();
    crate::subscription_catalog::clear_runtime_env();
    crate::env::remove_var("GCODE_OPENROUTER_MODEL");
    crate::env::remove_var("GCODE_ACTIVE_PROVIDER");
    crate::env::remove_var("GCODE_FORCE_PROVIDER");

    let runtime = tokio::runtime::Runtime::new().expect("tokio runtime");
    let provider = runtime
        .block_on(init_provider(&ProviderChoice::Jcode, None))
        .expect("init gcode provider");
    ...
    assert_eq!(
        std::env::var("GCODE_ACTIVE_PROVIDER").ok().as_deref(),
        Some("openrouter")
    );
    assert_eq!(
        std::env::var("GCODE_FORCE_PROVIDER").ok().as_deref(),
        Some("1")
    );
...
fn choice_for_login_provider_round_trips_core_targets() {
    assert_eq!(
        choice_for_login_provider(provider_catalog::GCODE_LOGIN_PROVIDER),
...
fn resolved_profile_default_model_uses_openai_compatible_override() {
    ...
    crate::env::set_var("GCODE_OPENAI_COMPAT_DEFAULT_MODEL", "llama3.2");
...
async fn init_provider_for_ollama_reapplies_local_compat_runtime_env_after_disabling_subscription_mode()
 {
    ...
    let saved: Vec<(String, Option<String>)> = [
        ...
        "GCODE_FORCE_PROVIDER",
        "GCODE_ACTIVE_PROVIDER",
    ]
    .iter()
    .map(|k| (k.to_string(), std::env::var(k).ok()))
    .collect();

    crate::env::set_var("GCODE_HOME", dir.path());
...
async fn auto_provider_noninteractive_skips_untrusted_external_auth_instead_of_blocking() {
    ...
    crate::env::set_var("GCODE_HOME", dir.path());
    crate::env::set_var("GCODE_NON_INTERACTIVE", "1");
    for key in [
        "ANTHROPIC_API_KEY",
        "OPENAI_API_KEY",
        "OPENROUTER_API_KEY",
