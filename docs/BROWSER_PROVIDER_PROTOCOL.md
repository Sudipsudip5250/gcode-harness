...
Status: draft
Owner: gcode
Audience: gcode core, browser bridge authors, adapter authors

## Why this exists

gcode should expose a single first-class `browser` tool while remaining compatible with multiple browser automation backends:
...
The protocol in this document defines the **normalized contract** between gcode and a browser provider.

This is intentionally **not** a demand that every bridge speak exactly the same native command language. Instead:

- gcode defines a **core semantic layer** it can rely on
...
1. **One first-class tool in gcode**
...
3. **Capability negotiation**
   - gcode should know what each provider can and cannot do.
...
- **browser tool**: the user/model-facing gcode tool.
...
- **browser session**: the provider's isolated session or attachment scope for a gcode session.
...
- direct Rust trait calls inside gcode
...
gcode should not care whether a provider uses tabs, contexts, profiles, or remote targets internally.
It only needs a stable handle it can reuse.

### `session.ensure`

Creates or reuses a browser session for a gcode session.

Request:

```json
{
  "client_session_id": "gcode_session_123",
  "browser_preference": "auto",
  "isolation": "per_gcode_session",
  "attach": "prefer",
  "persist": true,
  "metadata": {
    "owner": "agent",
...
gcode must not assume identifier shape or encode browser semantics into them.

---

## Normalized core methods

These are the semantics gcode can rely on.
...
### Rule 3: gcode core should only rely on normalized methods by default
...
- a gcode-side adapter knows how to use them safely
...
Semantics or qualities that influence gcode behavior:
...
## Recommended gcode integration policy

The gcode `browser` tool should:
...
4. Should `page.snapshot` support multiple named formats beyond `gcode.page_snapshot.v1`?
