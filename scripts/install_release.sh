...
#
# Paths after install:
# - ~/.gcode/builds/versions/<hash>/gcode (immutable)
# - ~/.gcode/builds/stable/gcode -> .../versions/<hash>/gcode
# - ~/.gcode/builds/current/gcode -> .../versions/<hash>/gcode
# - ~/.local/bin/gcode -> ~/.gcode/builds/current/gcode (launcher)
set -euo pipefail

repo_root="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"

profile="${GCODE_RELEASE_PROFILE:-release-lto}"
if [[ "${1:-}" == "--fast" ]]; then
  profile="release"
  shift
fi
...
bin="$repo_root/target/$profile/gcode"
...
# Install versioned binary into ~/.gcode/builds/versions/<hash>/
builds_dir="$HOME/.gcode/builds"
version_dir="$builds_dir/versions/$hash"
mkdir -p "$version_dir"
install -m 755 "$bin" "$version_dir/gcode"

# Update stable symlink
stable_dir="$builds_dir/stable"
mkdir -p "$stable_dir"
ln -sfn "$version_dir/gcode" "$stable_dir/gcode"
...
mkdir -p "$current_dir"
ln -sfn "$version_dir/gcode" "$current_dir/gcode"
printf '%s\n' "$hash" > "$builds_dir/current-version"

# Update launcher path to current channel
install_dir="${GCODE_INSTALL_DIR:-$HOME/.local/bin}"
mkdir -p "$install_dir"
ln -sfn "$current_dir/gcode" "$install_dir/gcode"

echo "Installed: $version_dir/gcode"
echo "Updated stable symlink: $stable_dir/gcode -> $version_dir/gcode"
echo "Updated current symlink: $current_dir/gcode -> $version_dir/gcode"
echo "Updated launcher symlink: $install_dir/gcode -> $current_dir/gcode"
