...
impl App {
    ...
    pub fn debug_socket_path() -> std::path::PathBuf {
        crate::storage::runtime_dir().join("gcode-debug.sock")
    }
}
...
pub(super) fn handle_info_command(app: &mut App, trimmed: &str) -> bool {
    if trimmed == "/version" {
        ...
        app.push_display_message(DisplayMessage {
            ...
            content: format!("gcode {}{}", version, is_canary),
            tool_calls: vec![],
            duration_secs: None,
            title: None,
            tool_data: None,
        ...
        let compaction_summary = if app.provider.supports_compaction() {
            ...
            if let Ok(manager) = manager.try_read() {
                ...
                format!(
                    "- supported: yes\n- mode: {}\n- gcode-managed: {}\n- active summary: {} ({})\n- compacted messages: {}\n- active messages: {}\n- summary chars: {}\n- estimated tokens: {}\n- effective tokens: {}\n- observed tokens: {}\n- usage: {:.1}%\n- compacting now: {}\n- budget: {}",
                    mode,
                    if app.provider.uses_gcode_compaction() {
                        "yes"
                    } else {
                        "no"
                    },
