...
def build_specs() -> dict[str, ToolSpec]:
    gcode = shutil.which("gcode") or str(Path.home() / ".local/bin/gcode")
    ...
    specs = {
        "gcode_memory_off": ToolSpec(
            name="gcode_memory_off",
            argv=[gcode, "--no-update", "--no-selfdev"],
            version_argv=[gcode, "version"],
            env={"GCODE_NO_TELEMETRY": "1", "GCODE_MEMORY_ENABLED": "0"},
            gcode=True,
        ),
        "gcode_memory_on": ToolSpec(
            name="gcode_memory_on",
            argv=[gcode, "--no-update", "--no-selfdev"],
            version_argv=[gcode, "version"],
            env={"GCODE_NO_TELEMETRY": "1", "GCODE_MEMORY_ENABLED": "1"},
            gcode=True,
        ),
        "pi": ToolSpec(
            name="pi",
...
def run_tool(spec: ToolSpec, sessions: int, cwd: Path, timeout_s: float, settle_s: float) -> ToolRunResult:
    ...
    try:
        if spec.gcode:
            temp_root = tempfile.mkdtemp(prefix="gcode-memory-bench-")
            env = os.environ.copy()
            if spec.env:
                env.update(spec.env)
            env["GCODE_HOME"] = os.path.join(temp_root, "home")
            env["GCODE_RUNTIME_DIR"] = os.path.join(temp_root, "run")
            env["GCODE_TEMP_SERVER"] = "1"
            env["GCODE_SERVER_OWNER_PID"] = str(os.getpid())
            os.makedirs(env["GCODE_HOME"], exist_ok=True)
            os.makedirs(env["GCODE_RUNTIME_DIR"], exist_ok=True)
            if spec.name == "gcode_memory_on":
                real_models = Path.home() / ".gcode" / "models"
                bench_models = Path(env["GCODE_HOME"]) / "models"
                if real_models.exists() and not bench_models.exists():
                    bench_models.symlink_to(real_models)
            socket_path = os.path.join(env["GCODE_RUNTIME_DIR"], "bench.sock")
            ...
            if not wait_for_socket(socket_path, timeout_s):
                raise RuntimeError("gcode server did not become ready")
            if spec.name == "gcode_memory_on":
                time.sleep(max(settle_s, 5.0))
            per_session_settle = max(settle_s, 2.0) if spec.name == "gcode_memory_on" else settle_s
