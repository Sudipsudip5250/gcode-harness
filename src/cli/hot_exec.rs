...
pub fn hot_restart(session_id: &str) -> Result<()> {
    ...
    crate::env::set_var("GCODE_RESUMING", "1");
...
pub fn hot_reload(session_id: &str) -> Result<()> {
    ...
    if let Ok(migrate_binary) = std::env::var("GCODE_MIGRATE_BINARY") {
...
pub fn hot_rebuild(session_id: &str) -> Result<()> {
    let cwd = std::env::current_dir()?;
    let repo_dir =
        build::get_repo_dir().ok_or_else(|| anyhow::anyhow!("Could not find gcode repository"))?;

    eprintln!("Rebuilding gcode with session {}...", session_id);
    ...
    std::thread::spawn(move || {
        ...
        let Some(repo_dir) = build::get_repo_dir() else {
            publish(SessionUpdateStatus::Error {
                session_id,
                action,
                message: "Rebuild failed: could not find the gcode repository.".to_string(),
            });
            return;
        };
...
pub fn hot_update(session_id: &str) -> Result<()> {
    ...
    match update::check_for_update_blocking() {
        ...
        Ok(None) => {
            update::print_centered(&format!("Already up to date ({})", env!("GCODE_VERSION")));
        }
        Err(e) => {
...
pub fn run_auto_update() -> Result<()> {
    let repo_dir =
        get_repo_dir().ok_or_else(|| anyhow::anyhow!("Could not find gcode repository"))?;
...
pub fn run_update() -> Result<()> {
    if update::is_release_build() {
        ...
        match update::check_for_update_blocking() {
            Ok(Some(release)) => {
                ...
                update::print_centered("Restart gcode to use the new version.");
            }
            Ok(None) => {
                update::print_centered(&format!("Already up to date ({})", env!("GCODE_VERSION")));
            }
    ...
    update::print_centered(&format!("Updating gcode from {}...", repo_dir.display()));
