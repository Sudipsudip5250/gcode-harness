...
async fn run_auth_doctor_validation(
    ...
    .await
    {
        ...
        Err(_) => format!(
            "validation timed out after {}s; run `gcode auth-test --provider {}` for detailed output",
...
pub(super) fn run_version_command(emit_json: bool) -> Result<()> {
    let report = VersionReport {
        version: env!("GCODE_VERSION").to_string(),
        semver: env!("GCODE_SEMVER").to_string(),
        base_semver: env!("GCODE_BASE_SEMVER").to_string(),
        update_semver: env!("GCODE_UPDATE_SEMVER").to_string(),
        git_hash: env!("GCODE_GIT_HASH").to_string(),
        git_tag: env!("GCODE_GIT_TAG").to_string(),
        build_time: crate::build::current_binary_build_time_string()
            .unwrap_or_else(|| "unknown".to_string()),
        git_date: env!("GCODE_GIT_DATE").to_string(),
        release_build: option_env!("GCODE_RELEASE_BUILD").is_some(),
    };
...
pub(super) async fn run_usage_command(emit_json: bool) -> Result<()> {
    ...
    if report.providers.is_empty() {
        ...
        println!();
        println!("Next steps:");
        println!("- Use `gcode login --provider claude` to connect Claude OAuth.");
        println!("- Use `gcode login --provider openai` to connect ChatGPT / Codex OAuth.");
        return Ok(());
    }
    ...
    if let Some(provider_arg) = provider_arg {
        let provider =
            crate::provider_catalog::resolve_login_provider(provider_arg).ok_or_else(|| {
                anyhow::anyhow!(
                    "Unknown provider '{}'. Use `gcode provider list` to see valid provider ids.",
                    provider_arg
                )
            })?;
