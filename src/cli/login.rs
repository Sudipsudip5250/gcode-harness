    ...
    match choice {
        ProviderChoice::Auto => {
            if options.uses_scriptable_flow()? {
                anyhow::bail!(
                    "Scriptable login flags require an explicit provider. Use `gcode login --provider <provider> ...`."
                );
            }
            ...
            if !io::stdin().is_terminal() {
                anyhow::bail!(
                    "`gcode login --provider auto` requires an interactive terminal. Use `gcode login --provider <provider>` in non-interactive mode."
                );
            }
...
pub async fn run_login_provider(
    ...
    let login_result = if explicit_scriptable_flow {
        ...
        match provider.target {
            ...
            LoginProviderTarget::Jcode => login_gcode_flow().map(|_| LoginFlowOutcome::Completed),
...
/// Best-effort: tell a running gcode server that on-disk auth has changed so it
...
fn login_gcode_flow() -> Result<()> {
    eprintln!("Setting up Jcode subscription access...");
    eprintln!(
        "Paste the gcode subscription API key from your account portal. This key is used for your curated gcode router access.\n"
    );
    ...
    let mut content = format!(
        "{}={}\n",
        crate::subscription_catalog::GCODE_API_KEY_ENV,
        key
    );
    ...
    let file_path = config_dir.join(crate::subscription_catalog::GCODE_ENV_FILE);
    crate::storage::write_text_secret(&file_path, &content)?;

    crate::env::set_var(crate::subscription_catalog::GCODE_API_KEY_ENV, key);
    if !api_base.trim().is_empty() {
        crate::env::set_var(
            crate::subscription_catalog::GCODE_API_BASE_ENV,
            api_base.trim(),
    ...
    crate::telemetry::record_auth_success("gcode", "api_key");
    Ok(())
}
...
async fn login_claude_flow(requested_label: Option<&str>, no_browser: bool) -> Result<()> {
    ...
    eprintln!(
        "Account '{}' stored at {}",
        label,
        auth::claude::gcode_path()?.display()
    );
...
fn login_azure_flow() -> Result<()> {
    ...
    eprintln!(
        "Reference: OpenCode supports Azure OpenAI with Entra credentials. gcode uses Azure OpenAI's newer `/openai/v1` API with either Microsoft Entra ID or an API key.\n"
    );
    ...
    if use_entra {
        ...
        eprintln!(
            "That means gcode can authenticate via `az login`, managed identity, or Azure environment credentials."
        );
    } else {
...
fn login_cursor_flow() -> Result<()> {
    ...
    eprintln!("gcode will use the native Cursor HTTPS transport.");
...
async fn login_antigravity_flow(no_browser: bool) -> Result<()> {
    ...
    eprintln!(
        "gcode will authenticate directly with Google Antigravity; the Antigravity desktop app is not required."
    );
    eprintln!(
        "If browser launch fails, or you pass `--no-browser`, gcode will prompt for the callback URL instead."
    );
    eprintln!(
...
async fn login_gemini_flow(no_browser: bool) -> Result<()> {
    ...
    eprintln!(
        "If browser launch fails, or you pass `--no-browser`, gcode will prompt for the manual authorization code."
    );
    eprintln!(
        ...
        Err(_) => {
            ...
            match input.trim() {
                ...
                "3" => {
                    ...
                    eprintln!("   - Fill in app name (e.g. 'gcode') and your email");
