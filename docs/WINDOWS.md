...
This document describes how gcode achieves cross-platform support for Linux, macOS, and Windows.
...
- Launcher: `%LOCALAPPDATA%\\gcode\\bin\\gcode.exe`
- Stable channel binary: `%LOCALAPPDATA%\\gcode\\builds\\stable\\gcode.exe`
- Immutable versioned binaries: `%LOCALAPPDATA%\\gcode\\builds\\versions\\<version>\\gcode.exe`
...
The transport layer abstracts IPC (Inter-Process Communication). On Unix, gcode uses Unix domain sockets. On Windows, gcode uses named pipes.
...
Socket paths are converted to pipe names: `/run/user/1000/gcode.sock` becomes `\\.\pipe\gcode`.
