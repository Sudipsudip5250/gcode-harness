...
#[test]
fn paused_gcode_shell_command_keeps_failures_visible() {
    let command = paused_gcode_shell_command("/tmp/gcode");
    assert!(command.contains("Press Enter to close"));
    assert!(command.contains("Jcode exited with status"));
    assert!(command.contains("gcode executable not found"));
}
