...
This document describes a proposed evolution of gcode's UI architecture from the
...
Today, gcode is effectively organized like this:
...
- a independent gcode window attached to one session
...
- current independent gcode window
...
2. Client spawns a independent gcode client attached to the same session.
...
This can initially be provided through existing gcode control channels such as:
...
- Preserve current `gcode --resume <session>` workflow.
...
- Should workspace mode be a new command (`gcode workspace`) or a runtime mode of
  the normal client?
...
This gives gcode a portable built-in multi-session workspace without sacrificing
