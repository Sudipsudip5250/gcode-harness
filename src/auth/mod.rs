...
/// CLI tools don't get installed/uninstalled while gcode is running, so caching
...
pub fn browser_suppressed(cli_no_browser: bool) -> bool {
    cli_no_browser || env_truthy("NO_BROWSER") || env_truthy("GCODE_NO_BROWSER")
}
...
impl AuthStatus {
    ...
    pub fn has_any_available(&self) -> bool {
        self.anthropic.state == AuthState::Available
            || self.gcode == AuthState::Available
    ...
    pub fn state_for_key(&self, key: LoginProviderAuthStateKey) -> AuthState {
        match key {
            ...
            LoginProviderAuthStateKey::Jcode => self.gcode,
        ...
        let (
            ...
            crate::provider_catalog::LoginProviderTarget::Jcode => {
                let (source, detail) = summarize_sources(vec![
                    env_source(crate::subscription_catalog::GCODE_API_KEY_ENV),
                    config_source(
                        crate::subscription_catalog::GCODE_API_KEY_ENV,
                        crate::subscription_catalog::GCODE_ENV_FILE,
                        "~/.config/gcode/gcode-subscription.env",
                    ),
                ]);
                (
                    source,
            ...
            crate::provider_catalog::LoginProviderTarget::OpenAiApiKey => {
                let (source, detail) = summarize_sources(vec![
                    ...
                    config_source("OPENAI_API_KEY", "openai.env", "~/.config/gcode/openai.env"),
            ...
            crate::provider_catalog::LoginProviderTarget::OpenAiCompatible(profile) => {
                ...
                let (source, detail) = summarize_sources(vec![
                    ...
                    config_source(
                        &resolved.api_key_env,
                        &resolved.env_file,
                        format!("~/.config/gcode/{}", resolved.env_file),
                    ),
    ...
    fn check_uncached() -> Self {
        ...
        if crate::subscription_catalog::has_credentials() {
            status.gcode = AuthState::Available;
        }
    ...
    fn check_uncached_fast() -> Self {
        ...
        timings.push(("gcode", step_start.elapsed().as_millis()));
...
fn openai_oauth_source(status: &AuthStatus) -> Option<(AuthCredentialSource, String)> {
        ...
        .is_empty()
    {
        return Some((
            AuthCredentialSource::JcodeManagedFile,
            "~/.gcode/openai-auth.json".to_string(),
        ));
    }
...
fn cursor_source() -> Option<(AuthCredentialSource, String)> {
    ...
    if config_source("CURSOR_API_KEY", "cursor.env", "~/.config/gcode/cursor.env").is_some() {
        return config_source("CURSOR_API_KEY", "cursor.env", "~/.config/gcode/cursor.env");
    }
    None
}
