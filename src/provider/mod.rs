...
#[cfg(test)]
use gcode_provider_core::FailoverDecision;
use std::sync::{Arc, RwLock};

pub use gcode_provider_core::{
...
pub(crate) use gcode_provider_core::{ProviderFailoverPrompt, parse_failover_prompt_message};
...
impl MultiProvider {
        ...
        match provider {
            ActiveProvider::Claude => {
                ...
                if let Some(anthropic) = self.anthropic_provider() {
                    ...
                    anyhow::bail!(
                        "Claude credentials not available. Run `gcode login --provider claude` first."
                    );
                }
            ...
            ActiveProvider::OpenAI => {
                let Some(openai) = self.openai_provider() else {
                    anyhow::bail!(
                        "OpenAI credentials not available. Run `gcode login --provider openai` first."
                    );
                };
            ...
            ActiveProvider::Copilot => {
                let Some(copilot) = self.copilot_provider() else {
                    anyhow::bail!(
                        "GitHub Copilot credentials not available. Run `gcode login --provider copilot` first."
                    );
                };
            ...
            ActiveProvider::Antigravity => {
                let Some(antigravity) = self.antigravity_provider() else {
                    anyhow::bail!(
                        "Antigravity credentials not available. Run `gcode login --provider antigravity` first."
                    );
                };
            ...
            ActiveProvider::Gemini => {
                let Some(gemini) = self.gemini_provider() else {
                    anyhow::bail!(
                        "Gemini credentials not available. Run `gcode login --provider gemini` first."
                    );
                };
            ...
            ActiveProvider::Cursor => {
                let Some(cursor) = self.cursor_provider() else {
                    anyhow::bail!(
                        "Cursor credentials not available. Run `gcode login --provider cursor` first."
                    );
                };
            ...
            ActiveProvider::OpenRouter => {
                let Some(openrouter) = self.openrouter_provider() else {
                    anyhow::bail!(
                        "OpenRouter/OpenAI-compatible credentials not available. Set the configured API key or run `gcode login --provider openrouter` first."
                    );
                };
    ...
    fn set_model_on_openai_compatible_profile(
        ...
        if !crate::provider_catalog::openai_compatible_profile_is_configured(profile) {
            anyhow::bail!(
                "{} credentials not available. Run `gcode login --provider {}` first.",
...
impl Provider for MultiProvider {
    ...
    fn model_routes(&self) -> Vec<ModelRoute> {
        ...
        if total_ms >= 250 || std::env::var("GCODE_LOG_MODEL_PICKER_TIMING").is_ok() {
            crate::logging::info(&format!(
    ...
    fn handles_tools_internally(&self) -> bool {
        match self.active_provider() {
            ActiveProvider::Claude => {
                // Direct API does NOT handle tools internally - gcode executes them
            ...
            ActiveProvider::Cursor => self
                ...
                .map(|o| o.handles_tools_internally())
                .unwrap_or(false),
            ActiveProvider::OpenRouter => false, // gcode executes tools
        }
    }
    ...
    fn uses_gcode_compaction(&self) -> bool {
        match self.active_provider() {
            ActiveProvider::Claude => {
                if self.anthropic_provider().is_some() {
                    true
                    ...
                    self.claude_provider()
                        .map(|c| c.uses_gcode_compaction())
                        .unwrap_or(false)
                }
            }
            ActiveProvider::OpenAI => self
                .openai_provider()
                .map(|o| o.uses_gcode_compaction())
                .unwrap_or(false),
            ActiveProvider::Copilot => self
                .copilot_provider()
                .map(|o| o.uses_gcode_compaction())
