...
If you want to iterate on onboarding repeatedly without touching your real auth state, use a separate sandbox rooted under `GCODE_HOME` and `GCODE_RUNTIME_DIR`.

This repo already supports that isolation:

- `GCODE_HOME` redirects gcode-owned state such as `~/.gcode` into a sandbox directory.
- `GCODE_HOME` also redirects app config into `GCODE_HOME/config/gcode`.
- `GCODE_RUNTIME_DIR` redirects sockets and other ephemeral runtime files.
...
That gives you a clean gcode launch with isolated state.
...
# Log into a provider without touching your normal gcode config
...
# Run arbitrary gcode commands in the sandbox
scripts/onboarding_sandbox.sh gcode auth status
scripts/onboarding_sandbox.sh gcode pair
```
...
- no real gcode config files are reused
...
This sandbox is designed to isolate gcode-owned state and trusted external-import state. If you later decide to test explicit import/reuse flows from external tools, do that intentionally and treat it as a separate test case from first-run onboarding.
