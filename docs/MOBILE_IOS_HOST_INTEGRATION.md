...
1. **Linux simulator first**
   ...
   - A new app flow should land in `gcode-mobile-core` and `gcode-mobile-sim` before relying on device testing.
...
```c
void *gcode_mobile_app_new(const char *initial_scenario_json);
void gcode_mobile_app_free(void *app);
```

### Dispatch and inspect

```c
char *gcode_mobile_dispatch(void *app, const char *action_json);
char *gcode_mobile_state(void *app);
char *gcode_mobile_tree(void *app);
char *gcode_mobile_logs(void *app, uint32_t limit);
void gcode_mobile_string_free(char *ptr);
```

### Platform events

```c
char *gcode_mobile_platform_event(void *app, const char *event_json);
```
...
{ "type": "pair_finished", "ok": true, "token": "...", "server_name": "gcode" }
{ "type": "websocket_event", "event": { "type": "text_delta", "text": "hello" } }
{ "type": "qr_payload_scanned", "payload": "gcode://pair?..." }
...
cargo build -p gcode-mobile-ffi --target aarch64-apple-ios --release
cargo build -p gcode-mobile-ffi --target aarch64-apple-ios-sim --release
```
...
- `gcode-mobile-core` reducer/protocol test
- `gcode-mobile-sim` automation test
