...
#[test]
fn gcode_auth_file_default_is_empty() {
...
#[test]
fn gcode_path_respects_gcode_home() {
    let _lock = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set("GCODE_HOME", temp.path());

    assert_eq!(gcode_path().unwrap(), temp.path().join("auth.json"));
    assert_eq!(
...
#[test]
fn gcode_auth_file_multi_account() {
    let auth = JcodeAuthFile {
...
#[test]
fn gcode_auth_file_legacy_migration_format() {
    let legacy_json = r#"{
        "anthropic": {
            "access": "legacy_acc",
            "refresh": "legacy_ref",
