...
///
/// Can be overridden with `$GCODE_RUNTIME_DIR`.
...
fn fallback_runtime_dir() -> PathBuf {
    std::env::temp_dir().join(format!("gcode-{}", runtime_user_discriminator()))
}
...
fn ensure_private_runtime_dir(path: &Path) {
    ...
    #[cfg(unix)]
    {
        let _ = gcode_core::fs::set_directory_permissions_owner_only(path);
    }
}

pub fn gcode_dir() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("GCODE_HOME") {
...
pub fn logs_dir() -> Result<PathBuf> {
    Ok(gcode_dir()?.join("logs"))
}

/// Resolve gcode's app-owned config directory.
///
/// Default location is the platform config dir + `gcode` (for example
/// `~/.config/gcode` on Linux). When `GCODE_HOME` is set, sandbox this under
/// `$GCODE_HOME/config/gcode` so self-dev/tests do not leak into the user's
/// real config directory.
pub fn app_config_dir() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("GCODE_HOME") {
        return Ok(PathBuf::from(path).join("config").join("gcode"));
    }

    let config_dir =
        dirs::config_dir().ok_or_else(|| anyhow::anyhow!("No config directory found"))?;
    Ok(config_dir.join("gcode"))
}

/// Resolve a path under the user's home directory, but sandbox it under
/// `$GCODE_HOME/external/` when `GCODE_HOME` is set.
///
...
pub fn harden_user_config_permissions() {
    if let Some(config_dir) = dirs::config_dir() {
        let gcode_config_dir = config_dir.join("gcode");
        if gcode_config_dir.exists() {
            let _ = gcode_core::fs::set_directory_permissions_owner_only(&gcode_config_dir);
        }
    }

    if let Ok(gcode_home) = gcode_dir()
        && gcode_home.exists()
    {
        let _ = gcode_core::fs::set_directory_permissions_owner_only(&gcode_home);
    }
}
...
pub fn harden_secret_file_permissions(path: &Path) {
    if let Some(parent) = path.parent() {
        let _ = gcode_core::fs::set_directory_permissions_owner_only(parent);
    }
    if path.exists() {
        let _ = gcode_core::fs::set_permissions_owner_only(path);
    }
}

/// Validate an external auth file managed by another tool before reading it.
///
/// gcode intentionally avoids mutating these files. We also reject obvious risky
...
pub fn ensure_dir(path: &Path) -> Result<()> {
    if !path.exists() {
        ...
        gcode_core::fs::set_directory_permissions_owner_only(path)?;
    }
    Ok(())
}
...
pub fn write_text_secret(path: &Path, content: &str) -> Result<()> {
    ...
    if let Some(parent) = path.parent() {
        gcode_core::fs::set_directory_permissions_owner_only(parent)?;
    }
    gcode_core::fs::set_permissions_owner_only(path)?;
    Ok(())
}
