...
fn with_temp_gcode_home<T>(f: impl FnOnce() -> T) -> T {
    let _guard = test_env_lock();
    let temp_home = tempfile::tempdir().expect("tempdir");
    let prev_home = std::env::var_os("GCODE_HOME");
    gcode_core::env::set_var("GCODE_HOME", temp_home.path());
    let result = f();
    if let Some(prev_home) = prev_home {
        gcode_core::env::set_var("GCODE_HOME", prev_home);
    } else {
        gcode_core::env::remove_var("GCODE_HOME");
    }
    result
}
...
fn test_binary_version_hash_mismatch_rejects_publish_candidate() {
    ...
    let error = validate_binary_version_matches_source_report(&report, Path::new("gcode"), &source)
...
fn test_find_repo_in_ancestors_walks_upward() {
    let temp = tempfile::tempdir().expect("tempdir");
    let repo = temp.path().join("gcode-repo");
...
#[test]
fn launcher_dir_uses_sandbox_bin_when_gcode_home_is_set() {
    with_temp_gcode_home(|| {
        let launcher_dir = launcher_dir().expect("launcher dir");
        let expected = storage::gcode_dir().expect("gcode dir").join("bin");
...
fn update_launcher_symlink_stays_inside_sandbox_home() {
    with_temp_gcode_home(|| {
        ...
        let expected_launcher = storage::gcode_dir()
            .expect("gcode dir")
            .join("bin")
            .join(binary_name());
