#!/bin/bash
# End-to-end test script for gcode
...
echo "=== E2E Testing Script for gcode ==="
echo ""

# Test 1: Check binary exists and runs
echo "Test 1: Check gcode binary..."
if command -v gcode &> /dev/null; then
    echo "✓ gcode binary found"
    gcode --version
else
    echo "✗ gcode binary not found"
    exit 1
fi
...
if [[ "${GCODE_REAL_PROVIDER:-0}" == "1" ]]; then
    echo ""
    echo "Test 7: Real provider smoke (GCODE_REAL_PROVIDER=1)..."
    scripts/real_provider_smoke.sh
    echo "✓ Real provider smoke passed"
fi

if [[ "${GCODE_REAL_AUTH_TEST:-0}" == "1" ]]; then
    echo ""
    echo "Test 8: Auth E2E validation (GCODE_REAL_AUTH_TEST=1)..."
...
echo "  gcode        # Start TUI mode"
echo "  gcode server # Start server mode"
echo "  gcode client # Connect to server"
