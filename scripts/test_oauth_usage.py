#!/usr/bin/env python3
"""
Test OAuth usage comparison between Claude Code CLI and gcode direct API.

This script:
1. Shells out to Claude Code CLI with a simple prompt
2. Uses gcode's debug socket to send the same prompt via direct OAuth
...
DEBUG_SOCKET = f"/run/user/{os.getuid()}/gcode-debug.sock"
MAIN_SOCKET = f"/run/user/{os.getuid()}/gcode.sock"
...
def run_gcode_oauth(prompt: str) -> dict:
    """Run via gcode debug socket using direct OAuth."""
    print(f"\n{'='*60}")
    print("Testing gcode direct OAuth API...")
    print(f"{'='*60}")
...
def main():
    ...
    # Test gcode OAuth
    gcode_result = run_gcode_oauth(TEST_PROMPT)

    # Check quota after gcode test
    time.sleep(1)  # Wait for API to update
    usage_after_gcode = get_oauth_usage()
    five_hour_after_gcode = usage_after_gcode.get('five_hour', {}).get('utilization', 0)
    gcode_quota_delta = five_hour_after_gcode - five_hour_after_cli
    print(f"\nQuota after gcode: {five_hour_after_gcode:.2f}% (delta: +{gcode_quota_delta:.4f}%)")
    ...
    if "error" in gcode_result:
        print(f"  Error: {gcode_result['error']}")
    else:
        print(f"  Time: {gcode_result.get('time', 'N/A'):.2f}s")
        usage = gcode_result.get('usage', {})
        if usage:
            print(f"  Input tokens: {usage.get('input_tokens', 'N/A')}")
            print(f"  Output tokens: {usage.get('output_tokens', 'N/A')}")
            print(f"  Cache read: {usage.get('cache_read_input_tokens', 0) or 0}")
    ...
    gcode_usage = gcode_result.get('usage', {})
    ...
    gcode_total = (gcode_usage.get('input_tokens', 0) or 0) + \
                  (gcode_usage.get('cache_creation_input_tokens', 0) or 0) + \
                  (gcode_usage.get('cache_read_input_tokens', 0) or 0) + \
                  (gcode_usage.get('output_tokens', 0) or 0)

    cli_time = cli_result.get('time', 0)
    gcode_time = gcode_result.get('time', 0)
    speedup = cli_time / gcode_time if gcode_time > 0 else 0
    token_savings = 100 * (1 - gcode_total / cli_total) if cli_total > 0 else 0
...
PERFORMANCE COMPARISON:
                    Claude CLI      gcode
  Response time:    {cli_time:.2f}s           {gcode_time:.2f}s ({speedup:.1f}x faster)
  Total tokens:     {cli_total:,}         {gcode_total:,} ({token_savings:.0f}% fewer)
...
ACTUAL QUOTA CONSUMPTION (from OAuth API):
  ...
  After gcode:      {five_hour_after_gcode:.2f}%  (+{gcode_quota_delta:.4f}%)
