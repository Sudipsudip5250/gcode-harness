...
pub fn login_provider_for_choice(choice: &ProviderChoice) -> Option<LoginProviderDescriptor> {
    match choice {
        ProviderChoice::Jcode => Some(crate::provider_catalog::GCODE_LOGIN_PROVIDER),
...
pub fn prompt_login_provider_selection(
    ...
    prompt_login_provider_selection_optional(providers, heading)?.ok_or_else(|| {
        anyhow::anyhow!("Login skipped. Run `gcode login` when you're ready to authenticate.")
    })
}
...
fn provider_login_hint_for_api_key_env(env_key: &str) -> String {
    if env_key == "OPENROUTER_API_KEY" {
        return "gcode login --provider openrouter".to_string();
    }
    ...
    crate::provider_catalog::openai_compatible_profiles()
        ...
        .find_map(|profile| {
            ...
            (resolved.api_key_env == env_key)
                .then(|| format!("gcode login --provider {}", resolved.id))
        })
        .unwrap_or_else(|| "gcode login".to_string())
}
...
fn ensure_external_api_key_auth_allowed_for_explicit_choice(env_key: &str) -> Result<()> {
    ...
    anyhow::bail!(
        "Skipped trusting external {} credentials. Run `{}` to authenticate gcode directly.",
        provider_name,
        login_hint
    )
}
...
fn ensure_openai_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    anyhow::bail!(
        "Skipped trusting existing ~/.codex/auth.json credentials. Run `gcode login --provider openai` to authenticate gcode directly."
    )
}
...
fn ensure_claude_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    anyhow::bail!(
        "Skipped trusting external Claude credentials. Run `gcode login --provider claude` to authenticate gcode directly."
    )
}
...
fn ensure_gemini_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    anyhow::bail!(
        "Skipped trusting Gemini CLI credentials. Run `gcode login --provider gemini` to authenticate gcode directly."
    )
}
...
fn ensure_antigravity_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    if maybe_prompt_for_generic_oauth_source(
        ...
        "gcode login --provider antigravity",
        false,
...
fn ensure_copilot_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    anyhow::bail!(
        "Skipped trusting external Copilot credentials. Run `gcode login --provider copilot` to authenticate gcode directly."
    )
}
...
fn ensure_cursor_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    anyhow::bail!(
        "Skipped trusting external Cursor credentials. Run `gcode login --provider cursor` to authenticate gcode directly."
    )
}
...
pub fn lock_model_provider(provider_key: &str) {
    crate::env::set_var("GCODE_ACTIVE_PROVIDER", provider_key);
    crate::env::set_var("GCODE_FORCE_PROVIDER", "1");
}

pub fn unlock_model_provider() {
    crate::env::remove_var("GCODE_FORCE_PROVIDER");
}
...
pub async fn login_and_bootstrap_provider(
    ...
    let runtime: Arc<dyn provider::Provider> = match provider.target {
        ...
        LoginProviderTarget::Jcode => Arc::new(provider::gcode::JcodeProvider::new()),
        ...
        LoginProviderTarget::Cursor => {
            ...
            crate::env::set_var("GCODE_ACTIVE_PROVIDER", "cursor");
        ...
        LoginProviderTarget::Gemini => {
            disable_subscription_runtime_mode();
            unlock_model_provider();
            crate::env::set_var("GCODE_ACTIVE_PROVIDER", "gemini");
            Arc::new(provider::gemini::GeminiProvider::new())
        }
        LoginProviderTarget::Antigravity => {
            disable_subscription_runtime_mode();
            unlock_model_provider();
            crate::env::set_var("GCODE_ACTIVE_PROVIDER", "antigravity");
...
async fn init_provider_with_options(
    ...
    if let Ok(profile_name) = std::env::var("GCODE_PROVIDER_PROFILE_NAME")
        && !profile_name.trim().is_empty()
    {
        crate::provider_catalog::apply_named_provider_profile_env(profile_name.trim())?;
        crate::env::set_var("GCODE_PROVIDER_PROFILE_ACTIVE", "1");
    }

    if std::env::var_os("GCODE_PROVIDER_PROFILE_ACTIVE").is_none()
        && std::env::var_os("GCODE_NAMED_PROVIDER_PROFILE").is_none()
    {
    ...
    let provider: Arc<dyn provider::Provider> = match choice {
        ProviderChoice::Jcode => {
            ...
            Arc::new(provider::gcode::JcodeProvider::new())
        }
        ...
        ProviderChoice::ClaudeSubprocess => {
            ...
            crate::env::set_var("GCODE_USE_CLAUDE_CLI", "1");
            init_notice(
        ...
        | ProviderChoice::OpenaiCompatible => {
            ...
            let display_name = if let Ok(named) = std::env::var("GCODE_NAMED_PROVIDER_PROFILE") {
                named
            } else {
                let resolved = resolve_openai_compatible_profile(profile);
            ...
            lock_model_provider("openrouter");
            if std::env::var_os("GCODE_PROVIDER_PROFILE_ACTIVE").is_some()
                || std::env::var_os("GCODE_NAMED_PROVIDER_PROFILE").is_some()
            {
                let profile_name = std::env::var("GCODE_NAMED_PROVIDER_PROFILE")?;
        ...
        ProviderChoice::Google => {
            ...
            init_notice("Gmail tool is available if you've run `gcode login google`.");
            unlock_model_provider();
            Arc::new(provider::MultiProvider::new_fast())
        }
        ProviderChoice::Auto => {
            ...
            if availability.has_any_provider() {
                ...
                crate::env::set_var("GCODE_ACTIVE_PROVIDER", multi.name().to_lowercase());
                Arc::new(multi)
            } else {
                let non_interactive = std::env::var("GCODE_NON_INTERACTIVE").is_ok();
                if non_interactive {
                    anyhow::bail!(
                        "No credentials configured. Run 'gcode login' or set ANTHROPIC_API_KEY to authenticate."
                    );
                }
