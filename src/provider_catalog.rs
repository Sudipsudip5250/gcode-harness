pub use gcode_provider_metadata::*;
use std::collections::{HashMap, HashSet};

pub const OPENAI_COMPAT_LOCAL_ENABLED_ENV: &str = "GCODE_OPENAI_COMPAT_LOCAL_ENABLED";
...
pub fn resolve_openai_compatible_profile(
    ...
    if let Some(base) = env_override("GCODE_OPENAI_COMPAT_API_BASE") {
        if let Some(normalized) = normalize_api_base(&base) {
            resolved.api_base = normalized;
        } else {
            eprintln!(
                "Warning: ignoring invalid GCODE_OPENAI_COMPAT_API_BASE '{}'. Use https://... (or http://localhost).",
                base
            );
        }
    }

    if let Some(key_name) = env_override("GCODE_OPENAI_COMPAT_API_KEY_NAME") {
        if is_safe_env_key_name(&key_name) {
            resolved.api_key_env = key_name;
        } else {
            eprintln!(
                "Warning: ignoring invalid GCODE_OPENAI_COMPAT_API_KEY_NAME '{}'.",
                key_name
            );
        }
    }

    if let Some(env_file) = env_override("GCODE_OPENAI_COMPAT_ENV_FILE") {
        if is_safe_env_file_name(&env_file) {
            resolved.env_file = env_file;
        } else {
            eprintln!(
                "Warning: ignoring invalid GCODE_OPENAI_COMPAT_ENV_FILE '{}'.",
                env_file
            );
        }
    }

    if let Some(setup_url) = env_override("GCODE_OPENAI_COMPAT_SETUP_URL") {
        resolved.setup_url = setup_url;
    }

    if let Some(model) = env_override("GCODE_OPENAI_COMPAT_DEFAULT_MODEL") {
...
pub fn active_openai_compatible_display_name() -> Option<String> {
    if let Ok(profile_name) = std::env::var("GCODE_NAMED_PROVIDER_PROFILE") {
    ...
    if let Ok(namespace) = std::env::var("GCODE_OPENROUTER_CACHE_NAMESPACE") {
    ...
    let api_base = std::env::var("GCODE_OPENROUTER_API_BASE")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
        .or_else(|| env_override("GCODE_OPENAI_COMPAT_API_BASE"));
...
fn apply_openai_compatible_profile_env_impl(
    ...
    if respect_named_profile_lock && std::env::var_os("GCODE_PROVIDER_PROFILE_ACTIVE").is_some() {
        return;
    }
    ...
    let vars = [
        ...
        "GCODE_OPENROUTER_AUTH_HEADER",
        "GCODE_OPENROUTER_AUTH_HEADER_NAME",
        "GCODE_OPENROUTER_DYNAMIC_BEARER_PROVIDER",
    ...
    if let Some(profile) = profile {
        ...
        crate::env::set_var("GCODE_OPENROUTER_API_BASE", &resolved.api_base);
        crate::env::set_var("GCODE_OPENROUTER_API_KEY_NAME", &resolved.api_key_env);
        crate::env::set_var("GCODE_OPENROUTER_ENV_FILE", &resolved.env_file);
        crate::env::set_var("GCODE_OPENROUTER_CACHE_NAMESPACE", &resolved.id);
        crate::env::set_var("GCODE_OPENROUTER_PROVIDER_FEATURES", "0");
        let static_models = openai_compatible_profile_static_models(profile);
        if static_models.is_empty() {
            crate::env::remove_var("GCODE_OPENROUTER_STATIC_MODELS");
        } else {
            crate::env::set_var("GCODE_OPENROUTER_STATIC_MODELS", static_models.join("\n"));
        }
        if resolved.requires_api_key {
            crate::env::remove_var("GCODE_OPENROUTER_ALLOW_NO_AUTH");
        } else {
            crate::env::set_var("GCODE_OPENROUTER_ALLOW_NO_AUTH", "1");
        }
    }
}
...
fn inline_key_env_name(profile_name: &str) -> String {
    let suffix = profile_name
        ...
        .collect::<String>();
    format!("GCODE_PROVIDER_{}_API_KEY", suffix)
}
...
pub fn apply_named_provider_profile_env_from_config(
    ...
    crate::env::remove_var("GCODE_PROVIDER_PROFILE_ACTIVE");
    crate::env::remove_var("GCODE_PROVIDER_PROFILE_NAME");
    crate::env::remove_var("GCODE_NAMED_PROVIDER_PROFILE");
    apply_openai_compatible_profile_env(None);
    crate::env::set_var("GCODE_OPENROUTER_API_BASE", &api_base);
    crate::env::set_var("GCODE_OPENROUTER_CACHE_NAMESPACE", profile_name);
    crate::env::set_var("GCODE_NAMED_PROVIDER_PROFILE", profile_name);
    ...
    if let Some(model) = profile
        ...
        .filter(|v| !v.is_empty())
    {
        crate::env::set_var("GCODE_OPENROUTER_MODEL", model);
    }
    ...
    match profile.auth {
        ...
        crate::config::NamedProviderAuth::Bearer | crate::config::NamedProviderAuth::Header => {
            ...
            if let Some(key_env) = key_env {
                ...
                crate::env::set_var("GCODE_OPENROUTER_API_KEY_NAME", &key_env);
            }
                ...
                .filter(|v| !v.is_empty())
            {
                ...
                crate::env::set_var("GCODE_OPENROUTER_ENV_FILE", env_file);
            }
            ...
            match profile.auth {
                crate::config::NamedProviderAuth::Bearer => {
                    crate::env::set_var("GCODE_OPENROUTER_AUTH_HEADER", "bearer");
                }
                crate::config::NamedProviderAuth::Header => {
                    crate::env::set_var("GCODE_OPENROUTER_AUTH_HEADER", "api-key");
                    if let Some(header) = profile
                        ...
                        .filter(|v| !v.is_empty())
                    {
                        crate::env::set_var("GCODE_OPENROUTER_AUTH_HEADER_NAME", header);
                    }
                }
                crate::config::NamedProviderAuth::None => {}
            }
