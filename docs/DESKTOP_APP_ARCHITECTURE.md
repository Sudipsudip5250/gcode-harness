...
```text
crates/
  gcode-desktop-protocol/   # shared protocol/event types if not already covered by server types
  gcode-desktop-ui/         # UI tree, layout, text/cache abstractions, renderer-agnostic pieces
  gcode-desktop-renderer/   # wgpu renderer and GPU resources
  gcode-desktop/            # app shell, platform window, protocol client, product UI
```
