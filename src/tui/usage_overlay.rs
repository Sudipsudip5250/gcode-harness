...
pub use gcode_tui_usage_overlay::{UsageOverlayItem, UsageOverlayStatus, UsageOverlaySummary};
use ratatui::{
    prelude::*,
...
impl UsageOverlay {
    ...
    fn apply_filter(&mut self) {
        self.filtered = self
            ...
            .enumerate()
            .filter_map(|(idx, item)| {
                gcode_tui_usage_overlay::item_matches_filter(item, &self.filter).then_some(idx)
            })
            .collect();
