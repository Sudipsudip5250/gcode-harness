...
gcode has two release paths: a fast local path for hotfixes, and CI for full releases.
...
5. Users can immediately run `gcode update`
...
~153s  Binaries uploaded, release live
         ✅ Linux + macOS users can `gcode update`
~16m   CI finishes Windows build, uploads to same release
         ✅ Windows users can `gcode update`
```
...
Tag push (v*)
    ...
    │     ├─► Update Homebrew formula (bitan-del/homebrew-gcode-harness)
    │     └─► Update AUR package (gcode-bin)
    │
...
- **Homebrew**: Updates `Formula/gcode.rb` in `bitan-del/homebrew-gcode-harness` with new SHA256 hashes
- **AUR**: Updates `PKGBUILD` and `.SRCINFO` in the `gcode-bin` AUR repo
...
`cargo-zigbuild` can cross-compile pure Rust code to macOS, but gcode depends on crates that link against macOS system frameworks:
...
The bottleneck is compiling gcode itself (120k lines of Rust). Dependencies are cached and don't need recompilation. The `build.rs` timestamp causes a full recompile of the main crate on every build.
