...
///
/// e.g. `/run/user/1000/gcode.sock` -> `\\.\pipe\gcode`
/// e.g. `/run/user/1000/gcode/myserver.sock` -> `\\.\pipe\gcode-myserver`
fn path_to_pipe_name(path: &Path) -> String {
    let stem: String = path
        ...
        .take(32)
        .collect();
    let stem = if stem.is_empty() { "gcode" } else { &stem };
    let normalized = path
        .to_string_lossy()
        .replace('\\', "/")
        .to_ascii_lowercase();
...
impl Stream {
    ...
    pub fn pair() -> io::Result<(Self, Self)> {
        ...
        let pipe_name = format!(r"\\.\pipe\gcode-pair-{}-{}", std::process::id(), counter);
