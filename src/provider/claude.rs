...
use gcode_provider_core::NativeToolResultSender;
...
/// Native tools that gcode handles locally (not Claude Code built-ins)
...
impl ClaudeCliConfig {
    fn from_env() -> Self {
        let cli_path = std::env::var("GCODE_CLAUDE_CLI_PATH")
            .ok()
            .filter(|value| !value.trim().is_empty())
            .unwrap_or_else(|| "claude".to_string());

        let mut model = std::env::var("GCODE_CLAUDE_CLI_MODEL")
            .ok()
            .filter(|value| !value.trim().is_empty())
        ...
        let permission_mode = std::env::var("GCODE_CLAUDE_CLI_PERMISSION_MODE")
            .ok()
            .filter(|value| !value.trim().is_empty())
            .or_else(|| {
                std::env::var("GCODE_CLAUDE_SDK_PERMISSION_MODE")
                    .ok()
                    .filter(|value| !value.trim().is_empty())
            })
            .or_else(|| Some(DEFAULT_PERMISSION_MODE.to_string()));

        let include_partial_messages = std::env::var("GCODE_CLAUDE_CLI_PARTIAL")
            .ok()
            .or_else(|| std::env::var("GCODE_CLAUDE_SDK_PARTIAL").ok())
            .map(|value| {
