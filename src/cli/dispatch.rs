        ...
        match resolve_resume_id(resume_id) {
            ...
            Err(e) => {
                ...
                if !output::quiet_enabled() {
                    eprintln!("\nUse `gcode --resume` to list available sessions.");
                }
                std::process::exit(1);
            }
        }
...
async fn run_default_command(args: Args) -> Result<()> {
    ...
    let in_gcode_repo = build::is_gcode_repo(&cwd);
    startup_profile::mark("is_gcode_repo");
    let already_in_selfdev = crate::cli::selfdev::client_selfdev_requested();

    if in_gcode_repo && !already_in_selfdev && !args.no_selfdev {
        output::stderr_info("📍 Detected gcode repository - enabling self-dev mode");
    ...
    if !server_running && std::env::var("GCODE_RESUMING").is_ok() {
    ...
    if std::env::var("GCODE_RESUMING").is_err() && server_running {
