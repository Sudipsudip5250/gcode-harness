...
fn write_test_api_key(temp: &TempDir, env_file: &str, env_key: &str, value: &str) {
    let config_dir = test_config_dir(temp).join("gcode");
...
fn named_openai_compatible_provider_sets_catalog_cache_namespace() {
    ...
    let _namespace = EnvVarGuard::remove("GCODE_OPENROUTER_CACHE_NAMESPACE");
    ...
    assert_eq!(
        std::env::var("GCODE_OPENROUTER_CACHE_NAMESPACE").as_deref(),
        Ok("example-compat")
    );
}
...
fn comtegra_profile_uses_endpoint_default_max_tokens() {
    let _lock = ENV_LOCK.lock().unwrap();
    let _override = EnvVarGuard::remove("GCODE_OPENROUTER_MAX_TOKENS");
...
fn max_tokens_env_overrides_profile_default() {
    let _lock = ENV_LOCK.lock().unwrap();
    let _override = EnvVarGuard::set("GCODE_OPENROUTER_MAX_TOKENS", "4096");
...
fn test_configured_api_base_accepts_https() {
    let _lock = ENV_LOCK.lock().unwrap();
    let prev = std::env::var("GCODE_OPENROUTER_API_BASE").ok();
    crate::env::set_var(
    ...
    if let Some(value) = prev {
        crate::env::set_var("GCODE_OPENROUTER_API_BASE", value);
    } else {
...
fn test_configured_api_base_rejects_insecure_http_remote() {
    let _lock = ENV_LOCK.lock().unwrap();
    let prev = std::env::var("GCODE_OPENROUTER_API_BASE").ok();
    crate::env::set_var("GCODE_OPENROUTER_API_BASE", "http://example.com/v1");
...
fn autodetects_single_saved_openai_compatible_profile() {
    ...
    let _openrouter_base = EnvVarGuard::remove("GCODE_OPENROUTER_API_BASE");
    let _openrouter_key = EnvVarGuard::remove("GCODE_OPENROUTER_API_KEY_NAME");
    let _openrouter_file = EnvVarGuard::remove("GCODE_OPENROUTER_ENV_FILE");
    let _openrouter_dynamic = EnvVarGuard::remove("GCODE_OPENROUTER_DYNAMIC_BEARER_PROVIDER");
...
fn autodetects_single_saved_local_openai_compatible_profile() {
    ...
    let _openrouter_file = EnvVarGuard::remove("GCODE_OPENROUTER_ENV_FILE");
    let _openrouter_dynamic = EnvVarGuard::remove("GCODE_OPENROUTER_DYNAMIC_BEARER_PROVIDER");
    let _openrouter_no_auth = EnvVarGuard::remove("GCODE_OPENROUTER_ALLOW_NO_AUTH");
    let _openrouter_api_key = EnvVarGuard::remove("OPENROUTER_API_KEY");
...
fn autodetected_profile_seeds_default_model_and_cache_namespace() {
    ...
    let _openrouter_file = EnvVarGuard::remove("GCODE_OPENROUTER_ENV_FILE");
    let _openrouter_dynamic = EnvVarGuard::remove("GCODE_OPENROUTER_DYNAMIC_BEARER_PROVIDER");
    let _openrouter_model = EnvVarGuard::remove("GCODE_OPENROUTER_MODEL");
    let _openrouter_cache_ns = EnvVarGuard::remove("GCODE_OPENROUTER_CACHE_NAMESPACE");
...
fn direct_deepseek_profile_uses_static_1m_context_when_catalog_is_absent() {
    let _lock = ENV_LOCK.lock().unwrap();
    let _base = EnvVarGuard::set("GCODE_OPENROUTER_API_BASE", "https://api.deepseek.com");
    let _key_name = EnvVarGuard::set("GCODE_OPENROUTER_API_KEY_NAME", "DEEPSEEK_API_KEY");
    let _api_key = EnvVarGuard::set("DEEPSEEK_API_KEY", "test");
    let _namespace = EnvVarGuard::set("GCODE_OPENROUTER_CACHE_NAMESPACE", "deepseek");
    let _model = EnvVarGuard::set("GCODE_OPENROUTER_MODEL", "deepseek-v4-flash");
    let _catalog = EnvVarGuard::set("GCODE_OPENROUTER_MODEL_CATALOG", "0");
